...
incomplete, deliberately skipped. **Chapter 38 (Windows/Wine), the first Part VIII chapter,
also got a first pass this session.** Full per-chapter detail (exact line counts, headers
read, every finding and example) lives in `PLAN.md`'s session log — this file only summarizes.
...
None of the 13 chapters touched this session (25-36, 38) are at their final page targets yet
(roughly 3-10 of a 30-70-page target each) — this was a breadth pass, not a depth pass.
...
Every chapter touched this session (25 through 36, and 38) has needed at least one of these.
...
1. **Continue through Part VIII** (Chapters 39-41: Web/Emscripten, Android/NDK, Verification
   Methodology) — same approach as Chapter 38: check whether each is API-reference-shaped (add
   worked examples grounded in real headers) or tooling/narrative-shaped like 38 (add real
   shell/build invocations grounded in the actual scripts/CI config). Read the real source
   before assuming which kind it is.
2. **Then Part IX** (Chapters 42-48): Porting/practice/roadmap — none touched yet this session.
3. **Alternatively, return to Part V, VI, or 38 for real depth**: every chapter touched this
   session has a first pass now, but all are still well short of their page targets. Don't
