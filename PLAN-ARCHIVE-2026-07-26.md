...
**Status: Phase 1 is deliberately paused at a CNA-stabilization checkpoint. The current book is
575 physical PDF pages (49 chapters + 6 appendices), fully rebuilt and reference-checked on
2026-07-26. Resume only after the CNA audit/backend feature work has stabilized, then re-audit
the source-backed matrices before extending them. This is a long-running, multi-session project
— this file is the durable task list and progress tracker across sessions. Update the status
column and the session log at the bottom every session.**
...
`main.pdf` is 575 pages; the targeted undefined-reference and duplicate-label checks are empty;
makeindex accepted 2,367 entries with no
...
current fully rebuilt total of 575 physical PDF pages.
...
- **2026-07-26, autonomous continuation — Chapter 14 state-object submission and backend-binding
  boundary audited.** This record supersedes the Chapter~14 table row's former **353**-line
  figure: Ch.14 is now **397** lines. Traced the four CNA state classes and collections,
  GraphicsDevice whole-state setters and draw routes, common backend interface and sampler caches,
  CPU/backend examples, and current local FNA state/device implementations. No CNA source
  changed; the sibling repository remained read-only.

  CNA assigns BlendState, DepthStencilState, and RasterizerState by value, then calls the
  corresponding backend application method. Mutating an original local value changes neither the
  device copy nor GPU. More importantly, mutating the C++ device getter changes only the CPU
  shadow; only re-assignment calls ApplyBlendState/ApplyDepthStencilState/ApplyRasterizerState.
  The former CNA test comment that FNA reference aliasing makes a bound Blend/Depth mutation
  dirty on the next draw is contradicted by FNA's current identity-only flush. FNA reapplies
  RasterizerState each draw, but that is not a portable update route.

  The common bridge carries less than the public value objects: BlendState sends six
  source/destination/function values and BlendFactor but no ColorWriteChannels or multisample
  mask; GraphicsDevice.MultiSampleMask is only a C++ shadow. DepthStencilState sends every
  whole-state field, while standalone ReferenceStencil retains the documented backend-dependent
  caveat. RasterizerState omits MultiSampleAntiAlias. Every CNA draw refreshes all 16 pixel
  sampler slots but carries only Filter, AddressU, AddressV, and MaxAnisotropy; AddressW,
  MaxMipLevel, and LOD bias never cross the interface, and vertex texture/sampler collections
  have no draw route. Existing CPU tests cover values/names/round trips/bounds and examples cover
  selected supported fields, not these omitted routes.

  Full `make -C latex book` succeeded at **565 pages**; target undefined-reference and
  duplicate-label checks were empty; makeindex accepted **2,316** entries with zero
  rejection/warning; newly added overfull boxes were removed; and `git diff --check` passed.
  Rendered physical pages **217--218** (printed **193--194**) are clean. Chapter 14 ends on
  physical page **224** (printed **200**) before Chapter 15. The plan-consistency validator
  remains absent from this checkout.

- **2026-07-26, autonomous continuation — Chapter 15 ShaderEffect object, submission, and clone
  boundary audited.** This record supersedes the Chapter~15 table row's former **333**-line
  figure: Ch.15 is now **379** lines. Traced `ShaderEffect` and `Effect` implementations and
  headers, `GraphicsResource` disposal, the common effect-backend contract, EasyGL and Headless
  submission behavior, focused clone coverage, and the already-recorded backend matrix. No CNA
  source changed; the sibling repository remained read-only.

  `ShaderEffect` copies both source strings at construction, calls the device factory once when a
  backend exists, and exposes only a boolean for both null-backend and compile-failed states. A
  returned invalid backend emits its compile diagnostic to stderr; the class exposes no diagnostic
  accessor and does not retry or recompile automatically. `Effect::Apply()` rejects disposal but
  sets the current effect after `ShaderEffect::OnApply()` merely logs an invalid/absent backend,
  so an invalid apply can still displace the device's current effect without binding a fallback.

  `SetUniformXxx()` and `SetTexture()` are direct backend submissions, not entries in an XNA-style
  `EffectParameterCollection`: the class retains no uniform table, sampler table, or texture
  owner and does no common name/unit/count/pointer validation. EasyGL acts immediately; other
  backends may stage, record, or inherit a no-op. The standalone setters do not check disposed
  state, unlike `Apply()`. `Clone()` returns a caller-owned raw pointer constructed only from
  device and copied sources: it recompiles an independent program, resets World/View/Projection
  to identity, and does not carry submitted uniforms/textures, name/tag, or program state. The
  only current clone test checks fresh identity, equal sources, and equal default-device validity;
  it does not test real compilation or state/lifetime/error paths.

  Full `make -C latex book` succeeded at **567 pages**; target undefined-reference and
  duplicate-label checks were empty; makeindex accepted **2,323** entries with zero
  rejection/warning; no added Chapter~15 overfull box appeared; and `git diff --check` passed.
  Rendered physical pages **227--228** (printed **203--204**) are clean. Chapter 15 occupies
  physical pages **225--232** (printed **201--208**) before Part IV / Chapter 16. The
  plan-consistency validator remains absent from this checkout.

- **2026-07-26, autonomous continuation — Chapter 16 generic resource ownership and device
  tracking boundary audited.** This record supersedes the Chapter~16 table row's former
  **1,842**-line figure: Ch.16 is now **1,905** lines. Traced `GraphicsDevice::Dispose`,
  `GraphicsResource` construction/copy/move/disposal, public texture/buffer/render-target
  ownership, current lifetime guidance, move/copy examples, focused CPU tests, and FNA's managed
  reference model. No CNA source changed; the sibling repository remained read-only.

  Normal construction registers one raw resource address, while each derived public resource
  owns its backend (usually uniquely; `Texture2D` deliberately shares it). Device disposal moves
  and clears the raw list, disposes the resources while the backend/context remains alive, then
  destroys the native backend. It is re-entrant-safe. `ResourceDestroyed` is invoked by the
  common base even when a C++ destructor takes the `Dispose(false)` path; only the resource's
  own `Disposing` event is conditional. The sibling lifetime guide says otherwise, so that narrow
  claim is stale against live source.

  The registration model is address-based: base copy/move operations carry `graphicsDevice_` but
  neither register the destination nor replace the old raw entry. A `Texture2D` value copy shares
  the backend and has independent disposal state, but the alias is untracked. Move construction
  transfers a handle while retaining the moved-from registration; same-device assignment only
  happens to retain the destination's old registration, and cross-device assignment transfers
  nothing coherently. If an untracked surviving value outlives device disposal, it can release a
  native backend after its context/device. Existing move and copy examples prove handle survival/
  no-double-free during a live device, not tracker/event accounting or device-first teardown.

  **needs_human / source-repository follow-up:** `GraphicsResource` needs an ownership-safe,
  address-transfer-aware design (or tracked GPU resources need non-movable/non-copyable APIs,
  with a deliberate `Texture2D` alias policy), plus tests for copy/move then device-first
  disposal. This book checkout does not own `../cna`; the finding is documented here rather than
  patched there.

  Full `make -C latex book` succeeded at **569 pages**; target undefined-reference and
  duplicate-label checks were empty; makeindex accepted **2,335** entries with zero
  rejection/warning; no added Chapter~16 overfull box appeared; and `git diff --check` passed.
  Rendered physical pages **236--237** (printed **212--213**) are clean. Chapter 16 occupies
  physical pages **235--266** (printed **211--242**) before Chapter 17. The plan-consistency
  validator remains absent from this checkout.

- **2026-07-26, autonomous continuation — Chapter 16 presentation-mode contract audited and
  stabilization checkpoint recorded.** This record supersedes the Chapter~16 table row's former
  **2,117**-line figure: Ch.16 is now **2,254** lines. Traced the public enum and manager/device
  delivery order, backend-create arguments and Reset path, all fourteen factory products' virtual
  resolution/mode/viewport/presentation implementations, relevant backend examples/tests, and
  the already-documented input/readback routes. No CNA source changed; the sibling repository
  remained read-only.

  `PresentationMode` belongs only to `GraphicsDeviceManager`, defaults to
  `FixedHeightDynamicWidth`, and is set before manager Reset forwards virtual dimensions. Direct
  device construction leaves the creation argument at that default; the private GraphicsDevice
  virtual-resolution helper has no located caller because construction and Reset call the backend
  hook directly. The new matrix corrects the previous implied universal contract: SDL_RENDERER
  and ASCII delegate to SDL; SDL GPU/WebGPU compute all five policies; EasyGL/Canvas only have
  fixed-height bookkeeping plus whole-window non-default mapping; Vulkan/BGFX/D3D do not create a
  mode-specific compositor; Software/Headless replace or simulate their grid; and DX3 remains
  hard-coded letterbox through free-direct. Ch.6 and Ch.9 now describe this as a manager request
  with backend-defined execution rather than promising bars, crops, or scaling everywhere.

  The author directed a pause after this coherent checkpoint while CNA's `feature/audit` and
  pending graphics-backend branches stabilize. On resumption, identify the integrated CNA source
  baseline and re-audit this matrix plus its Ch.6/9/backend cross-references before extending the
  book; do not treat the present results as a post-integration compatibility promise.

  Full `make -C latex book` succeeded at **575 pages**; target undefined-reference and
  duplicate-label checks were empty; makeindex accepted **2,367** entries with zero
  rejection/warning; no added Chapter~16 overfull box appeared; and `git diff --check` passed.
  Rendered physical pages **243--245** (printed **219--221**) are clean. Ch.16 now spans
  physical pages **235--271** (printed **211--247**), followed by the intentional
  physical-page-272 chapter-boundary blank before Chapter~17. The plan-consistency validator
  remains absent from this checkout.

- **2026-07-26, autonomous continuation — Chapter 16 static window-registry lifetime and
  collision boundary audited.** This record supersedes the Chapter~16 table row's former
  **2,044**-line figure: Ch.16 is now **2,117** lines. Traced the common
  `IGraphicsBackend` registry, all registration/unregistration sites, the SDL input read and
  `Mouse::SetPosition` inverse routes, GraphicsDevice construction/replacement/attached-window
  paths, focused Canvas/input tests, and the local FNA mouse implementation. No CNA source
  changed; the sibling repository remained read-only.

  The registry is one process-global `SDL_Window* -> IGraphicsBackend*` raw map. Lookup drives
  the second input-transform tier in both directions after SDL-renderer conversion is unavailable;
  only EasyGL, Canvas, SDL GPU, and WebGPU register. Assignment overwrites silently and erase
  knows only the window key. EasyGL registers before creating its GL context and later fallible
  initialization, so a constructor throw bypasses its destructor and leaves a dangling map
  pointer; the other three register at the end of their relevant setup. Serial multisample
  replacement removes the old entry before attempting the new backend, but a failed EasyGL
  replacement can retain that early stale entry.

  Two devices may receive the same borrowed public `DeviceWindowHandle`: the later registrant
  replaces the earlier pointer, then the earlier destructor erases the survivor's mapping.
  Address reuse has the analogous raw-key risk. There is no map synchronization or stated
  thread-affinity guarantee, so input lookup concurrent with backend creation/destruction is
  unsafe. The Canvas fake-window tests prove only construction; no located test covers lookup,
  duplicate attachment, owner-aware erase, post-registration construction failure, address reuse,
  or concurrency. FNA's ratio conversion has no equivalent backend-pointer registry.

  **needs_human / source-repository follow-up:** choose an explicit one-window/one-device versus
  multi-device routing contract, pair registration with fallible construction through RAII or
  late registration, make unregistration owner-aware, and define thread affinity or locking.
  The public attached-window compatibility implications make this a source-policy decision, not
  a safe book-only implementation choice.

  Full `make -C latex book` succeeded at **573 pages**; target undefined-reference and
  duplicate-label checks were empty; makeindex accepted **2,364** entries with zero
  rejection/warning; no added Chapter~16 overfull box appeared; and `git diff --check` passed.
  Rendered physical pages **241--243** (printed **217--219**) are clean. Ch.16 now spans
  physical pages **235--269** (printed **211--245**), followed by the intentional
  physical-page-270 chapter-boundary blank before Chapter~17. The plan-consistency validator
  remains absent from this checkout.

- **2026-07-26, autonomous continuation — Chapter 16 native SDL-handle bridge audited.** This
  record supersedes the Chapter~16 table row's former **1,969**-line figure: Ch.16 is now
  **2,044** lines. Traced the common backend handle contract, all backend implementations,
  Game/GameWindow construction and destruction, GraphicsDevice attached/owned window paths,
  TextInputEXT and Mouse routes, focused GameWindow/input tests, smoke examples, and FNA's
  ordinary window setup. No CNA source changed; the sibling repository remained read-only.

  `GetNativeSdlWindowEXT()` and `GameWindow::Handle` are borrowed views, not ownership
  transfers. The normal Game stores the device's raw SDL window in its GameWindow; the wrapper
  never destroys it. Headless/Software and D3D12 HeadlessEXT return null. SDL_RENDERER exposes an
  SDL renderer internally and ASCII forwards its inner renderer; every other backend, including
  DX3 with its private free-direct renderer, returns null. A non-null renderer pointer belongs to
  the current backend and cannot survive recreation/disposal. Device disposal destroys an owned
  SDL window without invalidating GameWindow's raw field, making an explicitly disposed device's
  retained GameWindow stale; normal member destruction avoids use after free only by order.

  An externally supplied `PresentationParameters::DeviceWindowHandle` is blindly cast to
  `SDL_Window*`, is not validated or destroyed, and must outlive backend teardown. Unlike the
  creator path it bypasses automatic TextInputEXT/Mouse handle publication and later clearing.
  Existing wrapper and smoke tests do not cover attached-device ownership/input behavior or a
  post-device-disposal GameWindow. **needs_human / source-repository follow-up:** decide whether
  an attached device owns the process-wide input target and whether disposal must invalidate the
  GameWindow view before adding tests/fixes; that is a multi-window compatibility decision.

  Ch.6 and Ch.9 now point readers to this detailed contract. Full `make -C latex book` succeeded
  at **571 pages**; target undefined-reference and duplicate-label checks were empty; makeindex
  accepted **2,361** entries with zero rejection/warning; `git diff --check` passed; and no
  added Chapter~16 overfull box appeared. Rendered physical pages **238--239** (printed
  **214--215**) are clean. Ch.16 spans physical pages **235--267** (printed **211--243**),
  followed by the intentional physical-page-268 chapter-boundary blank before Chapter~17. The
  plan-consistency validator remains absent from this checkout.

- **2026-07-26, autonomous continuation — Chapter 16 optional-factory result audited.** This
  record supersedes the Chapter~16 table row's former **1,905**-line figure: Ch.16 is now
  **1,969** lines. Traced the common optional factories, the public texture/query/render-target
  wrappers and binding state, D3D9's hardware probes, Software/Canvas/SDL\_RENDERER policy,
  GraphicsCapability, ShaderEffect binding, and focused tests. No CNA source changed; the
  sibling repository remained read-only.

  The common null defaults do not yield a uniform public ``unsupported'' result. OcclusionQuery
  deliberately becomes an inert object; a null Texture3D/TextureCube still validates its public
  data calls but discards writes and leaves reads untouched; null render targets still become
  public objects and make GraphicsDevice record a target/viewport after forwarding a null native
  pointer. D3D9 may take the cube/volume/query null paths after a per-device support probe;
  Software and Canvas intentionally inherit particular null defaults; SDL\_RENDERER documents its
  constructible-but-handleless 3D textures, but separately overrides its query factory to throw.

  Both null texture classes expose an unguarded NOXNA `GetBackend()` reference, and ShaderEffect
  cube/volume SetTexture calls it before delegating to its effect backend. That path is C++
  undefined behavior, not a documented capability exception. The broad ThreeD capability cannot
  preflight an individual cube/volume/target factory, only OcclusionQuery has a dedicated value,
  and existing tests cover normal construction/argument guards but not an injected null factory,
  read-buffer preservation, public bind state, or the ShaderEffect route.

  **needs_human / source-repository follow-up:** choose a compatible public policy before any
  repair: either fail public construction early with NotSupportedException, or preserve
  constructible unavailable objects while adding an explicit availability contract and guards to
  every access/binding route. This affects observable failure timing and must account for the
  intentional query fallback, so it is out of scope for this book-only checkout.

  Full `make -C latex book` succeeded at **569 pages**; target undefined-reference and
  duplicate-label checks were empty; makeindex accepted **2,343** entries with zero
  rejection/warning; no added Chapter~16 overfull box appeared; and `git diff --check` passed.
  Rendered physical pages **237--238** (printed **213--214**) are clean. Chapter 16 remains
  physical pages **235--266** (printed **211--242**) before Chapter 17. The plan-consistency
  validator remains absent from this checkout.
