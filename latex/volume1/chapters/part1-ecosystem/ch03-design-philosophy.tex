...
What makes this more than a documentation convention is the \texttt{NOXNA} CMake build
option described in Chapter~\ref{ch:what-is-cna}: with it enabled, every declaration inside
the XNA namespace tree that is \emph{not} tagged \texttt{NOXNA} is checked against the
expected XNA surface, and drift shows up as a \texttt{[[deprecated]]} warning under
\texttt{-Werror} --- turning ``did we accidentally leak a project-specific extension into the
public XNA-compatible surface'' from a code-review judgment call into a build failure.
...
\texttt{include/Microsoft/Xna/Framework/Graphics/}'' as the same thing throughout Part~III.
