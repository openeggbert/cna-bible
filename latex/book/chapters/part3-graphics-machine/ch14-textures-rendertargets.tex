...
renderer. \texttt{Dxt1} / \texttt{Dxt3} / \texttt{Dxt5} are the one partial exception, but only
...
EasyGL's render-target-cube renderer implements that upload. Every other constructed target
inherits the intentional empty \texttt{SetData()} body on the render-target renderer interface;
...
never entered into the renderer's 2D-only current-target tracker, so the following null call
...
call the two renderer methods directly and therefore prove the callback implementations, not
...
one Boolean to its renderer factory, but makes it true only for Preserve. Platform is therefore
delivered to a renderer as ``do not preserve.'' The later shared bind does the opposite
comparison and issues
its black implicit clear only for exact \texttt{DiscardContents}. On immediate renderers that
...
configuration. This is an observable three-way renderer split, not merely an unused hint.
...
the immediate persistent-resource renderers generally retain color. The property therefore
...
supported by any renderer today, everything else is silently treated as RGBA8 wherever it is
