...
\index{disposal!graphics resources}

Texture and render-target APIs are shared, but their native formats, readback paths, mipmaps,
multisampling, and attachment behavior differ substantially by renderer family.
\cnaclass{Texture2D} has the broadest coverage. \cnaclass{Texture3D} and
\cnaclass{TextureCube} now share the expected base type but retain narrower shader and renderer
evidence. Render targets still have named gaps. This chapter keeps those scopes separate.

\input{figures/fig-resource-lifetime.tex}
...
\section{Texture3D and TextureCube: current structure and history}
\label{sec:texture3d-inheritance}

The pinned headers declare both \texttt{Texture3D} and \texttt{TextureCube} as
\cnaclass{Texture} subclasses, as XNA does. Regression coverage assigns a
\cnaclass{Texture3D} to \texttt{GraphicsDevice.Textures[slot]} and checks that disposal releases
the renderer handle and unbinds the resource from the device's pixel and vertex texture
collections.

\begin{historynote}
Earlier revisions did not derive these classes from \cnaclass{Texture}; assignment to a
\cnaclass{TextureCollection} could not compile, and \cnaclass{Texture3D} skipped the base
unbind-on-dispose path. An earlier manuscript pass described that state. The current contract
above replaces it, while the regression preserves the reason for the change.
\end{historynote}
...
\texttt{SetTexture(int, TextureCube\&)} and \texttt{SetTexture(int, Texture3D\&)}
overloads, each binding straight to the matching GL texture target
...
\subsection{Building a texture from raw pixel data}

The following procedural checkerboard uses the constructor and \texttt{SetData} /
\texttt{GetData} signatures from \texttt{Texture2D.hpp}:
...
\subsection{Six faces, six SetData calls}
...
\subsection{Sampling a volume texture from a custom shader}
...
\subsection{Rendering into one RenderTargetCube face}
...
\begin{description}
  \item[Cube sampling.] Sampling a \cnaclass{RenderTargetCube} face after rendering into it,
        ...
        discriminating
        regressions: the clear-only case explicitly records the target, while the original
        render-all-faces/then-sample test no longer reproduces the black
