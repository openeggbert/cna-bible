...
backend-dependent in ways the call site cannot infer. EasyGL renders it correctly. Vulkan's
...
\section[Render-target divergences]{Render targets: a once-sharp per-backend divergence,
mostly closed since}
...
CNA splits those decisions inconsistently. \cnaclass{RenderTarget2D}'s constructor forwards
one Boolean to its backend factory, but makes it true only for Preserve. Platform is therefore
delivered to a backend as ``do not preserve.'' The later shared bind does the opposite
comparison and issues
its black implicit clear only for exact \texttt{DiscardContents}. On immediate backends that
ignore the Boolean, Platform content commonly survives because nothing overwrites the
persistent resource. Vulkan and WebGPU consume the false Boolean as a discard load operation,
so the same enum loses content there. BGFX can instead inherit a persistent view-clear
configuration. This is an observable three-way backend split, not merely an unused hint.

\cnaclass{RenderTargetCube} is less connected still. Its constructor stores and returns the
requested usage, but \texttt{CreateRenderTargetCube} has no preserve parameter and the
singular cube bind performs no implicit discard clear. Vulkan and WebGPU compensate in their
own pass construction by always discarding; SDL GPU loads unless an explicit clear was queued;
the immediate persistent-resource backends generally retain color. The property therefore
round-trips correctly while having no portable cube-content meaning.
...
cannot reach a cube backend at all.

Existing tests justify a deliberately narrow claim. SDL Renderer and EasyGL have
discriminating non-MSAA 2D color pixels for discard versus preserve; Vulkan has a two-frame
partial-draw test proving the same color distinction; BGFX's named usage test proves only
bind/unbind without a crash. No test covers \texttt{PlatformContents}, backbuffer usage,
cube usage, preserved depth/stencil, preserve plus MSAA, redundant binding, or a cube inside
\cnaclass{RenderTargetBinding}. The complete source/evidence matrix is
...
The render-target support ledger once listed five further per-backend divergences here
...
dimensions on every backend; whether those setters then change native rasterization is the separate
fourteen-product contract in \S\ref{sec:backend-viewport-scissor-matrix}.
...
cross-backend level --- throwing if a caller passes more than four bindings, regardless of
what any individual backend's own internal buffer happens to be sized for --- closing what
this section once described as no backend matching that real XNA limit at all.
