...
CNA does not leave custom shading entirely unaddressed, but ``cross-backend'' needs a precise
definition here. \cnaclass{ShaderEffect} (\texttt{NOXNA}) presents one constructor shape and
one \cnaclass{IEffectBackend} contract to every backend. A backend factory override proves
...
\toprule
\textbf{Backend} & \textbf{Input and path that actually consumes it} \\
\midrule
\endhead
\textbf{EasyGL} &
...
\textbf{SDL renderer} &
There is no effect backend, so construction leaves \texttt{IsEffectValid()} false. Passing the
non-null object to \cnaclass{SpriteBatch}\texttt{::Begin()} throws
\texttt{std::runtime\_error}; merely constructing it does not throw. \\
\bottomrule
\end{longtable}

WebGPU and DX3/free-direct likewise inherit the null \texttt{CreateEffectBackend()} default
...
\texttt{.fx} blob: it must provide backend-specific source for EasyGL, SDL GPU, and Direct3D,
...
\texttt{customEffectBackend} pointer. \cnaclass{ShaderEffect} populates it through its
...
facilities. The default-no-op setters on \cnaclass{IEffectBackend}
...
that concrete backend overrides it.
...
storage. If the device has a backend, construction asks it once for an
\cnaclass{IEffectBackend}. A returned but invalid backend does not make the common constructor
throw: it writes that backend's \texttt{GetCompileError()} text to standard error and leaves
\texttt{IsEffectValid()} false. A null backend and a failed compile are intentionally collapsed
to the same public boolean, and there is no public \cnaclass{ShaderEffect} accessor for the
diagnostic. Backends may of course fail before returning from their own factory, so this is not a
promise that every backend-specific setup failure becomes a boolean.
...
message when its backend is absent or invalid; it does not bind a fallback program. Calling
...
new effect after correcting source or changing the backend circumstances.
...
backend if one exists, while each \texttt{SetTexture()} immediately forwards the address of the
texture's backend. It performs no common-layer check of a uniform name, sampler-unit range,
array count, or matrix/array pointer. EasyGL uploads/binds at that call; some other backends copy
into a fixed staging block; inherited interface defaults can do nothing; and Headless records a
raw texture-backend pointer for tracing. Thus a successful call is not a portable guarantee of a
binding, and it never gives the effect ownership of the texture. Keep every texture alive by its
normal owner and set it again whenever the target backend/path requires rebinding. The standalone
setters also do not inspect \texttt{IsDisposed}; unlike \texttt{Apply()}, they continue to
forward while the backend object remains alive, so disposal must be treated as terminal by the
caller.
...
backend program and restores the three \cnaclass{IEffectMatrices} values to identity. It does
...
compile, matrix/state transfer, texture lifetime, invalid-apply behavior, or a backend reset.
...
raw SPIR-V bytes. The comment is stale cross-backend documentation, not a constraint enforced
...
\textbf{EasyGL support}; no other backend's general 3D draw dispatch currently reads
\texttt{GpuDrawParams::customEffectBackend}. For a long stretch of this project's history that was the only
...
\texttt{IVertexBuffer}\allowbreak{}\texttt{Backend::}\allowbreak{}%
...
backend is whether a \cnaclass{ShaderEffect} rewrite is usable at all and which contract it
...
BGFX has no valid public path; and the non-programmable or not-yet-wired backends reject,
