...
A \cnaclass{VertexDeclaration} is immutable after construction.  Each element stores an offset,
...
computed as the largest \(\text{offset}+\text{format size}\).  Empty element lists throw
\cnaclass{ArgumentNullException}.  Non-positive explicit strides throw the out-of-range exception.
...
SDL\_GPU similarly inherits a positive query answer without a query implementation.  Appendix
\ref{ch:feature-matrix} records those contradictions rather than copying the Boolean as support.
...
Part~IV's renderer chapters classify the actual destination of each family instead of inferring it
