...
Every public property round-trips in the small C\texttt{++} value objects, but the common renderer
interface is narrower. This is a structural limitation above any individual renderer mapping:
...
\begin{itemize}
  \item \cnaclass{BlendState} forwards its six source/destination/function values. Its
    ...
    mask and \texttt{MultiSampleMask} is absent from the renderer call. The device's own
    \texttt{MultiSampleMask} is only a C\texttt{++} integer shadow; unlike FNA's native setter,
    it makes no renderer call.
...
defaults, names, round trips, and bounds, while renderer examples exercise selected supported
pixel-sampler and state fields. No located test asks whether a getter mutation reaches a renderer,
...
current cross-renderer status in \texttt{docs/graphics-renderer-feature-matrix.md} is fully
correct on EasyGL, BGFX, SDL\_Renderer, and all three Direct3D renderers. Vulkan is the one
renderer with a real, historically serious bug in that forwarded subset --- for a long time its
...
\cnaclass{BlendFunction} mapping across all nine of the renderer's pipeline-creation sites. That
...
\texttt{SpriteBatch::Begin()} currently discards it before any renderer sees it.
...
no AddressW, MaxMipLevel, or LOD-bias argument. Renderer caches consequently use their own
