...
a no-op default any renderer may override --- reading each renderer's own header directly finds
...
renderers is also closed now: a stencil-only \texttt{Clear} call once never even checked its
own flag on any renderer (Task~871), but a real fix now dispatches it
...
made the requested stencil \emph{value} real on all three hardware renderers; later
