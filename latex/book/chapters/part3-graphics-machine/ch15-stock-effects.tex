...
\cnaclass{CompareFunction} sweep passed 24 of 24 cases on all three renderers. One real,
...
own tracking as ``a genuinely large, six-shader-file, two-renderer change,'' not attempted.
...
no-op, because neither renderer's alpha-test shader declares a color vertex attribute at all. A
game targeting all three hardware renderers should not rely on \cnaclass{AlphaTestEffect}'s
...
The audit's headline finding is a real formula bug present on all three renderers at once:
FNA's real shader doubles the first texture's RGB value (\texttt{color.rgb *= 2}) before
multiplying it against the overlay texture, and none of CNA's three renderer shaders
...
in either renderer's dual-texture shader).
...
to all three renderer shaders at once. \texttt{VertexColorEnabled} carries the identical
Vulkan/BGFX no-op gap \cnaclass{AlphaTestEffect} has, for the same underlying reason (no color
vertex attribute in either renderer's dual-texture shader) --- a lightmapped floor mesh that also
tries to carry per-vertex tint runs into the same wall on those two renderers.
...
any renderer at all --- and every renderer's only lit shader computed lighting in the fragment
stage unconditionally. The consequence, stated plainly: \textbf{CNA rendered per-pixel lighting
always, on all nine renderers, the exact opposite of XNA's own default} (per-vertex, Gouraud-%
...
\texttt{false}. Seven of the other eight renderers needed a genuinely new per-vertex-lit shader
...
analogy to the other eight. Doing this for real on \texttt{Software} would mean building a CPU
...
As of this writing, then: \texttt{preferPerPixelLighting} is genuinely real on seven of CNA's
nine renderers (\texttt{D3D9}, \texttt{EasyGL}, \texttt{Vulkan}, \texttt{BGFX},
...
renderer (\texttt{Software}) --- not the blanket ``ignored by every renderer except D3D9'' this
...
wired to any actual GPU state on one or more renderers (\texttt{VertexColorEnabled} on
Vulkan/BGFX in two different effects, \texttt{WeightsPerVertex}); or a bug shared identically
across all three hardware renderers because the shared, renderer-agnostic C\texttt{++} code
