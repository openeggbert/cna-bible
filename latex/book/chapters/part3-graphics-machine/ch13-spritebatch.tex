...
also the one class every one of the fourteen backend products in Part~IV must implement --- the class
...
clipping on any backend; a previously-enabled state can also leak through a later default
...
\ref{sec:backend-viewport-scissor-matrix} then determines whether the selected backend can
actually honor it.
\end{sourcenote}
...
void SpriteBatch::pushSprite(...)
{
    ...
    if (sortMode_ == SpriteSortMode::Immediate)
        flushSingle(info);              // dispatched to the backend right here, right now
    else
...
void SpriteBatch::flushBatch()
{
    ...
    for (const SpriteInfo& s : spriteQueue_)
        flushSingle(s);                 // still one backend->Draw() call per sprite
    spriteQueue_.clear();
}
\end{lstlisting}
...
meaning each sprite reaches the backend the instant it is submitted, with no queue involved at
all; this is the only sort mode where a game can safely change render state (a different
\cnaclass{Texture2D}'s \cnaclass{SamplerState}, say, via a raw backend call) \emph{between}
...
its own individual \texttt{backend->Draw()} call inside the \texttt{flushBatch()} loop above
...
not draw calls, and a scene with a thousand sprites issues on the order of a thousand backend
...
\cnaclass{Texture2D} (its last owning \texttt{shared\_ptr} released, backend pointer now
\texttt{nullptr}) would dereference a null \cnaclass{ITextureBackend} reference through
\texttt{GetBackend()} --- a guaranteed crash, not a graceful failure. The explicit guard is
...
most recent 3D rendering (or each backend's own construction-time default) had last left in
...
\texttt{examples/easygl\_spritefont\_multiglyph\_spacing\_test.cpp} (Task~425) is a real,
