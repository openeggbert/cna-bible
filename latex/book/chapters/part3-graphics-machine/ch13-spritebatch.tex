...
also the one class all 46 public renderer identities in Part~IV must implement --- the class
...
affected every renderer at once, which is unusual --- most of the bugs catalogued in Part~IV
...
rather than something Part~IV's chapters need to revisit renderer by renderer.
...
class in CNA --- every renderer chapter in Part~IV returns to it, because every renderer has
