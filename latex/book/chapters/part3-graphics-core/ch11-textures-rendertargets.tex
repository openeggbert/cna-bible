...
structural deviation from FNA; and render targets --- once the sharpest per-backend divergence
in the whole Graphics namespace, before a wave of later fixes closed most of it, covered in
full below --- still carry two genuinely open gaps of their own. All three are covered here
together, with the specific per-backend facts intact rather than smoothed into ``mostly
works.''
...
Two of this chapter's own already-stated facts constrain what this loop can safely do next.
...
scopes its own \texttt{GetData} verification to that one backend. And because
\cnaclass{TextureCube} does not inherit \cnaclass{Texture} (the structural deviation noted at
the top of this chapter), \texttt{skybox} cannot be bound to a custom \cnaclass{ShaderEffect}'s
texture slot --- only \cnaclass{EnvironmentMapEffect}'s dedicated \texttt{EnvironmentMap} field
can actually sample it, via that effect's bypass mechanism, not the generic binding path every
other texture in this chapter goes through.
