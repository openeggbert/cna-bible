...
second gap remains open on all three hardware backends alike:
\texttt{GraphicsDevice.Clear(ClearOptions::Stencil, ...)} is confirmed to not actually clear
the stencil buffer on EasyGL, Vulkan, or BGFX --- the same gap already flagged in
...
on changing it independently afterward, which sidesteps the still-open gap flagged above (no
real \texttt{SetReferenceStencil} override exists on EasyGL specifically, BGFX's own fix now
notwithstanding; a technique needing a per-draw-call reference value without rebuilding the
whole state object would hit that remaining gap directly on EasyGL). And because a
stencil-only \texttt{Clear} call does not actually clear the stencil buffer on any of the three
hardware backends today, a game using this exact two-pass technique across multiple frames must
not rely on \texttt{Clear} to reset the
