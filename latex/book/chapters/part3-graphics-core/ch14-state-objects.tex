...
Two of this chapter's own already-stated facts directly constrain whether this exact technique
is safe to ship across all three hardware backends. \texttt{ReferenceStencil} above is set once,
as part of constructing each \cnaclass{DepthStencilState} --- this worked example does not rely
on changing it independently afterward, which is exactly the still-open gap flagged above (no
\texttt{SetReferenceStencil} entry point exists on EasyGL or BGFX; a technique needing a
per-draw-call reference value without rebuilding the whole state object would hit that gap
directly). And because \texttt{GraphicsDevice.Clear(ClearOptions::Stencil, ...)} does not
actually clear the stencil buffer on any of the three hardware backends today, a game using this
exact two-pass technique across multiple frames must not rely on \texttt{Clear} to reset the
mirror mask between frames --- the first pass's unconditional \texttt{CompareFunction::Always} /
\texttt{StencilOperation::Replace} configuration is what actually keeps this specific technique
correct despite that gap, since it overwrites the stencil value outright rather than depending on
it having been zeroed first.
