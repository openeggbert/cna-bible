...
The latter can use several \texttt{NOXNA} construction/mutation hooks, so it must preserve the
...
graphics-device/backend boundary rather than becoming a harmless skipped part. This is why a
...
\texttt{ModelMesh::getEffectsPropertyMutable()} exist only as \texttt{NOXNA} escape hatches.
...
parent wiring, and part-to-effect collection bookkeeping. Backend tests go further: the EasyGL
...
\section{Three model asset routes, not two}
\label{sec:model-loading}

The old two-loader and \texttt{.model.json} framing no longer describes the resolver.
CNA now has two reader implementations but three current \cnaclass{Model} asset routes:
the wire-compatible XNB \texttt{ModelReader}, plus one \texttt{ModelTypeReader} that reads
self-contained \texttt{.cnj} descriptions and direct \texttt{.gltf}/\texttt{.glb} files. For an
...
full resolver contract is in Chapter~\ref{ch:content-and-assets}; the three resulting runtime
...
\begin{description}
  ...
  \item[The self-contained \texttt{.cnj} \texttt{ModelTypeReader} route]
        ...
        not in this reader's extension list. It creates one root whose name can come from the
        first JSON \texttt{bones} record, then one synthetic child bone per mesh, not the
        descriptor's general bone hierarchy. Each mesh receives that child as
        \texttt{ParentBone}, so independently poseable rigid parts work, but the original
        source hierarchy is not reconstructed. An optional \texttt{skeleton}/\texttt{animations}
        sidecar pair builds \cnaclass{SkinningData} on \cnaclass{Model.Tag}, and morph data lives
        on the corresponding part's \texttt{Tag}.
  ...
  \item[The direct \texttt{.gltf}/\texttt{.glb} \texttt{ModelTypeReader} route]
        ...
        Like the CNJ route, direct glTF creates a synthetic root plus one child bone per imported
        primitive rather than exposing the scene or skin hierarchy as \cnaclass{ModelBone}s.
        A real skin is retained as \cnaclass{SkinningData} on \cnaclass{Model.Tag}; morph
        targets and their tracks are retained on each \cnaclass{ModelMeshPart.Tag}. Extracted
        images are decoded in memory and cached by glTF image pointer during that one import, but
        mesh buffers and effects remain per primitive.
\end{description}
...
enabled. Those tests do not make the three routes interchangeable: use XNB for compiled XNA
asset fidelity, CNJ for a controlled self-contained/imported description, and direct glTF/GLB
for the first supported group of a glTF~2.0 file when the resolver priority selects it.
...
constructors are \texttt{NOXNA} extensions precisely because FNA creates these graph objects
...
constructor: CNJ and direct glTF/GLB both place their synthetic root at index~0, then append
synthetic mesh/primitive child bones. CNJ does not turn its general \texttt{"bones"} array into
that \cnaclass{ModelBone} hierarchy, and direct glTF stores its actual skeleton separately in
\cnaclass{SkinningData}. Consequently neither route currently has a serialized arbitrary root
bone for \texttt{rootBoneIndex} to represent. The gap is closed for a hand-built model, but
remains deliberately inapplicable to the two loose content routes; the reader-specific
distinctions are tracked in \S\ref{sec:model-loading}, not hidden by the runtime fix.
...
Its storage is populated by \texttt{Model} or a bone's \texttt{NOXNA AddChild};
...
\cnaclass{ModelMeshPartCollection} has only checked-by-convention integer access,
\texttt{Count}, and iterators --- there is no part name, \texttt{Contains}, or
\texttt{TryGetValue}. Its current integer operator uses raw \texttt{vector[]}, not
\texttt{at}; a negative or too-large index is undefined behaviour in CNA where FNA's
\texttt{ReadOnlyCollection} would throw. Count-check a hand-derived part index first.
...
integer operator also uses raw \texttt{vector[]} and has the same caller-side bounds
requirement. Its mutable operations are the \texttt{NOXNA} escape hatches described in
\S\ref{sec:model-draw-contract}, not a fourth normal collection-builder API.

All name searches are linear and return the first matching pointer; neither loader nor
collection constructor rejects duplicate names. The focused \texttt{ModelBoneCollectionTests.cpp}
and \texttt{ModelMeshCollectionTests.cpp} cover normal name/index/iterator behaviour and missing
names, but no located collection test covers the empty-name FNA difference or the unchecked
part/effect indices. The safe cross-collection rule is therefore simple: keep the model's
ownership and topology coherent at construction time, use name lookup only where that collection
actually exposes it, and bounds-check calculated integer indices before asking a collection for
an element.
