...
constructor always throws, so there is no parser or general parameter-to-backend binding behind
...
alternative to a stock effect's named property and does not create an arbitrary backend uniform.
...
surface, with Chapter~\ref{ch:shader-fx-gap}'s backend limits, for a genuinely custom uniform.
...
documented in the struct itself as ``ignored by every backend except D3D9'' --- but that is no
...
richest cross-backend fix this chapter has to show.
...
\texttt{DirectionalLight2} on \cnaclass{EnvironmentMapEffect} (all 3 backends)'' --- is still
...
\texttt{preferPerPixelLighting} fix above), and every backend's own \texttt{.cpp} references
those fields at the shader-uniform call site --- \texttt{EasyGLGraphicsBackend.cpp}'s own code
...
Chapter~\ref{ch:direct3d-backends}'s pixel-authentic \texttt{D3D9} backend. \texttt{D3D9}'s own
...
any backend at all --- and every backend's only lit shader computed lighting in the fragment
stage unconditionally. The consequence, stated plainly: \textbf{CNA rendered per-pixel lighting
always, on all nine backends, the exact opposite of XNA's own default} (per-vertex, Gouraud-%
...
A change touching a shared struct every backend reads, and requiring a genuinely new shader
...
fix across every backend; or leaving the divergence as a permanent, documented limitation), the
project owner chose the full fix, sequenced one backend at a time rather than attempted all at
...
effect, so this backend's own work was purely dispatch: stop hardcoding the flags to
\texttt{false}. Seven of the other eight backends needed a genuinely new per-vertex-lit shader
...
\texttt{WebGPU} backend, the new test's measured center pixel values (187/203) did not match the
other backends' own linear values (127/152) at all until the swapchain's own known
...
That last distinction matters on its own: once a backend's dispatch honors the real default,
every existing lit-scene pixel test on that backend was expected, up front, to very likely start
...
One backend was deliberately, honestly left out rather than silently deferred:
...
existing GPU shader stage the way every other backend's own fix was --- the project owner chose
...
nine rendering backends (\texttt{D3D9}, \texttt{EasyGL}, \texttt{Vulkan}, \texttt{BGFX},
...
backend (\texttt{Software}) --- not the blanket ``ignored by every backend except D3D9'' this
