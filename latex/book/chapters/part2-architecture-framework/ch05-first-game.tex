...
This chapter develops a compact CNA program from the XNA-style skeleton in CNA's README. It is
a complete translation unit, but the surrounding CMake target and named assets remain project
inputs. Later chapters supply the detailed framework contracts.
...
\textbf{The includes are the public surface.} Each header follows the XNA namespace path; no CNA-specific include is
...
shape of XNA \texttt{Program.cs}'s \texttt{Main} method, preserved so that porting an
...
The first program leaves \texttt{Update} empty. The next fragment extends the same skeleton with
arrow-key movement at a frame-rate-independent speed. It shows only the changed members and
additional includes:
...
\texttt{gameTime.get\allowbreak{}ElapsedGameTimeProperty().get\allowbreak{}TotalSecondsProperty()} --- the declared
...
with a four times higher refresh rate, which is the class of bug frame-rate-independent
...
\texttt{CNAEXT}: a CNA-only convenience overload, not part of XNA~4.0
(Chapter~\ref{ch:design-philosophy} covers what that tag means). This second program instead
calls \texttt{Draw(*logo\_, position\_, Color::White)}, the unmarked XNA~4.0 overload
that takes a \cnaclass{Vector2} position and an explicit tint color --- worth calling out
directly here, since a reader who wants code that ports cleanly to XNA/FNA should reach
...
The second program does not constrain the sprite to the viewport. This fragment clamps it to the
backbuffer bounds and plays a sound when a boundary is reached:
...
\section{A fourth program: rectangle collision}
...
overlaps it, the coin is collected and a counter increments. One more addition
...
\begin{sourcenote}
\textbf{Touching edges do not intersect.} \texttt{RectangleTests.cpp} checks two
$100\times100$ rectangles
placed at $(0,0)$ and $(100,0)$ --- sharing an edge exactly, with zero pixels of true overlap
--- return \texttt{false} from \texttt{Intersects()}, as do separated rectangles. Only
non-zero-area overlap counts. A game that places a
