...
derived constructor can technically already see CNA's default backend, but it precedes the
...
incorrect claim that no backend exists yet. This ordering constraint --- and CNA's
...
regardless of which of the twelve backends from Part~IV is actually doing the presenting.
...
\textbf{\cnaclass{SoundEffect}'s file-path constructor is a \texttt{NOXNA} convenience, the
