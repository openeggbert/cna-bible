...
\index{dependency acquisition}\index{developer tools}
...
CNA's alpha.1 release verification additionally records sharp-runtime
\texttt{625476d5b5fff5fa89f392c3c9af8638\allowbreak ff237692}. That is release evidence, not a CMake-enforced
dependency lock: \texttt{CNA\_SHARP\_RUNTIME\_ROOT} still accepts the caller's checkout and CNA
does not compare its Git SHA. This Bible retains its deeper sharp-runtime technical pin
\texttt{f827a6c5} for chapters auditing that sibling; Appendix~\ref{ch:repo-map} keeps the two
roles explicit rather than arbitrarily repinning the sibling book scope.
...
\section{Build-tree C++ consumption and the native C package}

The C++ framework remains primarily a build-tree/subdirectory product: a parent project adds the
checkout or follows a sibling template, and the module targets compose into its final consumer.
Alpha.1's optional native C API source defines a deliberately narrower install surface. Its CMake
rules would install the shared \texttt{CNA::CApi} target, C headers, \texttt{CNAConfig.cmake}, and
a package-version file; suitable Unix builds also define \texttt{CNA::CApiStatic}. The external
\texttt{hello\_cna.c} gate is designed to configure through \texttt{find\_package(CNA CONFIG)},
link the installed target, and run it. These are source-proven packaging routes, not a successful
alpha.1 artifact: the exact tag cannot compile \texttt{cna\_c\_api} because its C renderer table
omits NanoVG. Appendix~\ref{ch:native-c-api} records the reproduced failure.

The intended package's version is the native C ABI version, not the CNA product SemVer. It does not turn
all C++ module archives into a general installed C++ SDK, nor does it erase the sibling-resolution
contract of source-tree builds.
