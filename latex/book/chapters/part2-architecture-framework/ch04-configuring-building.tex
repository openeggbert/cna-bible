...
\begin{sourcenote}
\textbf{Two same-shaped names, two unrelated things.} \texttt{CNA\_CNAEXT} (a CMake
\texttt{option()}, off by default) and \texttt{CNAEXT} (the marker macro from
\S\ref{sec:cnaext}) share a name fragment purely by coincidence of naming convention, not by
...
\texttt{\#ifdef CNA\_CNAEXT} in its own header) covering HDR exposure/gamma, tonemapping mode,
shadow quality, and PBR material settings. Its own header comment is explicit about scope:
it ``does not perform any rendering itself'' --- it is configuration a renderer can read and
act on, with renderer support for each individual feature not yet implemented across the board
...
four established graphics renderers. Everything this book states about Graphics-layer
