...
Chapter~\ref{ch:easygl-backend} described CNA's own EasyGL implementation
...
Easy-gl's suite has four hand-rolled, mock-loader binaries:
\texttt{easy-gl-smoke-tests}, \texttt{easy-gl-resource-smoke-tests},
\texttt{easy-gl-context-lifecycle-tests}, and \texttt{easy-gl-webgl-tests}. The mechanism
making them useful without any real GPU or display is worth describing directly: a fake
\texttt{GLGetProcAddressFn} loader
...
unavailable entry points. The desktop-shaped initialization and resource smoke tests are
skipped under Emscripten because their invented version strings cannot occur in a real WebGL
host. This is an honest test-model boundary, not proof that the corresponding browser behavior
was executed.

Nested build defaults are asymmetric. Easy-gl defaults both tests and examples ON, and CNA does
not override those options, so each of CNA's five GL-profile builds also compiles the four test
binaries. Its SDL hello-triangle example may configure too because CNA already supplies SDL.
Meta-gl defaults all tests, GPU tests, examples, documentation, sanitizers, and debug options
OFF when nested. CNA therefore inherits extra work from the upper library but not the lower
one. This is a configuration fact; the read-only source audit did not build the siblings to
measure its wall-clock cost.
