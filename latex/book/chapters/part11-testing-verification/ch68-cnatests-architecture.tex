...
\section{One configure, one renderer}

Renderer selection enters only the chosen renderer family.  Metal and Glide contribute
portable policy suites unconditionally, but their native implementation is still gated.  Thus
no build registers all renderer tests, and a result whose renderer identity is omitted cannot
be interpreted safely.

There are three layers of renderer gating:

\begin{enumerate}
  \item source filtering decides which renderer GoogleTests enter \texttt{CnaTests};
