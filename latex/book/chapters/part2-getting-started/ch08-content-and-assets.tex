...
ContentManager(IServiceProvider*)}, \texttt{ContentManager(IServiceProvider*, const
std::string\& rootDirectory)}, and a \texttt{NOXNA}-tagged no-argument
...
\texttt{RegisterTypeReader<T>(std::unique\_ptr<LooseFileContentTypeReader<T>>)} for a whole new
