...
\texttt{EasyGLRenderer.cpp} directly confirms why: there is no \texttt{glColorMask} call
anywhere in the file at all, so \cnaclass{ColorWriteChannels} is accepted by the public API and
silently never reaches the GPU on this renderer --- a real, currently open gap, not something
...
The fix landed in \texttt{cna} itself --- confirmed by a direct source read of
\texttt{ContentManager.cpp}'s own
\texttt{BuildVertexBuffer\allowbreak{}FromRawBytes()} function, whose code comment
explicitly credits where the bug was actually found:
...
themselves are not yet implemented, and only the \texttt{EasyGL} graphics renderer is
...
the input/audio/devices/networking surface of Part~VII) is a genuinely useful thing to watch for
