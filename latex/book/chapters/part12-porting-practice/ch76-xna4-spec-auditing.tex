...
Per-namespace type counts show where the real API surface concentrates.
\texttt{Microsoft.\allowbreak{}Xna.\allowbreak{}Framework.\allowbreak{}Graphics} alone accounts
for 175 of the 544 types.
\cnans{GamerServices} (53) and \cnans{Content.Pipeline.Graphics} (46) are the next largest ---
...
--- present and correctly shaped, present but tagged \texttt{CNAEXT} (and therefore not really
part of what you are checking), or missing. A member present in CNA but absent from
\texttt{xna4-spec} and not tagged \texttt{CNAEXT} at all is the specific ``Extra-unmarked''
...
retagged) --- exactly the class of bug the \texttt{CNAEXT} compile-time gate from
...
(\texttt{modules/math/include/Microsoft/Xna/Framework/Ray.hpp}) finds a complete, correctly-shaped match for
...
outcome distinct from both ``missing'' and ``present but \texttt{CNAEXT}-tagged'': a member that
...
this book's own audits do not require a \texttt{CNAEXT} tag for it.
...
\cnaclass{Viewport} lives in \cnans{Microsoft.Xna.Framework.Graphics}. \texttt{xna4-spec}'s
own \texttt{Microsoft.\allowbreak{}Xna.\allowbreak{}Framework.\allowbreak{}Graphics/\allowbreak{}Viewport.xml} lists: two constructors
...
(\texttt{modules/graphics/include/\allowbreak{}Microsoft/\allowbreak{}Xna/\allowbreak{}Framework/\allowbreak{}Graphics/\allowbreak{}Viewport.hpp})
...
not requiring a \texttt{CNAEXT} tag. \texttt{Height}, \texttt{MaxDepth}, \texttt{MinDepth},
...
\texttt{CNAEXT} tag needed because nothing about it is an addition. It is the first concrete
