...
The first tier is universal and effectively free. Every single GL object class inherits
\texttt{easygl::detail::GenerationTracked}, which stamps each resource with the
\texttt{meta-gl}-tracked context generation number at creation time
...
(\texttt{test\_resource\_registry\_context\_lost}, \texttt{test\_resource\_registry\_wired\_to\_meta\_gl})
both construct a minimal, test-only \texttt{FakeResource} stub rather than exercising any real
resource type, for exactly this reason:
