...
\item[CNA] The project this book is about: a native XNA~4.0 reimplementation compiled with a
...
\item[ContentManager] The central XNA class resolving an asset name to loaded content. A same-
...
\item[ENet] The third-party UDP networking library backing CNA's real Networking
...
\item[\texttt{EXT} suffix] A visible naming convention used by many non-XNA methods and types,
...
\item[FNA] An open-source C\# reimplementation of the XNA~4.0 API, maintained
...
\item[Golden-image test] A captured frame compared with a retained reference image under an
...
\item[.cnj] CNA's own JSON-based content envelope. For a registered loose reader it is considered
...
\item[.xnb] Real XNA's compiled binary content-pipeline output format. CNA has a substantial,
...
\item[MojoShader] A translator used by the FNA/FNA3D stack for compiled Direct3D~9 effect
...
\item[CNAEXT] An empty declaration marker for non-XNA surface inside the XNA namespace tree. In
...
\item[PbrEffect] A \texttt{CNAEXT} glTF~2.0-style metallic-roughness material effect with no
...
\item[xna4-spec] A sibling repository containing 544 indexed, machine-readable XML type files
