...
\item[ASCII] One of CNA's twelve graphics backends: a text-glyph renderer layered on top of
  ...
  with a luminance-to-glyph quantization step. See Chapter~\ref{ch:canvas-ascii-backends}.
...
\item[DX3] CNA's graphics backend fronting the sibling \texttt{free-direct} project; a 2D-only
  backend, in the same spirit as \texttt{SDL\_RENDERER}. See Chapter~\ref{ch:dx3-freedirect-backend}.
\item[EASYGL] CNA's OpenGL/OpenGLES graphics backend, built on the sibling \texttt{easy-gl}
  project; CNA's most feature-complete backend by its own account. See Chapter~\ref{ch:easygl-backend}.
...
\item[\texttt{EXT} suffix] CNA's naming convention for a method or property bolted onto an
  ...
  where \texttt{NOXNA} marks a whole declaration with no XNA counterpart. See Chapter~\ref{ch:game-loop}.
...
\item[HEADLESS] A CNA graphics backend that never rasterizes a real pixel at all --- it only
  ...
  to produce a real screenshot for that reason. See Chapter~\ref{ch:graphicsdevice}.
...
\item[PbrEffect] A \texttt{NOXNA} glTF~2.0-style metallic-roughness PBR material effect ---
  ...
  predates the glTF PBR model. Available on the WebGPU backend. See Chapter~\ref{ch:webgpu-backend}.
