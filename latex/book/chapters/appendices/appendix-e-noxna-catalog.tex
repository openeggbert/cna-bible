...
Chapter~3's design-philosophy discussion introduced \texttt{NOXNA} as CNA's marker
...
project's own \texttt{NOXNA} build gate (Chapter~3) already checks it mechanically, which a
...
\item[Custom shaders] The entire \texttt{ShaderEffect} runtime-HLSL-compilation path on the
  three Direct3D backends --- Ch.15, Ch.23.
\item[\cnaclass{PbrEffect}] An entire NOXNA glTF~2.0-style metallic-roughness PBR material
  ...
  \texttt{MetallicFactor} / \texttt{RoughnessFactor} / \texttt{EmissiveFactor}) --- Ch.21.
\item[\cnaclass{SkinnedPbrEffect}] PBR's separately-declared GPU-skinned sibling, rather
  ...
  stride-68 \texttt{VertexPositionNormalTangentTextureSkinned} layout (position, normal,
  tangent, UV, four weights and four byte indices). As with \cnaclass{SkinnedEffect}, game
  ...
  whose current PBR path remains unskinned --- Ch.12, Ch.21.
...
\item[\cnaclass{Texture}] Two static format-utility methods with no real-XNA counterpart,
  ...
  alignment and to validate \texttt{SetData} / \texttt{GetData} byte counts --- Ch.11.
\item[\cnaclass{TextureCube}] \texttt{DDSFromStreamEXT(GraphicsDevice\&, Stream\&)}, a static
  ...
  pipeline never offered a runtime DDS loader for cube maps at all --- Ch.11.
\item[\cnaclass{Texture3D}] \texttt{SetDataPointerEXT()}, a raw-pointer overload of the
  volume-texture upload path, alongside the already-documented finding (Ch.8) that this same
  class's compressed-volume \texttt{byteCount}-vs-dimension validation was once dormant --- Ch.11.
...
\item[\cnaclass{GraphicsDevice}] \texttt{SetGraphicsProfileEXT(GraphicsProfile)}, an internal-use
  ...
  general runtime profile switch, since real XNA has none either --- Ch.9.
\item[\cnaclass{PresentationParameters}] \texttt{getHeadlessEXTProperty()} /
  ...
  in this repository) and Ch.42's verification methodology both depend on --- Ch.9.
\end{description}

\section{Audio (Chapter~27)}
...
\section{Devices and Sensors (Chapter~29)}
...
\section{Content and assets (Chapter~8)}
...
Chapter~32. \cnaclass{ContentManager} itself adds three further \texttt{NOXNA} methods purely
...
\section{GamerServices (Chapter~30)}
...
the field \texttt{RankingEXT} for the same reason as Ch.28's video-track members above) and
