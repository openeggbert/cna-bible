...
Chapter~\ref{ch:design-philosophy}'s design-philosophy discussion introduced \texttt{CNAEXT} as CNA's marker
...
not responsibly be, an exhaustive line-by-line listing of every \texttt{CNAEXT}-tagged member
in a codebase of several hundred header files; that list changes with every commit, and the
project's own \texttt{CNAEXT} build gate (Chapter~\ref{ch:design-philosophy}) already checks it mechanically, which a
static list in a book cannot. What this appendix collects instead is every \texttt{CNAEXT} (or
...
\begin{description}
\item[\cnaclass{Effect}] \texttt{Apply()} itself is \texttt{CNAEXT} --- real FNA has no public
...
\item[\cnaclass{GraphicsDevice}] \texttt{SetContextRecoveryEnabled()} and
  ...
  optional renderer hook; the latter is native only on Vulkan, throws on D3D9, and is silent
  elsewhere --- Ch.\ref{ch:graphicsdevice}, Ch.\ref{ch:backend-architecture}.
\item[\cnaclass{IGraphicsRenderer}]
...
\item[Custom shaders] The entire \texttt{ShaderEffect} runtime-HLSL-compilation path on the
  three Direct3D renderers --- Ch.\ref{ch:shader-fx-gap}, Ch.\ref{ch:direct3d-backends}.
\item[\cnaclass{PbrEffect}] An entire CNAEXT glTF~2.0-style metallic-roughness PBR material
...
\item[\cnaclass{SkinnedPbrEffect}] PBR's separately-declared GPU-skinned sibling, rather
  ...
  every frame; the loader does not do that binding. This is entirely \texttt{CNAEXT}: real XNA
  ...
  Every renderer with a real \cnaclass{PbrEffect} shader implements this sibling except WebGPU,
...
\item[\cnaclass{AnimationPlayer}] The entire class, mirroring Microsoft's own unshipped XNA
  ...
  likewise \texttt{CNAEXT} sample-derived types: the content reader attaches a
...
\item[\cnaclass{PresentationParameters}] \texttt{getHeadlessEXTProperty()} /
  \texttt{setHeadlessEXTProperty()} --- an opt-in, no-window device-creation mode for renderers
  ...
  no-window operating mode the \texttt{Headless} and \texttt{Software} renderers already use
  unconditionally, offered here as an opt-in for a renderer that normally does want one --- the
...
other direction --- a \texttt{CNAEXT} safety guard protecting a code path real XNA never had to
guard against.
...
\texttt{CNAEXT} marker and \texttt{EXT} suffix convention is applied faithfully to code ported
...
The largest concentration of \texttt{CNAEXT} / \texttt{EXT} surface in the whole project. Whole
\texttt{CNAEXT} subsystems: \cnaclass{CNA::Input::Clipboard}, \cnaclass{Joysticks},
...
The entire \cnans{CNA::Devices} namespace is \texttt{CNAEXT} in spirit --- \cnaclass{Camera},
...
The \texttt{.cnj} content format itself is a \texttt{CNAEXT} system in its entirety, alongside
...
\texttt{CNAEXT} vertex/track/clip types (\cnaclass{VertexPositionNormalTextureSkinned},
\cnaclass{KeyframeEXT}, \cnaclass{BoneTrackEXT}, \cnaclass{AnimationClipEXT}) --- see
Chapter~\ref{ch:avatar}. \cnaclass{ContentManager} itself adds three further \texttt{CNAEXT} methods purely
...
The entire \cnaclass{SkinnedModelEXT} real-rendering extension is \texttt{CNAEXT} in its
...
silently exposing the setter. Marking it \texttt{CNAEXT} and making it public is what lets this
...
\texttt{CNAEXT} compile-time gate (build with the \texttt{CNAEXT} CMake option enabled) is the
