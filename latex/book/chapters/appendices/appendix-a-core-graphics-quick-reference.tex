...
that every one of the 42 implementation families renders it with the same semantics.
...
the CNA-added convenience that applies the current pass.  \cnaclass{EffectParameter} exposes
typed scalar/vector/matrix/string/texture setters and getters plus class/type/dimension metadata
and nested collections.  CNA's public arbitrary compiled-effect constructor remains a refusal;
\cnaclass{ShaderEffect} is a distinct renderer-specific source/program route.  The existence of a
stock effect or parameter setter therefore does not prove arbitrary XNA \texttt{.fxb} execution.
...
\texttt{7a64362efef4119bf880459ef1704fb2c52199e2}.  Then pair the header with its implementation,
