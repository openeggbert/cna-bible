...
\input{figures/fig-skin-index-spaces.tex}
...
Only channels whose target resolves through the skin's joint map enter a skeletal clip. Rigid
node TRS animation is retained separately as scene-node-indexed clips. On an unskinned model,
\texttt{Model.Tag} carries \cnaclass{ModelAnimationsEXT}; its tracks address the same flattened
scene-node indices used by \cnaclass{ModelBone}. A mixed skinned model cannot put that second
carrier into the already occupied legacy Tag slot, so rigid tracks outside the retained skin
palettes are explicitly counted and reported as dropped. D6 is therefore fixed for the ordinary
rigid-model route without pretending that one legacy pointer can hold two unrelated carriers.
...
The draw scratch vector is \texttt{thread\_local} at alpha.1, removing the earlier cross-thread
scratch race. Graphics resources and the shared model graph are not thereby made generally
thread-safe; keep sampling, palette installation, mutable model updates, and drawing under the
application's graphics-thread ownership rule.
