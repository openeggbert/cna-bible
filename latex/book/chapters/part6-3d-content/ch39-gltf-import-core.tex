...
URIs, and external percent-decoded file URIs are all supported. Images use the same three storage
forms and are exposed as memory blocks; the runtime front end decodes those blocks without staging
temporary files.

External URI support is not a containment guarantee. The default cgltf buffer loader combines the
document directory with the decoded URI and opens that path; CNA's external-image branch likewise
joins \texttt{gltfDir} and the decoded image URI. Neither canonicalizes the result and proves it
remains below the document directory. An absolute URI or enough \texttt{..} components can
therefore reach a readable file outside the asset tree. Do not load an untrusted glTF/GLB as
though this path were sandboxed; stage and validate its complete dependency set first.
