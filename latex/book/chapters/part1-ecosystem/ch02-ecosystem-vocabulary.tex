...
\index{CNA ecosystem}\index{sibling repositories}
...
looks for them there. This chapter is the map; later chapters (Part~\ref{part:sharp-runtime} for
\texttt{sharp-runtime}, Part~\ref{part:sibling-libraries} for \texttt{easy-gl} and \texttt{free-direct}) are the
territory.

\input{figures/fig-ecosystem.tex}
...
\section{The sibling build graph}
...
designed for use in native C\texttt{++} applications,'' tracked class-by-class in a SQLite
database (\texttt{plan.sqlite3}) whose exact blob exists in the pinned commit despite also
matching a current ignore rule, with an honest, small state machine
...
\section{free-direct: DirectX 3 scoped to two legacy games}
...
what is implemented vs.\ what is missing).'' CNA's release prose repeats a dated
227-of-245 public-FNA-type presence audit (92.7\%). That denominator came from diffing FNA's
public type list against CNA headers, not from \texttt{xna4-spec}, and alpha.1 does not ship a
generator that re-derives it as a tag invariant. Treat it as a historically recorded,
methodology-limited presence snapshot---not behavioral coverage and not a fresh count made by
this edition.
