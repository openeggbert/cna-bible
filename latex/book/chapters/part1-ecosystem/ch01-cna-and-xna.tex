...
to 2013. CNA itself is built on SDL3, with rendering delegated to one of
\RendererIdentityCount{} public identities chosen at build time and mapped to
...
framework modules, \RendererIdentityCount{} public renderer identities over
...
\begin{itemize}
  ...
  \item \textbf{Not universally compiled-shader compatible.} CNA has four distinct answers to
        ...
        No one arbitrary bytecode route works identically on all \RendererFamilyCount{} families;
...
is closed only once its own regression suite is re-run clean. One recorded BGFX phase moved the
renderer from a narrower feature set to a broad 2D/3D pixel corpus; later work continued beyond
that historical closure. Chapter~\ref{ch:project-practice} covers the Task/Phase discipline as a working
