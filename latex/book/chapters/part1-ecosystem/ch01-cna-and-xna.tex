...
built on top of SDL3, with the actual rendering delegated to one of 46 public renderer identities
chosen at build time and mapped to 42 implementation factories.
...
\section{The scale of verification, in one table}

The individual per-renderer chapters in Part~IV state the evidence available to their executed
audit cohort. Seeing that pinned evidence side by side is itself informative --- both about
where the project's verification effort was heaviest, and about how unevenly ``done'' was
distributed across renderers that all present the same public API. This is an evidence snapshot,
not the current 46-identity registry; its ASCII row is historical:

\begin{center}
\small
\begin{tabularx}{\textwidth}{lXX}
\toprule
\textbf{Renderer} & \textbf{Renderer-specific tests} & \textbf{Verification method} \\
\midrule
EasyGL & full suite exceeds 4{,}500 & real GPU readback (Mesa/llvmpipe) \\
Vulkan & 126 of 127 (4{,}371 of 4{,}373 overall) & real GPU readback \\
BGFX & 103 of 105 (4{,}375 of 4{,}377 overall) & real GPU readback \\
D3D9 & 14 CTest binaries, 39-scene oracle & real GPU via Wine + DXVK \\
D3D11 & 6 CTest binaries, 96+ checks & real GPU via Wine + DXVK \\
D3D12 & 1 CTest binary, 80/80 checks & real GPU via Wine + vkd3d-proton, off-screen only \\
SDL\_RENDERER & 67 of 67 integration tests & real GPU readback \\
ASCII (removed) & fully automated, no BLOCKED items & real GPU readback (inherited SDL\_RENDERER) \\
CANVAS & structural/compile-verified only & no real browser in this project's own dev loop \\
WebGPU & native CTest, gracefully skips headless & real GPU readback where a display exists \\
FREEDIRECT & no BLOCKED decisions remain open & CPU compositor, no GPU involved at all \\
\bottomrule
\end{tabularx}
\end{center}

Two things worth noticing in that table rather than skimming past it. First, the total test
count climbs steadily release over release, but it is not one number: EasyGL's own count grew
large enough, early enough, that it stopped being tracked as a precise fraction the way the
newer renderers still are --- ``exceeds 4{,}500'' is itself evidence of how much of this
project's total verification effort sits on the single most mature renderer. Second, the
verification \emph{method} column is not uniform either: a real GPU readback comparison
(pixel-exact or oracle-based) is a categorically stronger claim than ``compiles and structural
review only,'' which is exactly why Chapter~\ref{ch:canvas-ascii-backends} is explicit that
\texttt{CANVAS}'s own checkmarks mean something narrower than every other row in this table.
...
\begin{itemize}
  ...
  \item \textbf{Not wire-compatible with Microsoft DirectPlay/DirectDraw/DirectSound.} The
        ...
        Part~IX) that powers the \texttt{FREEDIRECT} renderer is explicit that it targets
...
\begin{description}
  ...
  \item[Pixel-level oracle corpora, not just ``it compiles.''] The \texttt{D3D9} renderer in
        ...
        \emph{the real XNA 4.0 runtime's own render} of the same scene, at zero tolerance ---
        31 scenes as of an earlier pass through this book, 39 real scene files on disk as of
        this book's own most recent direct check, with zero known divergence throughout.
...
counts (``4{,}370 unit tests,'' ``0 of 39 oracle scenes diverge'') rather than vaguer language ---
