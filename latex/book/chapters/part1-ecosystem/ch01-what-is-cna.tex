...
them incrementally''; ``decouple gameplay-facing API from rendering backend implementation
...
these six are really one idea stated four ways --- the backend-abstraction goal --- and
Chapter~\ref{ch:backend-architecture} is where that idea becomes a concrete
\cnaclass{IGraphicsBackend} contract rather than a design principle; the first two are the
``why native C\texttt{++}, why not just use FNA'' argument already made above.

\section{The scale of verification, in one table}

The individual per-backend chapters in Part~IV each state their own current test count, but
seeing them side by side is itself informative --- both about where the project's
verification effort is heaviest, and about how unevenly ``done'' is distributed across
backends that all present the same public API:

\begin{center}
\small
\begin{tabularx}{\textwidth}{lXX}
\toprule
\textbf{Backend} & \textbf{Backend-specific tests} & \textbf{Verification method} \\
\midrule
...
ASCII & fully automated, no BLOCKED items & real GPU readback (inherits SDL\_RENDERER) \\
...
DX3 & no BLOCKED decisions remain open & CPU compositor, no GPU involved at all \\
\bottomrule
\end{tabularx}
\end{center}
...
newer backends still are --- ``exceeds 4{,}500'' is itself evidence of how much of this
project's total verification effort sits on the single most mature backend. Second, the
...
\begin{description}
  ...
  \item[A compile-time purity check.] A dedicated build define, \texttt{CNA\_STRICT\_XNA\_API},
        turns the otherwise-inert \texttt{NOXNA} marker macro into a real
        ...
        \texttt{NOXNA} marker macro (Chapter~\ref{ch:design-philosophy} explains the
