...
\texttt{CMakeLists.txt} and \texttt{cmake/BackendSelection.cmake} do at configure time, checked
...
SHARP\_RUNTIME)} runs regardless of backend choice); \texttt{easy-gl} and \texttt{free-direct}
are each pulled in only when the matching \texttt{-DCNA\_GRAPHICS\_BACKEND} value is selected,
...
\texttt{free-api}'s own README. \texttt{BackendSelection.cmake}'s own comment explains why this
...
CNA's own already-vendored targets, set up before backend selection runs, so the whole
...
infrastructure, not machinery built only for CNA's own \texttt{DX3} backend.
\end{sourcenote}
...
of five directly against \texttt{include/easygl/\{Framebuffer,Program,ProgramPipeline,
TransformFeedback,Sync\}.hpp} finds \texttt{attach\_texture\_2d} already taking
