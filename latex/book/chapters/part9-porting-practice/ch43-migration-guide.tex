...
The six steps above assume the port compiles and the graphics backend actually starts.
...
\texttt{Could not find a package configuration file for Vulkan} (\texttt{VULKAN} backend) &
...
CMake \texttt{FetchContent} hangs or fails cloning \texttt{bgfx.cmake} (\texttt{BGFX} backend)
...
\cnaclass{VideoPlayer}-related link errors & Missing FFmpeg dev packages --- hits every
  backend, not just video code. Install \texttt{libavcodec-dev libavformat-dev
...
Any 3D call throws \texttt{std::runtime\_error} & You are on \texttt{SDL\_RENDERER} --- not a
  bug, it is 2D-only by design (Chapter~\ref{ch:sdl-renderer}). Switch to \texttt{EASYGL},
...
Blank/black window, or an OpenGL-based backend fails to create a context in CI/headless
...
Flaky test failures that do not reproduce in isolation & \textbf{Never run multiple
  backends' full \texttt{ctest} suites concurrently} --- confirmed to cause transient
  GPU/driver-contention false failures. Run backend test suites sequentially, and re-run any
...
\texttt{docs/xna-4-api-coverage.md}'s per-backend support table and its ``Known deviations
