...
\subsection{A separate backend, not a toggle --- and why that was a genuinely close call}
...
runtime toggle or \texttt{NOXNA} extension directly on \texttt{SDL\_RENDERER}, instead of a
separate, selectable \texttt{CNA\_GRAPHICS\_BACKEND} value. The architectural case for the
...
forwards to a live \texttt{SdlGraphicsBackend} instance unmodified, and the only genuinely new
code this backend contributes at all is a replacement \texttt{Present()}. A single boolean flag
on the existing backend could, in principle, have delivered the identical behavior with strictly
less code --- no new CMake option, no new entry in \texttt{BackendSelection.cmake}'s
\texttt{STRINGS} list, no fourteenth value for a reader of Chapter~\ref{ch:backend-architecture}
to learn.
...
``discoverability and consistency with how this project already exposes backend choice.'' A
\texttt{CNA\_GRAPHICS\_BACKEND=ASCII} build is a single, obvious CMake flag any reader of this
...
way, and would have made this backend read as an undocumented mode of another backend rather
than a first-class, named part of the project's own twelve-backend catalog. The plan document
...
Neither backend overrides \texttt{SetViewport()}, so a custom public viewport never remaps its
...
Both products ignore the backend preserve Boolean yet keep browser-canvas or SDL target
...
\section{Both backends are 2D-only by explicit design, not by omission}

Every backend documented in the previous three chapters implements the full backend interface,
\cnaclass{IGraphicsBackend}, 3D pipeline included, even where a given hardware
...
Clear calls illustrate why ``the backend throws for 3D'' does not automatically describe the
...
off-screen target. The direct combination hooks below still throw when tests call the backend
...
the wrapped \texttt{SdlGraphicsBackend}'s own existing \texttt{ThrowNo3D()} calls, rather than
re-declaring the same throw logic a second time --- a design decision the backend's own plan
...
is complete and correct, by calling each reachable 3D entry point on a real, live backend
...
\begin{itemize}[nosep]
  ...
  \item \texttt{CreateEffectBackend}.
\end{itemize}
...
\texttt{AsciiGraphics}\allowbreak\texttt{Backend} also never overrides the public
...
vertex-buffer backend reference, but \texttt{CreateVertexBuffer} and
\texttt{CreateIndexBuffer16} already throw (Checks~J/K above), so no real vertex or index
buffer can ever exist on this backend to construct that argument with. They are structurally
...
backend with the identical one-line pattern as every other method in the file.

\subsection{CANVAS: the identical contract, without a dedicated test}

\texttt{CanvasGraphicsBackend} follows the exact same shape, documented directly in its own
...
\cnaclass{IGraphicsBackend}'s own shared defaults, as are the five factories for
\texttt{Texture3D}, \texttt{TextureCube}, \texttt{RenderTargetCube},
\texttt{OcclusionQuery}, and \texttt{EffectBackend}. The header comment names its own
...
implementation. Three separate backends, two of them with no code relationship to each other
...
claim consistent with the rest of this backend's own ``structural, not pixel-verified''
...
\texttt{CanvasGraphicsBackend} for \texttt{AsciiGraphicsBackend} --- the two backends' checks
would be close to identical, since both ultimately assert the same
\cnaclass{IGraphicsBackend}-level contract.
