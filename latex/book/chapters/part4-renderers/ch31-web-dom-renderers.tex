...
Three CNA identities render XNA-style sprites in a browser without WebGL.  \texttt{CANVAS}
paints pixels into a Canvas2D context.  \texttt{HTML\_DOM} represents sprites as pooled,
CSS-transformed elements.  \texttt{SVG\_DOM} emits SVG image nodes and filters.  All are
...
\texttt{SVG\_DOM} & SVG \texttt{image} nodes and \texttt{feColorMatrix} & not claimed & yes / no / no \\
\bottomrule
\end{longtable}
...
texture pixels.  The retained tree is inspectable and naturally matches vector composition,
but it inherits SVG's filter, sampling, and browser-layout semantics.

Like HTML DOM, it owns a DOM off-screen target, has no MRT or queries, and its 3D refusal
bypasses \texttt{WarnAndStub}.  Its verification tier is weaker: no renderer test is registered,
the host target is behind an off-by-default \texttt{CNA\_BUILD\_SVG\_DOM\_HOST\_TESTS} option,
and no CI workflow references that option.  A runnable browser script exists, which is a route
to evidence rather than evidence that the route currently gates changes.
...
The three families can display an identical sprite scene while failing in different places:
