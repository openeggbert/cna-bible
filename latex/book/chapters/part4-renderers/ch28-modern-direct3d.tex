...
\section{D3D9: the one renderer that targets pixel authenticity, not just parity}

\texttt{D3D9} is architecturally different in ambition from every other renderer in this book.
...
renderer, diffing the two resulting images at zero tolerance: an exact match, not an
...
\emph{real} here, consulting an actual \texttt{D3DCAPS9} structure --- the only CNA renderer
...
problems earlier in this renderer's history and are now fixed --- the specular fix in
...
\begin{sourcenote}
D3D9's real lost-device lifecycle is also reachable through two framework-reserved keys:
after ordinary input processing, non-repeated F9 enters \texttt{Lost} and raises
\texttt{DeviceLost}; F10 performs the real reset, bracketing it with
...
renderer's own planning document (\texttt{plan\_dx.md}) is worth stating explicitly, because
getting it wrong is exactly the kind of mistake that makes a hand-rolled Direct3D renderer
fragile under two of its most routine events --- a window resize and a lost device. A plain
resize and device-removed recovery touch genuinely different resource sets, and this renderer
...
mention for what it reveals about porting a hand-rolled COM-based renderer under a non-MSVC
toolchain: every \texttt{ID3D11*} / \texttt{IDXGI*} / \texttt{ID3D12*} interface pointer anywhere in
this renderer uses \texttt{Microsoft::WRL::ComPtr<T>}, not a bare \texttt{Release()} call or a
...
\section{DIRECTX10: a deliberate MVP with one dangerous default}
...
extended primitive method, so the base class calls its real colored draw implementation after
...
intent.  This is the sole unconditional inherited colored downgrade among all 42 renderer
families.

That behavior makes capability checks necessary but insufficient.  \texttt{ThreeD} can be true
because colored geometry is real, while effect semantics are still absent.  A porter should
...
\texttt{D3D11} is CNA's first renderer built directly on native Direct3D~11, verified in the
...
presentation geometry on either native Direct3D renderer today. This is a real renderer limitation,
...
\subsection{Viewport/scissor: D3D9 and D3D11 are direct; D3D12 is full-target-only}
...
D3D9's source path is equally real, but its evidence label needed correction. The renderer plan
now calls viewport and scissor ``oracle-proven.'' A direct recount/search of the current
39-scene \texttt{tools/xna-oracle/scenes} corpus finds cull-mode scenes but no viewport or
...
D3D12 is not the analogous modern implementation. It inherits both setters' empty defaults,
and every ordinary, extended, instanced, and SpriteBatch command list constructs a fresh
full-target \texttt{D3D12\_VIEWPORT} and \texttt{D3D12\_RECT} immediately before drawing.
Consequently the public properties round-trip but custom values never reach rasterization.
The renderer's real rasterizer-state pixel suite proves culling/fill/depth-bias-era PSO fields;
it does not turn that specific full-target-only source behavior into viewport/scissor support.
The complete product comparison is \S\ref{sec:backend-viewport-scissor-matrix}.
...
Both have strong
direct-renderer proof: stencil-plane bytes are read back from the GPU, changed to a second
value, and depth is distinguished by whether one identical triangle passes. Those tests bypass
\cnaclass{GraphicsDevice}'s active-format mask. That matters because the shared mask retains
stencil for public \texttt{Depth16}/\texttt{Depth24}; D3D11/12 physically map
\texttt{Depth24} to D24S8, but \texttt{Depth16} has no stencil plane. D3D12 also requires a
bound colour resource at the start of every combination helper, so its headless off-screen
device can throw before a depth-only no-op.
...
updates that field. A target can therefore lose a real depth clear when the backbuffer has no
...
The single most important structural fact about this renderer's own verification is worth
stating plainly, because it qualifies every number above: \textbf{the routine CTest suite for
this renderer is entirely off-screen}. Real swap-chain creation and \texttt{Present()} through
...
The renderer's real smoke test makes that contract observable rather than treating fence
...
state, real public render-target and render-target-cube renderers (including multiple render
...
\section{Persistent Direct3D resources do not make every usage contract complete}

All three Direct3D factories ignore the 2D preserve Boolean, but their color resources persist
across binding. Shared code therefore supplies the visible policy: Discard immediately clears
black plus optional depth, while Preserve and Platform do not clear. The implicit request
never includes stencil. A redundant bind is not a no-op; D3D9/11 can resolve the same target
before rebinding it, D3D12 runs its own unbind/finalization path, and shared code then repeats
the discard clear and rectangle reset.

Cube construction has no usage input on any of the three. Their persistent resources
therefore retain content unless explicitly cleared, subject to the public D3D11/12
cube finalization and unbind failures below. MRT uses the first 2D target's shared usage
decision, as FNA does, but no current Direct3D test compares usage values, Platform, preserved
depth/stencil, or redundant binding. The strong resolve/mip tests prove finalization when their
paths run, not \cnaclass{RenderTargetUsage}; see
...
assume the two renderers are near-identical beneath that layer. A late pair of tasks in the
Direct3D~11/12 implementation plan --- opened well after both renderers were otherwise
...
book's former conclusion was too broad: both test suites invoke the cube renderer's
...
renderer algorithms with a missing public finalization edge, not end-to-end public features.
...
both renderers, not a silent oversight --- the project owner explicitly asked for it to be
...
renderers: an $8\times8$ cube face requested at 4x MSAA, cleared, resolved on unbind, and read
...
renderers' shared foundation runs out. \texttt{D3D11} calls
...
at all, so \texttt{D3D12RenderTargetCubeRenderer::GenerateMipsEXT()} implements the whole chain by
...
every individual feature, and a reader porting code that depends on either renderer's own timing
or cost characteristics for this specific operation should not assume the other renderer behaves
...
method on \cnaclass{IGraphicsRenderer} binds a non-null cube directly and delegates a null cube
to the 2D-null path; it never calls the outgoing cube's unbind callback. Both Direct3D
renderers inherit that default and track only a current 2D target. D3D11's 2D-null branch
...
The smoke checks cited above deliberately bypass this chain. They create a renderer cube, call
...
overload. Until renderer-level cube tracking or explicit overrides close that edge, public
...
\section[\texttt{HeadlessEXT}: letting three D3D12 tests join the routine suite]{\texttt{HeadlessEXT}: the CNAEXT property that let three D3D12 tests join the
routine suite}
...
initialized SDL video and created a real window for every renderer except \texttt{HEADLESS}/%
...
\cnaclass{GraphicsDevice} at all, on a renderer whose only route to one was a launcher too slow for
...
CNAEXT \texttt{HeadlessEXT} property, read directly from the real header:
...
// all -- the same thing the Headless and Software renderers do
// unconditionally, but as an opt-in for a renderer that normally does want a
// window. Only renderers that can genuinely operate without a swap chain
...
Defaulting to \texttt{false} keeps production behavior byte-identical for every existing renderer
and caller; only \texttt{D3D12} honors it today, since its own renderer already treated a null
...
\cnaclass{GraphicsDevice} path against this specific renderer before. \cnaclass{SpriteBatch}
...
this renderer --- meaning any real game calling any of these three ordinary, load-bearing
...
project ever to drive that specific call path against this renderer. All four are now fixed; the
