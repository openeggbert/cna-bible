...
fourteen products and calls out that partial-state outcome.
\end{sourcenote}
...
The three backends do not share one presentation-interval policy. D3D9 preserves CNA's
...
immediately calls the same device-reset path used for a resize. This is the only CNA backend
...
make \texttt{IDirect3DDevice9::Reset()} return failure. The backend logs that result, restores
...
The cross-backend table in \S\ref{sec:backend-swap-interval-matrix} places these results beside
the other eleven products.
...
D3D11 and D3D12 ignore all three backend arguments. Their windowed swapchains are fixed
...
Both \texttt{CreateSwapChainForHwnd} calls omit a fullscreen descriptor, and neither backend
...
D3D11/12 is not DXGI exclusive fullscreen. The fourteen-product comparison and test boundary
...
pixel test because the check invokes backend methods directly.

D3D9's source path is equally real, but its evidence label needed correction. The backend plan
...
The backend's real rasterizer-state pixel suite proves culling/fill/depth-bias-era PSO fields;
...
Both have strong
direct-backend proof: stencil-plane bytes are read back from the GPU, changed to a second
