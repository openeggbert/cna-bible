...
independently from the actually bound 2D/cube attachment rather than inferring stencil from every
...
supplies the visible policy instead. \texttt{DiscardContents} clears black plus every real depth
...
This is now public pixel evidence rather than inference from persistent allocation. The shared
...
\section{A reopened feature whose public routing edge is now closed}
...
An interface-default audit did expose a public routing hole after those mechanisms landed:
...
and the tracking edge which makes them public features rather than isolated smoke mechanisms.
