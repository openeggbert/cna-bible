...
\index{CNA::Internal::Renderers::SdlGpu::SdlGpuRenderer}. Its source lives under
...
the float values in the pipeline-cache key and baking them into each pipeline. The renderer does
...
\texttt{SdlGpuRenderer} has no \texttt{SupportsCapability()} override, so
...
Cube usage never reaches this renderer and shared cube binding queues no implicit clear.
...
this renderer has no \texttt{ReadBackbuffer()} override at all:
\texttt{GraphicsDevice::GetBackBuffer\allowbreak Data()} reaches
\cnaclass{IGraphicsRenderer}'s default \texttt{runtime\_error}; working
\cnaclass{RenderTarget2D::GetData()} tests are a separate resource-readback path and do not
make the public backbuffer method available. The complete inherited fourteen-product distinction is
...
renderer allocates its native \texttt{SDL\_GPUTexture} with exactly one level and inherits
\texttt{ITextureRenderer::UpdatePixelsLevel()}'s empty body. A higher-level
...
Ninth, hardware instancing is absent at a different layer: this otherwise 3D-capable renderer
...
Unlike Vulkan/BGFX, this renderer carries independent pending colour, depth, and stencil flags
for the swapchain, each 2D target/MRT attachment, and each cube face. The matching
\texttt{SDL\_GPU\_LOADOP\_CLEAR} is selected only when that flag is set, so the renderer side is
...
CMake test registration directly (\texttt{modules/renderers/sdl-gpu/examples/CMakeLists.txt}) rather than trusting a
