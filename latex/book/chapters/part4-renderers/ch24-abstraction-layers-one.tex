...
\section{One view clear is broader than seven public combinations}

\texttt{EnsureViewState()} always installs the colour, depth, and stencil
\texttt{BGFX\_CLEAR} flags; it does not vary those flags with
\cnaclass{ClearOptions}. Colour and stencil entry points also touch the view, while
\texttt{ClearDepth()} merely changes the global scalar and cannot initiate an otherwise-empty
view. If a draw is later submitted, the full three-attachment clear still happens once at view
start. Multiple public clears in one unadvanced frame consequently collapse to the last global
values and precede all submitted draws. The cross-frame stencil regression proves the stencil
value is real, not same-view issue ordering or attachment selectivity; see
\S\ref{sec:backend-clear-matrix}.

BGFX's persistent view state makes target usage observable. When a 2D target requests
Preserve, the backend disables view clearing with
\texttt{BGFX\_\allowbreak{}CLEAR\_\allowbreak{}NONE}; Discard relies on the shared implicit
clear to install all three flags. Platform is passed to the backend as ``not preserve'' but
receives no shared clear, so it can inherit whatever clear flags that recycled view ID last
held. Cube binding and MRT creation likewise lack a usage-specific clear reset. The named BGFX
usage test only proves two-frame bind/unbind does not crash; despite its comments, it contains
no pixel oracle. The full distinction is
