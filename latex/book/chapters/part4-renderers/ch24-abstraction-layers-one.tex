...
Preserve, the renderer disables view clearing with
\texttt{BGFX\_\allowbreak{}CLEAR\_\allowbreak{}NONE}; Discard relies on the shared implicit
clear to install all three flags. Platform is passed to the renderer as ``not preserve'' but
...
skips), and the renderer-specific \texttt{ctest} suite passes 103 of 105. The two remaining
...
\texttt{modules/renderers/bgfx/examples/CMakeLists.txt} directly for anything instancing-related finds nothing at
all, in contrast to \texttt{D3D9}, \texttt{EasyGL}, \texttt{Vulkan}, and \texttt{WebGPU}, each
of which has its own dedicated instancing test file under its renderer module's \texttt{examples/} tree (a real
\texttt{*instanced*.cpp} naming pattern, one file per renderer). This renderer is the one real
gap in that otherwise complete cross-renderer picture --- implemented, never independently
pixel-verified.
...
passes only that one buffer's renderer pointer through as \texttt{GpuDrawParams::instanceVb}:
...
Two real, silent-failure paths follow directly from this renderer's own implementation of the
...
GPU instancing on this renderer --- a large tiled foliage field, a particle system driven
...
important limitations, while later feature work should be evaluated from the current renderer
