...
\section{Recorded BGFX campaign}
...
Chapter~\ref{ch:counting-discipline}. The two remaining outcomes were classified as environment
ceilings: one render-target MSAA-resolve test
...
three times without finding a root cause and remains deferred. The row-by-row triage recorded
38 gaps, closed 37 in its final batch, and left the depth-bias item described below; that item was
subsequently fixed. These are historical campaign results, not present-day population counts.
...
\subsection{Routing a test around a BGFX/OpenGL limitation}
...
\section{DepthBias: an upstream BGFX limit}
...
The screenshot flush route follows from bgfx's numbered \emph{views}, an ordering mechanism
distinct from render-target binding. Every
\texttt{bgfx::submit()} call targets a specific numeric view id, and bgfx processes views in
ascending id order each frame. The original renderer assigned view 1 to every render target.
\texttt{Detail::AllocateRtViewId()} now allocates from a free-list-backed pool:
...
View 0 is reserved for the backbuffer. \texttt{kBackbufferFlushViewId} is the highest reserved
view, so bgfx processes it last; the allocator's exclusive upper bound keeps it out of the
render-target pool. Freed IDs return to \texttt{RtViewIdFreeList()}, preventing normal
creation/destruction churn from exhausting bgfx's \texttt{BGFX\_CONFIG\_MAX\_VIEWS} ceiling
(256 by default). Simultaneously live targets can still exhaust the bounded pool, in which case
creation throws \texttt{std::runtime\_error}.

\section{Sampling a render target: a wrong-handle-type cast, since fixed}
\label{sec:bgfx-wrong-handle-cast}

Both \texttt{BgfxSpriteBatchRenderer::Draw} and the environment-mapping branch once cast any
\cnaclass{ITextureRenderer} to the ordinary-texture concrete class. Render targets use sibling
classes whose first member is a framebuffer handle. Because bgfx represents framebuffer and
texture handles with the same \texttt{uint16\_t idx} layout, the cast compiled and usually did
not crash; it used a framebuffer-pool index as a texture-pool index and could sample an unrelated
texture.

The current SpriteBatch route obtains dimensions through virtual \texttt{GetWidth()} and
\texttt{GetHeight()}. Environment mapping resolves cube sampling through
\texttt{IBgfxCubeSamplable} with a checked cast. Focused 2D and cube render-target tests cover the
non-crashing type-confusion that ordinary smoke tests missed.

\section{Corrected silent-black-mesh defect}
...
\subsection{Recognizing the trigger condition}
...
The shared renderer interfaces once left \cnaclass{Texture3D} and \cnaclass{TextureCube}
\texttt{GetData()} as silent no-ops. The current copy-before-read route avoids placing a
readback-only usage flag on ordinary sampled textures. BGFX documents
...
\section{Current qualification}

The recorded closure predates the readback and mip work above. The still-deliberate
...
than from the historical campaign summary.
