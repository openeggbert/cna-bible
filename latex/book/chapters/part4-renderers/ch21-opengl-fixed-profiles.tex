...
Missing features confirmed still open include: the backend-level
...
backend's persistent FBO preserves both Preserve and Platform when not explicitly cleared
...
The default backbuffer has a related but distinct selection gap. The backend factory never
...
\begin{sourcenote}
EasyGL is the only product whose backend hook adds real work to
...
The seven backend methods are real immediate \texttt{glClear} paths, and the depth/stencil
...
prime/test depth around a target-only call. This is why the cross-backend matrix in
