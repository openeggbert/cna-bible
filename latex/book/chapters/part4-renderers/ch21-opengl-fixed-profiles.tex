\chapter[OpenGL I: The Independent Compatibility Ladder]{OpenGL I: The Independent\\Compatibility Ladder}
...
\texttt{OPENGL2}, mobile fixed function through \texttt{OPENGLES1}, and the CPU
\texttt{TINYGL} family. Chapter~\ref{ch:opengl-programmable}
...
\section{Four identities, four implementation families}
...
\texttt{TINYGL} & CPU fixed function & none & one software framebuffer; no render targets, cube/volume textures, or queries \\
\bottomrule
\end{longtable}

All four implement CNA directly. None routes through EasyGL, and none is an alias for an
...
\section{TINYGL: fixed function as a CPU rasterizer}

\texttt{TINYGL} fetches the pinned C-Chads TinyGL revision and renders a fixed-function OpenGL
1.x-style pipeline into its own CPU framebuffer. It owns ordinary and indexed extended draws,
including colored, textured, colored-textured, normal-textured and skinned layouts. It reports
only \texttt{ThreeD} and \texttt{WireFrame} among the graphics capabilities.

The single-context framebuffer is also the hard boundary: there is no FBO model, so
\cnaclass{RenderTarget2D}, cube targets and MRT are unavailable; texture cubes, volume textures,
occlusion queries, programmable effects, instancing, multi-stream input, MSAA and anisotropic
filtering are refused. This is a real CPU 3D renderer, not the SOFTWARE family and not a native
OpenGL context. The tag contains fourteen TinyGL suites with 113 named checks and a dedicated
workflow; this edition records their presence rather than claiming a fresh workflow run.
...
\texttt{TINYGL} & native CPU execution with framebuffer readback & fixed-function subset and fetched dependency \\
\bottomrule
\end{longtable}
...
Chapter~\ref{ch:opengl-programmable}. Choose \texttt{TINYGL} when a CPU-only fixed pipeline is
the intended deployment; a similar API surname does not erase the implementation
boundary.
