...
target's own height (the current binding record's \texttt{height}) for the coordinate-origin flip instead of the
...
The earlier Task~880 account also missed a draw-family correction that has since landed.
\texttt{EasyGLSpriteBatchRenderer::FlushBatch()} reads the active native viewport. For a genuine
sub-region it leaves the GL rectangle in place and builds its orthographic projection from that
viewport's width and height, making sprite coordinates viewport-local as in FNA. Only the default
full-target case resets to the full active render target or physical window. Dedicated pixel tests
cover one custom viewport and two batches using different viewports in one frame, so SpriteBatch
no longer overwrites a public custom rectangle or leaves later 3D work behind a false public cache.
...
the seven typed \texttt{VertexPositionXxx} shapes --- reaches the renderer through a different,
