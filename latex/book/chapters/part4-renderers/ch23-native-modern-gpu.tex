...
Five public identities belong in this cluster, but they do not share one implementation.
...
\texttt{IGL} & native & yes / yes & explicit no & IGL shader stages on OpenGL or Vulkan \\
\bottomrule
\end{longtable}

The table states observed behavior as well as capability bits. \texttt{SDL\_GPU} inherits the base
...
The Vulkan history illustrates a recurring defect shape: an interface accepts a complete state
packet while the implementation consumes only part of it.

\section{Historical Vulkan campaign baseline}
...
tests and three segfaulting skinned-model content tests. Both were closed before the recorded
campaign baseline, without test exclusions.

\section{BlendState: native mapping and historical defect}
...
five independent pixel tests before the fix. The fix replaced this with a
...
\section{Stencil testing: another discarded-parameter defect}

\cnaclass{DepthStencilState} (Chapter~\ref{ch:state-objects}) had an almost identical
history. The renderer's state-application entry point accepted fifteen parameters but
stored only two of them (the depth-enable and depth-write flags) --- every
...
independent-override path (Chapter~\ref{ch:state-objects}) on Vulkan specifically. BGFX has
since closed the same gap with its own dynamic rebuild and differential test; EasyGL remains
the audited exception. A second, smaller bug found in the same investigation: depth
format selection tried a stencil-\emph{less} format before it tried any stencil-capable
alternative.

\section{OcclusionQuery: current contract and render-pass limit}

Before its fix, occlusion queries on this renderer were disconnected from draw submission. The
renderer defers 3D and 2D draws into a pending-batch
...
reported zero regardless of visibility. The fix resolved three design questions: how to tag an individual pending draw with the query
...
\subsection{Using OcclusionQuery on Vulkan}
...
query.Begin();
DrawBoundingProxyGeometry(occluderCandidate); // a cheap stand-in for the detailed mesh
query.End();

// One or more frames later, after the GPU result is available:
if (query.getIsCompleteProperty() && query.getPixelCountProperty() > 0) {
    DrawFullDetailMesh(occluderCandidate);
}
\end{lstlisting}

A query may cover several draws within one render pass. The pinned Vulkan route does not sum a
query correctly across a render-pass boundary. Keep render-target changes outside an
\cnaclass{OcclusionQuery}'s \texttt{Begin()} / \texttt{End()} span.

\section{Remaining limitation}
\label{sec:vulkan-depthbias}

One unresolved observation remains: the extreme-magnitude
\cnaclass{RasterizerState.DepthBias} case produced no visible bias in the recorded environment.
The same result occurred on D3D9 (Chapter~\ref{ch:direct3d-backends}), so the evidence does not
attribute it to Vulkan rather than the shared driver environment.

\section{Two corrected render-target black-frame defects}
\label{sec:vulkan-rendertarget-black}

\begin{historynote}
\texttt{docs/rendertarget-support.md} still lists two black-frame defects as open. The pinned
source and regressions close both; the document is historical.
\end{historynote}

The first defect was not cube-specific. \texttt{VulkanRenderer::Clear()} once changed only
global clear values, while command recording discovered passes solely from draw calls. A
...
The first repair added a deduplicated target list so a clear could create a pass without a
draw. The current architecture has superseded that intermediate mechanism: each public
\texttt{Clear()} is now a \texttt{PendingClear} carrying its target, bind-cycle segment,
monotonic command order, selected aspects, values, and an owning target reference. Command
recording discovers clear-only segments from that stream itself. The dedicated clear-only
\cnaclass{RenderTarget2D} regression still fills two targets without a draw and samples both,
while the newer ordered-clear suite proves this is no longer merely a pass-existence fix.

The second regression renders SpriteBatch content into all six faces of a
\cnaclass{RenderTargetCube}, unbound, then sampled through \cnaclass{EnvironmentMapEffect}.
The original regression now produces the expected blue center across five repeated runs and its
registered invocation. The project did not bisect the fixing commit. A later cube-view/layout
rewrite is a plausible cause, but the relevant transition is mip-count-gated while this fixture
uses one level. The defect is verified closed; its fixing commit is unknown.

\section{Clear is aspect-selective and issue-ordered}

Focused attachment tests establish that colour, depth, and stencil values reach Vulkan.
\texttt{REMED-GFX-129} closes the ordering contract: every public
clear is now an ordered command at its exact position among SpriteBatch and 3D draws, with
independent target/depth/stencil flags. A leading clear on a discard cycle may fold into the
render-pass load action; a clear after any draw is emitted as \texttt{vkCmdClearAttachments}
after replaying the preceding command slice. Preserve targets use the latter path even for a
leading explicit clear, so their load operation does not swallow it. \texttt{ClearDepth()}
also records a clear-only segment rather than merely updating a global fallback value.

Two renderer-neutral suites make those details observable. The eight-mask
\texttt{Vulkan\_\allowbreak{}GraphicsDevice\_\allowbreak{}ClearOptions} fixture establishes old colour/depth/stencil
contents and encodes each surviving aspect into distinct colour probes. The ordered-clear
fixture queues draw--clear, clear--draw, multiple-clear, backbuffer, 2D-target, cube-face, and
MRT sequences without an intervening readback or present. A renderer that collapsed them into
one pass-opening value would therefore fail even if its eventual clear colour were correct.

Render-target usage now follows one shared predicate: only \texttt{DiscardContents} maps to
discard; \texttt{PreserveContents} and \texttt{PlatformContents} both map to preservation.
Vulkan receives that Boolean for 2D and cube targets. Its single-sample and MSAA render-pass
caches provide matching clear/load variants, and preserving depth/stencil attachments receive
the layouts and store operations needed to survive a full unbind/rebind cycle. Cube colour is
per face while its depth/stencil image is deliberately shared by all six faces, matching FNA.
The cube-usage, per-face-MSAA, and depth/stencil-usage suites distinguish those paths, including
Platform. Discard still produces the deterministic shared black/max-depth/zero-stencil bind
clear, and an explicit ordered clear supersedes either usage policy. See
...
A cube-map render target's MSAA route uses a multisampled 2D texture array and a separately
cube-flagged resolve texture. Multiple render targets are capped at four by shared code mirroring
FNA's \path{MAX_RENDERTARGET_BINDINGS}; the cap does not come from Vulkan's device limit.
\texttt{Texture2D} uploads above mip level zero allocate and update the requested subresource,
and a minification test samples its authored color. Chapter~\ref{ch:backend-architecture},
\S\ref{sec:texture2d-update-matrix}, gives the cross-renderer proof boundary. Non-
\texttt{Color} formats remain blocked by the shared \texttt{Texture::ValidateFormat} contract
described in Chapter~\ref{ch:easygl-backend}, not by a Vulkan-specific refusal.

\section{Render-target mipmaps, MSAA, and viewport}

\begin{historynote}
\texttt{docs/rendertarget-support.md} lists render-target mip generation, MSAA, and custom
viewport effects as open for Vulkan. All three are implemented at the pin; those status rows
predate the current source and tests.
\end{historynote}

\texttt{VulkanRenderTargetRenderer::MaybeGenerateMips()} runs a per-level
\texttt{vkCmdBlitImage} cascade against resolved level-0 content after the render pass.
Render-target MSAA covers both \cnaclass{RenderTarget2D} and \cnaclass{RenderTargetCube}. Each
SpriteBatch batch and 3D draw captures the active viewport and applies it while replaying its
render-target segment. Custom sub-regions therefore work on the backbuffer and
\cnaclass{RenderTarget2D} without collapsing to the final value at \texttt{Present()} time.
Dedicated target and deferred-capture pixel tests cover both scopes and multiple viewport changes
inside one unflushed bind cycle.

Scissor follows the same per-command rule. Each queued draw retains the rectangle and
\texttt{ScissorTestEnable}; replay clamps an enabled rectangle to the current target and expands
disabled or degenerate state to its full extent before \texttt{vkCmdSetScissor}. The target and
deferred-scissor oracles establish that later state cannot retroactively unclip earlier queued
draws. Pass-opening full-target state is a default; each draw replaces it with its snapshot.
...
variant (Appendix~\ref{ch:cnaext-catalog}) --- so an SRGB swapchain format would apply an automatic gamma encode XNA
...
lack of an exotic present mode. \texttt{PresentInterval::Two}'s mapping
...
\subsection{Two additional readback findings}
...
\section{The 512-live-descriptor-set ceiling}
...
\section{IGL: one CNA identity over two IGL backends}

\texttt{IGL} integrates Meta's IGL~1.1.1 and can create an OpenGL or Vulkan device below the one
CNA identity. Its renderer owns effect-aware 3D draws, sprite submission, real 2D/cube targets,
two-to-four-target MRT, and volume-texture upload/sampling. It has no IGL occlusion-query API;
CNA returns an immediately complete zero-sample object and reports the capability false, avoiding
a spin or false GPU promise.

The backend distinction remains evidence-relevant. OpenGL and Vulkan have separate context,
coordinate and resource behavior even though \cnaclass{GetGraphicsRendererType()} says IGL. The
release examples bring up both backends, but their existence is not a claim that this edition
reran every IGL check. Volume upload and sampling are supported; volume readback is an explicit
IGL~1.1.1 gap and raises rather than fabricating voxels.

\section{Comparing the five evidence tiers}
...
macOS lifecycle gate while its 3D pixel surface remains explicitly unproven. \texttt{IGL} has
backend-specific example evidence and explicit capability boundaries that must be named as
OpenGL or Vulkan.
...
\input{chapters/part4-renderers/fragments/legacy-webgpu.tex}
\input{chapters/part4-renderers/fragments/legacy-sdlgpu.tex}
