...
Task~875 was not specific to cube targets. \texttt{VulkanGraphicsBackend::Clear()} once changed
...
against the other thirteen products in \S\ref{sec:backend-clear-matrix}.

The constructor comparison creates two additional usage boundaries. Only exact
\texttt{Preserve\allowbreak{}Contents} reaches the backend as true, so
...
cross-backend matrix and proof boundary. Forwarding a real, non-\texttt{Color}
...
\begin{sourcenote}
That deliberate UNORM choice is fixed backend policy, not a response to the caller's current
...
backend constructor. \texttt{VulkanGraphics\allowbreak Backend} does not override
\texttt{IGraphicsBackend::SetSwap\allowbreak Interval()}, so a later
...
test-only full-backend recreation helper) can currently change that initial choice. No
...
section covers what actually happens every frame once it exists. This backend double-buffers
...
\texttt{VulkanGraphicsBackend.hpp} --- with one \texttt{VkFence}, one pair of
...
\texttt{Present()} calls the backend's real \texttt{SubmitFrame(bool deferSwap)} with
...
layout every other backend and \cnaclass{Color} itself use --- \texttt{ReadBackbuffer} checks
...
every device this backend might run on.
...
Every \cnaclass{Texture2D} and every \cnaclass{RenderTarget2D} constructed on this backend
...
\cnaclass{VulkanTextureBackend}'s own constructor and \cnaclass{VulkanRenderTargetBackend}'s
...
\cnaclass{RenderTarget2D} instances combined on this backend, not 512 of each independently.
The \texttt{VK\_DESCRIPTOR\_POOL\_CREATE\_FREE\_DESCRIPTOR\_SET\_BIT} flag means this is a real
budget of \emph{concurrently undisposed} resources, not a lifetime total: each backend's own
...
construction time, on this backend specifically --- not a per-frame budget, and not shared with
any other backend's own resource limits (EasyGL and BGFX have no equivalent fixed-pool
ceiling at all; each stock effect type on this same Vulkan backend additionally has its own,
...
realistic way to reach this ceiling in practice --- worth knowing as a real, backend-specific
...
% Phase C: preserve the two other pre-existing native-modern renderer chapters as
% sections until the source-grounded Part IV rewrite redistributes this material.
