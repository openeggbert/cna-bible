...
Task~871 first established that colour, depth, and stencil values reach real Vulkan
attachments. \texttt{REMED-GFX-129} then closed the more demanding contract: every public
...
command-buffer-record time. The first repair covered the backbuffer only;
\texttt{REMED-GFX-062} then snapshots the active viewport into every SpriteBatch batch and 3D
draw and applies that captured value while replaying the draw's own render-target segment.
Custom sub-regions now work on the backbuffer and \cnaclass{RenderTarget2D}, rather than
collapsing to a full-target reset or to the final live value at \texttt{Present()} time.
Dedicated target and shared deferred-capture pixel tests cover both scopes and multiple
viewport changes inside one unflushed bind cycle.

Scissor follows the same per-command rule. Each queued draw retains both the rectangle and
\texttt{ScissorTestEnable}; replay clamps an enabled rectangle to the current target and
expands disabled or degenerate state to its full extent before \texttt{vkCmdSetScissor}. The
target-scissor fixture and cross-renderer deferred-scissor oracle prove that a later rectangle,
target switch, or ordinary full-state restore cannot retroactively unclip earlier queued
draws. The pass-opening full-target viewport/scissor calls remain valid defaults, but every
real draw overrides them from its own snapshot.
