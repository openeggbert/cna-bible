...
\texttt{PendingBufferCount} tracks queued plus submitted chunks rather than decrementing eagerly
at submission time. Its whole-chunk accounting is regression-tested in the no-resampling case;
the source-byte/output-byte conversion boundary under resampling remains open below.
...
sample rate and mono/stereo channel layout for the complete vector, and a range form adding byte
offset/count plus loop start/length) --- both requiring \emph{headerless} PCM data as already emphasized;
...
SoundEffect beep(pcmData, 44100, AudioChannels::Mono);
...
\subsection{A fixed early-pop bug, and the remaining resampling boundary}

The queue count is worth the same scrutiny as the \cnaclass{Cue} lifetime bug above, because a
real, externally-reported bug (found the same week, independently
...
that specific chunk's own full byte count had actually finished playing. The regression fixture
submits two one-second 44.1-kHz stereo S16 chunks, each
$44{,}100\times2\times2=176{,}400$ bytes. The old loop could report one pending chunk after only
about 50~ms and none after about 1.2~s, rather than two and one respectively.
...
That fix is not yet a general audible-consumption oracle. \texttt{submittedChunkSizes\_} records
source bytes, while \texttt{SDL\_GetAudioStreamQueued} observes the live stream after SDL has
established its conversion destination. When source and mixer formats differ, those byte counts
need not share a unit. The pinned audio plan therefore still leaves
\texttt{AUD-07-011} (completion accounting under resampling) and the 22.05/44.1/48-kHz dynamic
conversion campaign open. Until those close, treat \texttt{PendingBufferCount} as proven against
the same-format regression, not as a sample-accurate playback-position signal for every rate.
...
Dynamic submission bounds-checks offsets and counts but does not require S16 byte counts or float
sample counts to be divisible by the selected channel-frame width; the corresponding
\texttt{AUD-07-005/006} P0 validation tasks remain open alongside the resampling completion gap.
...
\texttt{BufferReady} event, and an intended buffer-duration constraint of 100 to 1000
milliseconds in multiples of 10. The implementation does not currently enforce that contract
exactly: its setter validates \texttt{TimeSpan::Milliseconds}, the sub-second component, rather
than total milliseconds. The upper-bound branch can never fire and a duration such as 1,100~ms
has a 100-ms component and passes. Callback-cadence verification remains open.
...
throwing \texttt{System::ArgumentOutOfRangeException} for a bad \emph{millisecond component},
with the total-duration hole above. \texttt{Start()} / \texttt{Stop()} begin/end capture;
...
Capture-start truthfulness is also unfinished. \texttt{Start()} asks SDL for a 44.1-kHz mono S16
stream, but sets \texttt{State} to \texttt{Started} even when opening the device failed. A read
then returns zero, which is indistinguishable from a healthy stream with no data yet. The cached
device list has no refresh/hotplug policy. Permission denial, unplug, returned-format validation,
frame alignment and concurrent stop/read are consequently open contracts, not implications of the
basic capture tests.
...
const auto captureToken = mic->BufferReady.Add(
    [mic, &playback](System::Object*, const System::EventArgs&)
    {
...
// Before playback leaves scope:
mic->Stop();
mic->BufferReady.Remove(captureToken);
\end{lstlisting}
...
Dynamic-buffer resampling completion/frame alignment and microphone state/format/permission
boundaries likewise remain open in \texttt{plan\_audio.md}; real implementations and focused
tests do not by themselves close those campaigns.
