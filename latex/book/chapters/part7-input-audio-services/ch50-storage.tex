...
The selector's \texttt{sizeInBytes} and \texttt{directoryCount} arguments are ignored by every
overload; there is no capacity-based choice to make. \texttt{DeviceChanged} is declared but
never raised. If the lazily resolved root does not exist yet, \texttt{FreeSpace} and
\texttt{TotalSpace} return \texttt{LLONG\_MAX} rather than a measured filesystem value. These
properties preserve XNA's selection shape, not a removable-device inventory.

Two \texttt{CNAEXT} static methods exist specifically because CNA has no Xbox-360-style
...
The same root also stores Chapter~\ref{ch:gamerservices}'s achievements and leaderboards under
\texttt{GamerServices/}. Changing \texttt{SetAppNameEXT} therefore moves more than save
containers: existing local gamer progress becomes invisible under the newly selected app root.
...
namespace, end to end, ultimately roots itself in that one \texttt{CNAEXT} extension point.
...
if (container->FileExists("savegame.dat"))
{
    ...
    int offset = 0;
    while (offset < static_cast<int>(loaded.size()))
    {
        const int read = readStream->Read(
            loaded.data(), offset, static_cast<int>(loaded.size()) - offset);
        if (read == 0) break; // unexpected early EOF: reject the save below
        offset += read;
    }
    if (offset != static_cast<int>(loaded.size())) { /* reject truncated save */ }
}

container->Dispose();
\end{lstlisting}

\subsection{Two real, currently unguarded path inputs}

The first unchecked path arrives before a \cnaclass{StorageContainer} even exists.
\texttt{BeginOpenContainer(\allowbreak{}displayName,...)} stores \texttt{displayName} unchanged, and
\texttt{EndOpenContainer} passes it to a constructor that forms
\texttt{rootPath/displayName/playerFolder} and immediately creates the resulting directories.
Only the empty string is rejected. An absolute display name discards the root under normal
\texttt{std::filesystem::path::operator/} rules, while \texttt{..} components can climb above
it. This differs from \texttt{StorageDevice::DeleteContainer}, which now uses the shared
\texttt{ResolveContainedPath} helper and rejects absolute or escaping names. Opening and deleting
the same nominal container therefore do not currently share the same safety boundary.

The second unchecked input is inside the resulting container. Every method in the worked example
above --- \texttt{CreateFile}, \texttt{FileExists},
...
\subsection{A second, compounding gap: the five tests stop at StorageDevice}

The \texttt{..}-traversal gap above is worth reading beside the module's unusually narrow test
boundary. \texttt{modules/storage/tests/Microsoft/Xna/Framework/Storage/StorageDeviceTests.cpp}
contains the entire five-case Storage suite. Those cases prove that
\cnaclass{StorageDevice::DeleteContainer} rejects an empty name, an absolute path, a long
\texttt{..} escape, and \texttt{.}, while preserving a happy-path deletion. In other words,
the traversal question was reviewed, guarded, and tested --- but only on the device operation.

No test mentions \cnaclass{StorageContainer}, \texttt{BeginOpenContainer}, \texttt{GetFileNames},
\texttt{GetDirectoryNames}, or \texttt{GlobMatch}. The container path used by game file writes
therefore remains unguarded even though its neighboring device path is protected. The same
absence covers \texttt{StorageContainer}'s own small
...
four-tier platform-resolution chain, not a single fixed path, and it is lazily evaluated and
...
\texttt{SDL\_GetPrefPath} is the tier that normally resolves on desktop platforms --- on Linux
it typically returns an app-specific directory under \texttt{\$XDG\_DATA\_HOME} or
\texttt{\textasciitilde/.local/share} itself,
...
\subsection{Dispose is a flag and event, not an operation guard}

\cnaclass{StorageContainer::Dispose()} is idempotent, sets \texttt{IsDisposed}, and raises
\texttt{Disposing}; it does not close a retained native handle because the container owns none.
None of the file, directory, enumeration, property, or open operations checks that flag, however,
so they continue to work after disposal. The container also retains a non-owning
\texttt{const StorageDevice*}; destroying the device first leaves
\texttt{getStorageDeviceProperty()} dangling even though path operations themselves use the
already-copied storage path. Keep the device alive through its containers and treat a disposed
container as unusable even though the current implementation does not enforce either rule.
...
access) --- currently produces byte-identical CNA behavior and leaves sharing to whatever the
host C\texttt{++} runtime and OS do for a plain \texttt{std::fstream} open. A game relying on
...
present for API completeness with real XNA's own exception surface. At the pin it is thrown only
when \texttt{FreeSpace} or \texttt{TotalSpace} catches a filesystem exception; container opens
and file operations do not first consult \texttt{IsConnected} or translate their own filesystem
failures into this type. The local-filesystem backing makes physical removal less common than on
a real Xbox~360, but this is not a universal error wrapper for the namespace.
...
device-selection UI stubs, and the \texttt{CNAEXT} app-name/storage-root extension together
