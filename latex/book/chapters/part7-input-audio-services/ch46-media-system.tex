...
Devices-specific hardware-skip note attached to the wrong namespace; the current whole-project
audit counts 6{,}171 cases across all modules.
...
// library context), then handing it directly to MediaPlayer.
Song* introTrack = Song::FromUri("Intro Theme", "Content/Audio/intro.ogg");
MediaPlayer::Play(introTrack);
...
\subsection{The resolver advertises three undecodable extensions}

\cnaclass{MediaLibrary} correctly avoids indexing AAC and WMA, but the loose-file
\texttt{SongTypeReader} separately advertises \texttt{.opus}, \texttt{.aac}, and
\texttt{.wma} alongside MP3, Ogg, WAV, and FLAC. CNA's vendored SDL3\_mixer build has no Opus,
AAC, or WMA decoder. Resolution can therefore select a path that construction cannot decode.
This is a real inconsistency between extension discovery and the playback dependency, not a
promise that every listed extension works.
