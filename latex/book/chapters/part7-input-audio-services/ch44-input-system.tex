...
registered graphics backend, then falls back to pass-through. Those tiers make the result
backend- and presentation-dependent rather than a universal guarantee; the audit below and
...
\item[\texttt{ClickedEXT}] a \texttt{NOXNA System::MulticastAction<int>} event (Chapter~\ref{ch:sharp-runtime-namespaces}
...
\subsection{Logical coordinates are a backend contract}
...
the cache assignment even if the physical cursor landed elsewhere. The ASCII integration
test uses this same weak immediate set/get shape.

Other backends are not interchangeable:
...
\item BGFX and D3D9/11/12 also pass through, but those backends currently ignore logical
  presentation and draw in physical coordinates; input matches that limitation rather than
  the requested mode.
\item DX3's correct direct helper methods are shadowed by the SDL renderer created inside
...
The complete fourteen-product matrix and its test evidence are in
Chapter~\ref{ch:backend-architecture},
\S\ref{sec:backend-input-coordinate-matrix}. FNA has no corresponding backend split:
