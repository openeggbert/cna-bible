...
For all four synthetic types, no port is bound, \texttt{ConnectToHost} is a no-op,
\texttt{Find} returns empty without the 150~ms network wait, and \texttt{SendData} leaves no
local data available. Their useful behavior is session lifecycle: \texttt{StartGame},
\texttt{EndGame}, state changes, and events. Only \texttt{SystemLink} passes
\texttt{RealNetworkingEnabled}.
...
settled. The current net module contains 293 test cases; the whole audited project contains
6{,}171. Cross-machine delivery is proven by an out-of-process two-instance ENet harness, while
