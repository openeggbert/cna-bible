...
cache path. An absolute, drive, or UNC spelling is rejected before that manager call, as is a
relative path whose normalized logical form starts above the content root. The declaration looks
generic, but the implementation is explicitly instantiated only for \cnaclass{Texture2D} and
...
\texttt{optional} return is the necessary value-type distinction. Focused tests cover the
Texture2D sibling, empty, relative-escape, absolute, and null-manager paths.

Finally, \cnaclass{XnbReadLimits} is not a universal admission policy. Type-reader and
shared-resource counts are checked; collection and decoded-pixel limits work when a particular
built-in reader calls the corresponding helper. \texttt{maxObjectNestingDepth} now bounds both
nested generic type names and recursive object dispatch at 256; the latter uses an RAII guard so a
failed child read restores the counter. \texttt{maxStringBytes} is live for the raw type-reader
name, although it is checked only after \texttt{BinaryReader::ReadString()} has allocated and does
not apply separately to ordinary serialized String fields. Focused tests cover the default/custom
type-name limit, the reader-table propagation and name bound, and object depth both at and beyond
the limit.

\texttt{maxFileSize} remains the conspicuous partial control: it is consulted for the compressed
payload only after \cnaclass{ContentManager} has already read the whole XNB. An oversized
uncompressed file can therefore allocate first. Treat the defaults as observable defenses at
their named call sites, not proof that every XNB field has a pre-allocation file, string, or
...
either. The pinned source contains no separate CurveReader guard or focused count-limit
regression, so the common collection limit must not be projected onto this bespoke loop.
...
\subsection{Texture2DReader validates exact bytes and the allocatable mip topology}
...
The byte and dimension preflights now form one coherent boundary. Width and height must each be
positive; \cnaclass{CheckedMultiplyOrThrow()} computes \texttt{width * height * 4} without signed
overflow before the 256-MiB decoded-size check. A device is obtained before allocation, and either
axis above \cnaclass{GraphicsDevice::GetMaxTextureDimension()} is rejected. Each raw Color level
must contain exactly \texttt{levelWidth * levelHeight * 4} bytes. Each DXT level must likewise
match the exact block-rounded compressed byte count before decompression, so neither shortage nor
surplus reaches the minimum-based DXT helper. A negative declared count is a
\cnaclass{ContentLoadException}; a physically truncated stream remains
\texttt{EndOfStreamException}.

Mip topology is explicit rather than inferred permissively. The declared count must be positive
and no greater than the complete floor-halved chain. Because CNA's public Texture2D allocation can
represent either level zero alone or the complete chain, the reader accepts exactly those two
shapes and rejects a partial prefix that would otherwise allocate fabricated missing levels. This
also bounds every later shift and \texttt{SetData()} call to a real allocated mip. The decoded-size
limit still applies to the base RGBA image rather than an independent aggregate budget for all
levels, but a complete geometric mip chain adds less than one third over that base.
...
the reader. CNA now requires its dimensions, Color upload format, and mip count to match the file
before the first upload; it does not separately compare device identity. The returned C\texttt{++}
handle is still the moved/reused value, not a guarantee that a caller-owned object was overwritten
as a transactional replacement. FNA likewise reuses a non-null reference, but has managed
reference identity rather than this value-handoff boundary.

The focused tests prove registration, unsupported \texttt{Bgr565}, a mismatched Color byte count,
negative and zero dimensions, checked rejection of overflow-shaped dimensions, a one-axis device
limit, and an excessive mip count. They do not prove a successful direct Color or DXT decode,
legacy/version-4 mapping, Xbox swapping, partial mip-prefix rejection, exact compressed surplus,
...
\subsection{Texture3DReader slices DXT volumes and refuses unsupported renderers}
...
dimensions must also be individually positive. Both the decoded-byte product and the later voxel
count use \cnaclass{CheckedMultiplyOrThrow()}, and the voxel result must fit \texttt{Int32} before
the Color vector and API count are formed. These checks close both prior signed-overflow sites.
They do not add a per-axis device maximum or a reader-level aggregate mip-data budget.
...
reference-instance behavior, but notably unlike CNA Texture2D's transported value. For a new
resource, \cnaclass{Texture3D}'s constructor queries \cnaclass{GraphicsCapability::Texture3D} and
throws \cnaclass{NotSupportedException} on a 2D-only renderer. A renderer that advertises the
capability but returns no resource, or refuses the complete update, is also surfaced by
\cnaclass{SetData()} rather than silently discarding voxels. A supplied existing pointer still
bypasses the constructor capability query, but not that upload-time complete-or-refuse check.

Evidence for the ordinary wire is narrow. The joint 3D/cube unit file proves registration,
rejects one unsupported \texttt{Bgr565} word, zero width/depth, and checked-multiply overflow, then
directly feeds a locally written 2-by-2-by-1 Color body to \texttt{ReadUntyped()}. That test expects
a clean refusal when the renderer lacks Texture3D, or reads four exact voxels back when it supports
the capability. There is no external
Texture3D XNB anywhere in the fixture tree, no header/table/ContentManager XNB test, and no
direct DXT, multi-slice, mip, truncation, raw byte-count, version, missing-device, or
existing-pointer case. The whole-container fuzzer has real Model, Texture2D, and SoundEffect
...
ordinary field-order implementation with repaired arithmetic and explicit capability behavior,
not as coverage backed by external fixtures or complete adversarial volume-texture coverage.

\subsection{TextureCubeReader loads a real DXT1 cube and validates raw Color byte counts}
...
The uncompressed branch now follows Texture2D's exact equality guard. After
\texttt{ReadBytesExactOrThrow()} proves the stream supplied the declared count, the reader requires
exactly \texttt{faceSize * faceSize * 4} bytes before indexing the vector. Both an undersized and
oversized raw declaration therefore fail as \cnaclass{ContentLoadException}; the regression test
reproduces the formerly out-of-bounds undersized case.
...
product is now computed by \cnaclass{CheckedMultiplyOrThrow()}, so an overflow-shaped size fails
cleanly before the helper. There is no reader-level device-size admission (apart from a separate
D3D9 profile check inside the resource constructor). The base face cap is consequently an
arithmetic and per-face allocation guard, not an aggregate cube budget or portable GPU limit.
...
default null cube factory produces an object without backing storage. The public upload no longer
silently skips that case: the first face's \cnaclass{SetData()}
throws \cnaclass{NotSupportedException}, as does a renderer that declines the complete region. A
successful reader return therefore means all serialized face uploads reported complete, although
there is still no pre-allocation cube-capability query.
...
check exact pixels, face orientation/order, every face, DXT3/DXT5, or raw Color success. Focused
tests separately reject a mismatched raw cube byte count, zero and overflow-shaped cube sizes, plus
an unsupported Texture3D format; cube level topology, version/device/existing cases, cache/unload,
and another producer/platform remain uncovered.
...
The optional default character has a separate invariant even though the four parallel list lengths
remain unchecked. \cnaclass{SpriteFont}'s constructor requires a present default to occur in the
character map, and the property setter enforces the same rule before changing state. The fallback
lookup also checks for \texttt{end()} before dereferencing it, mirroring FNA's dictionary failure
instead of invoking undefined behavior. This applies equally to XNB and CNJ because both readers
finish by calling the same constructor. Graphics tests cover rejection at construction and set,
state preservation after a rejected set, a valid default, and actual unknown-character fallback;
the CNJ suite independently rejects a serialized absent default.
...
many runtime SpriteFont tests hand-build tiny fonts rather than validating XNB tables. No located
test covers a true XNB default character, list/count mismatch, duplicate or malformed character
map, invalid rectangles/floats, truncation, nonzero reader version, direct-existing failure,
cache/unload, another producer/platform, or a SpriteFont seed in the container fuzzer.
...
for a path which normalizes above the root and rejects absolute, drive, and UNC spellings. This
logical helper does not canonicalize the eventual filesystem candidate, so the broader top-level
manager behavior for an in-root symlink still applies. It also asks directly for
\texttt{Texture2D} or \texttt{TextureCube}.
...
exact-type loading/cast boundary. The shared external-reference suite covers a Texture2D sibling,
empty value, relative escape, absolute path, and null manager. No stock-effect-specific test
exercises a nonempty, missing, wrong-type, or symlinked external texture reference.
