\chapter{Loading an Asset: ContentManager and Resolution}
...
\subsection{An XNB header chooses a decoder; it is not yet a complete input-admission firewall}
\label{sec:xnb-container-boundary}

The ten-byte XNB header is parsed as three literal magic bytes, one of FNA's sixteen accepted
platform bytes, version 4 or 5, one flags byte, and a signed little-endian \texttt{totalLength}.
Bad magic/platform/version becomes \cnaclass{ContentLoadException}; a physically truncated header
instead retains \cnaclass{BinaryReader}'s end-of-stream exception. CNA distinguishes the real
\texttt{0x80} LZX and \texttt{0x40} MonoGame LZ4 bits, rejects the combination of both, and gives
LZ4 a clear unsupported-compression error. The other flag bits are not validated: a flag such as
\texttt{0x01} has neither compression bit and follows the uncompressed route. This is intentionally
FNA-shaped platform/version compatibility, including old identifiers, rather than a promise to
accept every later MonoGame platform or flag convention.

\texttt{totalLength} is not validated by \texttt{ParseXnbHeader()} itself. Normal
\cnaclass{ContentManager} loading first reads the \emph{entire} filesystem file into a
\texttt{std::string}, then rejects only a declared length below ten or above the bytes actually
read. That fixed the historical over-read when a corrupted LZX header claimed more bytes than its
file contains, and the focused test proves that direction. It is deliberately not equality:
a smaller declared length and trailing physical bytes are accepted. The uncompressed path then
hands every byte after the ten-byte header to \cnaclass{ContentReader}, not only the declared
range, and no later end-of-asset check rejects unused trailing data. The LZX path instead computes
its compressed span from the smaller declared length and ignores physical trailing bytes. Thus the
same malformed under-declaration has two different payload boundaries; a header length is not a
file-integrity assertion on current \texttt{develop}.

The LZX route needs an additional four-byte decompressed-size field after the container header.
The current preflight accepts a compressed \texttt{totalLength} from ten through thirteen, then
constructs and reads that four-byte field before any check that fourteen physical bytes exist.
With a file that really ends in that range this is outside the established safe parsing contract;
with extra physical bytes it reaches the later negative-compressed-size rejection instead. No
located header, manager, container-fuzz, or LZX test exercises this short-compressed-prefix case.
The decompressed-size value is required to be nonnegative and at most
\texttt{maxDecompressedSize}; compressed bytes are separately limited by \texttt{maxFileSize}.

Those two LZX checks are useful but do not bound what the decoder writes. Each block declares a
frame size (normally 32 KiB), and \texttt{LzxDecoder::Decompress()} writes that full frame to a
growing \cnaclass{MemoryStream}. Only after all frames does the wrapper compare its accumulated
position with the declared decompressed size. A malicious compressed stream can therefore cause
the buffer to grow past the declared and configured decompressed limit before that final mismatch
is noticed; the stream's own signed maximum eventually yields a different allocation/stream
failure, not the promised content-limit error. The deterministic LZX fuzz test mutates two small
real payloads and is valuable crash coverage, but it does not construct an overproducing
multi-block stream or assert a cumulative output budget.

The name \texttt{maxFileSize} is therefore narrower than its declaration suggests. It constrains
only the declared compressed LZX payload after \cnaclass{ContentManager} has already materialized
the whole file; uncompressed XNBs and an LZX file padded after a small declared length have no
file-size gate at all. The public manager offers no limits parameter or setter, so ordinary
\texttt{Load<T>()} always uses the global defaults; only a standalone \cnaclass{ContentReader} or
direct decompression caller can provide a custom limit record. Current FNA is less defensive still:
it trusts its header lengths, treats the LZX bit as a boolean compression choice, preallocates the
declared decompressed buffer, and has no corresponding configurable ceilings. CNA's existing
checks are still worthwhile, but they do not justify treating untrusted containers as bounded.

The manifest scanner is intentionally weaker again. It reads every candidate file whole, parses
only an uncompressed header/table, does not compare its declared length with the bytes read, and
catches every failure into an indistinguishable empty reader-name list. That is acceptable for its
best-effort diagnostics role, but it must not be reused as validation. A source-level corrective
pass should make the physical and declared length exact, reject a compressed header shorter than
fourteen bytes before pointer arithmetic, stream or pre-limit input before allocation, and enforce
a cumulative LZX output ceiling during decoding. Those are CNA implementation decisions; this
book documents the present boundary rather than claiming that the independent
\texttt{feature/audit} branch has already supplied them.

\subsection{A type-reader name is a canonical registry key, not a validated .NET type identity}
\label{sec:xnb-type-name-contract}

The body begins with a 7-bit count, then one length-prefixed reader-name string and one signed
32-bit version for each entry. CNA preserves the raw string for diagnostics but turns its parsed
form into the key for the process-global \cnaclass{ContentTypeReaderManager} factory. A bare
name and its assembly-qualified form consequently select the same reader. The normalizer removes
the outer assembly portion and recursively removes every generic argument's assembly portion, so
the assembly/version/culture/public-key identity in an ordinary XNB is \emph{not} part of CNA's
reader identity. This is the practical C\texttt{++} substitute for FNA's reflection path:
FNA first tries an exact creator string, then rewrites selected framework assembly references and
asks the runtime to resolve a \texttt{Type}; CNA has no such type identity and must have already
registered the resulting canonical spelling.

That spelling is structural, not a general .NET-name validator. The parser takes every character
up to a top-level \texttt{[}, comma, or closing bracket as a base name; it neither trims nor
requires a nonempty base name, checks the backtick arity against the number of arguments, or
restricts the name alphabet. Its recursive generic parser correctly preserves nested bracket
pairing and strips each nested assembly qualifier, but the top-level caller intentionally ignores
the final cursor so that an outer assembly suffix need not be parsed. As a consequence it also
accepts arbitrary trailing material, including malformed delimiter text, after a completed name.
An empty or oddly spaced name normally fails only later as an unregistered factory key, rather
than at the table boundary. A generic's argument parser similarly need not consume all text
inside its bracket pair. These are compatibility-oriented canonicalization rules, not evidence
that a file supplied a syntactically valid reflected type.

Only the parser's limited bracket errors are normalized at this boundary: an unbalanced list, a
missing argument opener, or text other than a comma/closing bracket after an argument becomes
\cnaclass{ContentLoadException}. A malformed 7-bit integer, a truncated name or version, and
other \cnaclass{BinaryReader} failures propagate in their original exception family. The
\texttt{ParseXnbTypeReaderTable()} documentation currently promises one content-load error for
every malformed table, but its catch block covers only \texttt{std::invalid\_argument}; callers
must not rely on that broader promise. The focused tests cover healthy bare and nested generic
names, two deliberately malformed bracket cases, count-limit rejection, and one real MonoGame
entry. They do not cover empty names, trailing garbage, whitespace, arity disagreement, truncated
fields, duplicate names, or an overlong name.

The count itself is the one admission value checked here: it must be nonnegative and no greater
than \texttt{maxTypeReaderCount} (4,096 by default), before the vector reserves storage. A zero
entry table is also accepted; a later non-null root dispatch then has no selected reader. Each
name, however, goes through ordinary \texttt{BinaryReader::ReadString()}; its declared byte
length is not compared with \texttt{maxStringBytes}. For CNA's seekable in-memory body stream
the binary reader first rejects a length beyond the remaining bytes, but a physically large,
well-formed name can still allocate below that stream bound. Nor does parsing generic brackets
consult \texttt{maxObjectNestingDepth}. Deeply nested generic names recurse both in bracket
parsing and canonical-string reconstruction, and the temporary bracket depth is a signed
\texttt{int}. The advertised string and object-depth limits are therefore still not reader-name
limits.

Finally, the table deliberately preserves file order and does not reject duplicate raw or
canonical names. \cnaclass{ContentReader} creates a fresh registered reader for every entry and
only then checks that entry's serialized version, so two identical names may occupy distinct
dispatch indexes (and may fail independently if their versions differ). An unknown canonical name
is rejected as unregistered before version compatibility is consulted. This is sufficient for
ordinary XNB dispatch, but it means table admission is not a uniqueness, identity, or resource
policy. A future hardening pass should impose a per-name byte limit and generic nesting limit,
require full structural consumption with a defined assembly-suffix grammar, and choose explicitly
whether empty, duplicate, or arity-inconsistent keys remain accepted for compatibility.
...
for arbitrary higher levels and the backend is asked to update them. Large levels also make the
...
owns a non-copyable backend handle. It inherits exact serialized version zero. CNA and current
...
backend. Thus the base decoded-size cap neither validates a serialized mip topology nor protects
all backend update paths.
...
reference-instance behavior, but notably unlike CNA Texture2D's transported value. Backend
support is not checked at either point: the default 3D backend factory can return null, after
which Texture3D's upload and readback paths simply skip backend work. Loading an XNB can therefore
return a nominal Texture3D whose supplied voxels were not represented by a real backend resource
...
or backend-capability case. The whole-container fuzzer has real Model, Texture2D, and SoundEffect
...
or universally backend-safe volume-texture support.
...
the backend update. The reader also has no generic cube-capability gate: a backend using the
default null cube factory produces an object whose upload/readback methods simply skip backend
...
many backend SpriteFont pixel tests hand-build tiny fonts rather than validating XNB tables. No
...
or a model draw on a real backend.  Successful loading of the cube therefore establishes the
...
\subsubsection{\cnaclass{SongReader}: an external-file reference}
\label{sec:xnb-song-reader}

\cnaclass{SongReader} is registered as a version-zero reader.  Its canonical factory key is:

\begin{lstlisting}
Microsoft.Xna.Framework.Content.SongReader
\end{lstlisting}

It has no embedded encoded-audio payload, format tag, byte count, shared resource, or decoder
hand-off.  After the root-object reader index, its complete body is one XNB \cnaclass{String}
followed by one signed \cnaclass{Int32}:

\begin{lstlisting}
String  referenceName  // normal XNB 7-bit byte-length prefix, then UTF-8 bytes
Int32   durationMs
\end{lstlisting}

The first value is a file reference, not an invocation of the generic external-reference
mechanism.  CNA first combines the owning manager's \cnaclass{RootDirectory} and the logical asset
name (for example,
\texttt{Content/Music/Menu}) and takes that path's parent.  It replaces backslashes with forward
slashes in both inputs, appends the serialized reference as a sibling, and applies only C++ lexical
path normalization.  Thus a reference such as
\texttt{"../Audio/loop.wma"} means a sibling-directory lookup rather than a second content-manager
load.  This matches the role of FNA's \cnaclass{FileHelpers.ResolveRelativePath()}, but it is an
important distinction from CNA's generic external-reference helper: that helper rejects a logical
path which climbs above the content root, while \cnaclass{SongReader} has no equivalent containment
check.  A sufficiently deep \texttt{..} reference, an absolute reference, or a symlink reached
after lexical normalization can therefore name a real file outside the configured content tree.
Neither lexical normalization nor the subsequent existence probes canonicalizes the path or proves
that it is a regular audio file.  Compiled content is normally trusted input here; it is not a
sandboxed reference format.

For a resolved path longer than four characters, the reader removes the final four characters
without inspecting what they are, then calls \cnaclass{Normalize()} on the resulting stem.  It first
accepts the extensionless stem itself if it exists, then probes \texttt{.ogg}, \texttt{.oga}, and
\texttt{.qoa}, in that order.  The first existing candidate wins.  If all four probes fail, it
restores the original, unstripped path.  This is FNA's historical XNA compatibility rule: an XNA
asset conventionally stores a \texttt{.wma} placeholder, while a desktop runtime substitutes an
available supported companion.  It also happens to round-trip MonoGame content which has already
serialized \texttt{.ogg}, because removing and then re-appending its four-character extension
finds the same file.  It is not general extension validation: \texttt{.mp3} loses four characters
too, and the three supported spellings are case-sensitive filesystem probes on a case-sensitive
host.

There are two malformed-edge differences worth keeping separate from the valid-format contract.
CNA skips the stripping step when the resolved string has four or fewer characters; FNA calls
\texttt{Substring(0, length - 4)} unconditionally, so a shorter string throws before either
duration or constructor processing.  For a four-character string FNA probes the empty stem, while
CNA retains the original string.  Neither behavior is covered by a checked-in test and neither is
how a real \cnaclass{Song} asset is emitted.  More practically, CNA's final \cnaclass{Song}
constructor tests \cnans{std::filesystem::exists()}, which admits an existing directory; FNA's
\cnaclass{File.Exists()} rejects a directory.  A CNA directory reference can consequently survive
loading and fail only when the media backend later attempts to open it.

\paragraph{The media file is not decoded here, but a missing file still fails during \texttt{Load}.}
The reader performs its path probes before it reads \texttt{durationMs}, then constructs
\cnaclass{Song} only after that integer has been consumed.  CNA's constructor performs the final
existence test and throws \cnaclass{FileNotFoundException} when the selected candidate (or the
unstripped fallback) is absent.  Consequently a truncated duration can produce a stream exception
first, but a well-formed XNB whose companion is missing fails from \cnaclass{ContentManager::Load<Song>},
not when the game first calls \cnaclass{MediaPlayer::Play()}.  This corrects a stale claim in the
current CNA reader and test comments: current FNA's own \cnaclass{Song} constructor also calls
\cnaclass{File.Exists()} and throws \cnaclass{FileNotFoundException} immediately.  The parity boundary
is therefore eager \emph{existence}, not lazy failure.  CNA's media player still opens the file
lazily with SDL3's streaming music loader, so deletion/replacement after loading, a directory
accepted by CNA, or an unsupported/corrupt-but-existing audio stream remains a later playback
boundary rather than an XNB decoding result.

Unlike scalar readers, \cnaclass{SongReader} cannot use a standalone \cnaclass{ContentReader}.
It requires a non-null owning \cnaclass{ContentManager} to form the base path and throws
\cnaclass{ContentLoadException} before consuming either body field when none is present.  Normal
\cnaclass{ContentManager::Load<Song>()} always supplies one, but the direct reader surface does not.
The reader does not opt into deserializing an existing object, so normal root dispatch passes no
existing song.  A direct C\texttt{++}
\cnaclass{ReadUntyped()} call can still box a copyable \cnaclass{Song} in \cnans{std::any}; the
generic adapter unboxes it, but \cnaclass{SongReader::Read()} intentionally ignores it and returns a
fresh value, just as FNA's typed reader ignores its \texttt{existingInstance} parameter.  CNA's
ordinary manager cache is a separate value-cache story: \cnaclass{Song} is copyable, so a later
matching \cnaclass{Load<Song>()} returns a copy of the cached value, not an in-place deserialization
of an earlier caller's object.

\paragraph{What the real companion fixture proves --- and what it does not.}
The vendored MonoGame fixture \texttt{one\_two\_three.xnb}, in the checked-in uncompressed Song
fixture directory, is a 131-byte version-five XNB.  Its root reader is \cnaclass{SongReader}; the
body names its \texttt{.ogg} companion and records \texttt{769282} milliseconds.  Its genuine
72,796-byte companion sits beside it.  The end-to-end manager test proves the
canonical registration, header/table dispatch, sibling lookup, \texttt{.ogg} strip-and-reprobe
round trip, final handle, and duration together; a built-in-registration test repeats the load
with a fresh manager.  A hand-built full XNB additionally proves that an unresolved reference
propagates CNA's \cnaclass{FileNotFoundException} through the manager path.

That is useful but intentionally narrow evidence.  The fixture is MonoGame-produced rather than
an original XNA \texttt{.wma}-placeholder asset, and it neither plays/decodes the companion nor
proves the \texttt{.oga} or \texttt{.qoa} alternatives.  No located test covers a null manager,
direct existing-instance input, sibling \texttt{..} and absolute/symlink paths, separator
normalization, short or malformed strings, negative/extreme duration values, directory admission,
or a Song root in the whole-container fuzzer.  Treat the successful fixture as evidence for the
one real companion graph it contains, not as a security or media-playback certificate.

\subsubsection{\cnaclass{VideoReader}: five trusted metadata fields around a deferred media open}
\label{sec:xnb-video-reader}

\cnaclass{VideoReader} is also registered at reader version zero, under the canonical factory
key:
...
Like \cnaclass{SongReader}, it serializes an external media name rather than encoded media bytes.
Unlike Song, the reference is followed by four more pieces of metadata.  Its complete root-object
body is therefore:
...
Int32   width
Int32   height
Single  framesPerSecond
Int32   soundTrackType      // cast directly to VideoSoundtrackType
\end{lstlisting}

FNA reads these fields through its generic \texttt{ReadObject<T>()} calls, while CNA calls the
matching typed primitive methods directly.  That is a C\texttt{++} implementation-style
difference, not a second wire format: both consume the same String, four signed 32-bit values
(including the soundtrack enum), and one IEEE-754 single in the same order.  Neither reader
validates that the soundtrack integer is one of the three named \cnaclass{VideoSoundtrackType}
values.  It is simply cast, and current playback code on both sides treats that field as metadata
rather than a cue to duck, mute, or select an audio track.

\paragraph{Sibling resolution is deliberately filesystem-shaped, and not a containment boundary.}
The reader forms a base path from \cnaclass{ContentManager}'s \texttt{RootDirectory} plus the
logical XNB asset name, then resolves the serialized string as a sibling.  CNA changes
backslashes to slashes, uses \cnans{std::filesystem::path}'s lexical normalization, and probes
the resulting filesystem path directly.  FNA's \texttt{FileHelpers.ResolveRelativePath()} uses a
URI to collapse the same relative-directory syntax before using its filesystem probes.  Neither
path is \cnaclass{ContentReader::ReadExternalReference<T>()}: there is no generic logical-root
check here.  A sufficiently deep parent traversal, an absolute reference, or a symlink reached
after lexical normalization can consequently identify a file outside the configured content
tree.  This is compiled-content path handling, not a sandbox for an untrusted reference string.

After resolution, the normal XNA convention is a fake \texttt{.wmv} suffix.  Both readers remove
the final four characters without checking that they actually spell \texttt{.wmv}, first accept
the extensionless stem if it exists, then probe \texttt{.ogv} and \texttt{.ogg} in that exact
order.  The first existing path wins; when neither probe exists, the original unstripped path is
retained.  Thus a real \texttt{intro.wmv} can become an adjacent \texttt{intro.ogv} or
\texttt{intro.ogg}; a pre-existing \texttt{intro.ogv} also round-trips through strip and
re-probe.  This is narrower than Song's \texttt{.ogg}/\texttt{.oga}/\texttt{.qoa} set and does
not make CNA's ordinary loose-file \cnaclass{Video} extensions (such as \texttt{.mkv} or
\texttt{.mp4}) into supported XNB substitutions.

There are two small malformed-reference qualifications.  CNA only strips when the \emph{resolved}
path exceeds four characters; FNA calls \texttt{Substring(0, length - 4)} unconditionally, so a
short direct-reader path can fail in FNA before the numeric fields are read.  At exactly four
characters, both eventually retain the original string, although CNA skips FNA's empty-stem
filesystem probes.  CNA uses \cnans{std::filesystem::exists()}, which also reports a directory;
FNA uses \texttt{File.Exists()}, which does not.  No current Video reader test covers any of
these edge cases, separator normalization, a path escape, a symlink, the \texttt{.wmv} to
\texttt{.ogv}/\texttt{.ogg} success paths, or an invalid soundtrack integer.

\paragraph{Loading creates trusted metadata; opening and most validation happen later.}
\cnaclass{VideoReader} needs both an owning \cnaclass{ContentManager} and that manager's
\cnaclass{GraphicsDevice}.  A standalone \cnaclass{ContentReader} fails immediately with
\cnaclass{ContentLoadException} when no manager exists; a manager without a device fails after
the five fields have been read when it requests its internal device.  Normal
\texttt{ContentManager::Load<Video>()} supplies both in a configured game.  The resulting
seven-argument \cnaclass{Video} constructor stores the path, device, duration, dimensions,
frame rate, and soundtrack type verbatim.  It neither tests that the file exists nor opens a
decoder.  In particular, an unresolved but well-formed XNB reference loads successfully, and
negative/extreme duration or dimension values, non-finite frame rates, and arbitrary enum values
are admitted as trusted serialized metadata.

The actual FFmpeg open happens when \cnaclass{VideoPlayer::Play()} receives that Video.  A
successful open then checks the file's width and height and compares its frame rate against the
serialized value before allocating the frame texture.  This makes a validly parsed but
metadata-mismatched XNB fail at playback with \cnaclass{InvalidOperationException}, rather than
at \texttt{Load}.  There is a precise current FNA difference at the tolerance edge: FNA rejects
a difference of one frame per second or more (\texttt{>= 1.0f}), while CNA rejects only a value
strictly greater than one (\texttt{> 1.0f}); no CNA test exercises that exact one-fps boundary.
More seriously for callers expecting FNA-style failure, CNA's FFmpeg wrapper returns false when a
path cannot be opened or contains no usable video stream.  \cnaclass{VideoPlayer::Play()} then
returns in the stopped state without throwing.  FNA's ordinary Theora player opens the path when
played and throws \cnaclass{FileNotFoundException} if it cannot open it.  The timing is therefore
deferred in both designs, but CNA's unresolved-reference outcome is a silent stopped player,
not FNA's playback-time exception.  A corrupt existing file can similarly reach the decoder
boundary rather than becoming an XNB parsing error.

This graphics and decoder dependence also explains the platform boundary already described in
Chapter~\ref{ch:media-system}: on Windows, Android, and Emscripten the current build excludes the
FFmpeg-backed Video implementation.  The public declarations remain, but using this reader's
result reaches a link-time absence rather than an intentionally implemented runtime fallback.

\paragraph{Existing instances, caching, and the evidence actually present.}
\cnaclass{VideoReader} inherits the default false
\texttt{CanDeserializeIntoExistingObject} capability, so ordinary root dispatch supplies no
existing Video.  A direct type-erased C\texttt{++} call can still box a copyable Video in
\texttt{std::any}; the generic bridge unboxes it, but the reader discards it and constructs a
fresh value, just as FNA ignores its \texttt{existingInstance} parameter.  \cnaclass{ContentManager}'s
normal Video cache is consequently a separate value-cache: later \texttt{Load<Video>()} calls
return copies of the cached Video rather than populating an object the caller previously held.
No located test covers direct existing-instance input or a Video cache identity/lifetime case.

The strongest current end-to-end test builds a complete uncompressed version-five \texttt{XNBw}
container in memory, writes it beside CNA's 160-by-90, 25-fps, two-second
\texttt{chroma\_420.mkv} FFmpeg fixture, then loads it through the real header, type table,
registration, reader, manager, and \cnaclass{VideoPlayer} route.  It proves the five-field
order, GraphicsDevice hand-off, fallback to the original \texttt{.mkv} reference after the
\texttt{.ogv}/\texttt{.ogg} probes fail, and a first frame of the declared size.  Focused tests
also cover registration, all five declared values, deliberately mismatched metadata reaching
the deferred player check, and the missing-manager-device error.  This is meaningful integration
evidence, but its provenance matters: the \texttt{.xnb} is assembled by the test at run time and
the companion is CNA's synthetic FFmpeg media fixture.  No real MonoGame- or XNA-produced Video
XNB has been located; no real compressed Video XNB, XNB Video container-fuzzer seed,
missing-reference playback assertion, malformed/truncated field test, extension-probe success
case, direct-existing case, or external-producer differential test exists.  Song's real compiled
fixture is not evidence for this five-field Video body.
