...
\cnaclass{Texture2D} records its weak renderer entry; and move-only \cnaclass{SoundEffect} and
...
\cnaclass{Curve}] self-contained \texttt{.cnj} only. The CNAEXT
...
\texttt{const std::string\& rootDirectory}; and a \texttt{CNAEXT}-tagged no-argument
...
Two
\texttt{CNAEXT} extension points exist for cases the built-in readers don't cover:
...
reconstruct or use a graphics renderer whose own thread-affinity contract is separate.
...
to add the FNA-compatible resource-manager constructor or a deliberately CNAEXT equivalent, wire
...
remains fixed even if the working directory changes. CNA additionally exposes a CNAEXT public
...
\begin{description}
  \item[\cnaclass{Texture2D}.] Its table contains only weak references to the GPU renderer and
    optional CPU pixel shadow. Keeping a returned \texttt{Texture2D} value alive keeps that
    renderer alive; dropping all such values lets it free even if the content manager itself is
    ...
    next load builds a \emph{different} renderer; it does not assert that the retained first
...
\texttt{Texture2D} gets a newly constructed renderer on the post-unload load. They do not test a
...
current source. Treat those \texttt{CNAEXT} methods as diagnostics, not as a cache-lifetime API.
...
content tree independently, tooling can use CNA's real, \texttt{CNAEXT}-tagged inventory:
...
\section{Where this leaves the rest of Part~V}
