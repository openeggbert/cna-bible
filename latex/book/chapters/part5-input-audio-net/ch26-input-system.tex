...
\item[\texttt{ClickedEXT}] a \texttt{NOXNA System::MulticastAction<int>} event (Chapter~35
...
This chapter's own intro named \texttt{SetPosition}'s \emph{write}-direction letterbox scaling on \texttt{SDL\_RENDERER} (converting a logical position to real window coordinates before warping); a later audit pass specifically checked whether the \emph{read} direction --- an incoming \texttt{SDL\_EVENT\_MOUSE\_MOTION} event's real window coordinates converted back to logical ones --- was equally correct, since every prior test happened to use a 1:1 window with no backend scaling at all, never actually exercising that conversion branch. It was: a real window with a 100$\times$100 logical presentation letterboxed into a 200$\times$200 physical window (a 2$\times$ scale factor) confirms a window-space motion event at $(100, 100)$ reads back through \texttt{Mouse::GetState()} as logical $(50, 50)$, exactly mirroring the already-tested write direction. A read/write pair that only one direction had ever actually verified is exactly the kind of asymmetric test gap worth checking for deliberately, not assuming symmetric coverage from a single direction's passing test.
...
not a polled one: \texttt{TextInput} (a \texttt{NOXNA System::MulticastAction<charcs>} ---
Chapter~35 covers this \texttt{sharp-runtime} event type in full) fires once per composed
...
\texttt{StartTextInputWithTypeEXT(type)} is a \texttt{NOXNA} addition with no FNA analog at
...
\subsection{Worked examples: the NOXNA-only device subsystems}

The five subsystems named above share a common shape worth noting once: each is a small,
static, purely \texttt{NOXNA} class with no FNA or real-XNA precedent at all, since none of
