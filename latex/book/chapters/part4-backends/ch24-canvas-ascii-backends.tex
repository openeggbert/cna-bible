...
\texttt{true}. It therefore reports \texttt{ThreeD=true}, \texttt{CustomEffects=true}, and
\texttt{OcclusionQuery=true} even though the calls above prove that 3D operations throw, the
custom-effect factory returns \texttt{nullptr}, and the query factory forwards to SDL
renderer's throw. The internal
\texttt{SupportsDepthStencil()} check in the test is truthful; the public capability enum
currently is not for these three entries. Chapters~\ref{ch:graphicsdevice}
