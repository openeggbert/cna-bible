...
Every backend documented in the previous three chapters implements the full
\cnaclass{IGraphicsBackend} interface, 3D pipeline included, even where a given hardware
...
document (\texttt{plan\_ascii.md}, Phase~G7) records explicitly as ``same net behavior, less
code.'' \texttt{ascii\_throwno3d\_test.cpp} exists purely to prove this forwarding is complete
and correct, by calling each reachable 3D entry point on a real, live backend instance and
checking that it actually throws:
...
Seventeen checks in total, covering three distinct failure shapes rather than one uniform
rule: twelve directly-reachable entry points (the six \texttt{Clear*} overloads,
\texttt{SetDepthTestEnabled}, \texttt{SetBlendEnabled}, \texttt{SetDepthWriteEnabled},
\texttt{CreateVertexBuffer}, \texttt{CreateIndexBuffer16}, \texttt{CreateOcclusionQuery})
throw a real \texttt{std::runtime\_error}; \texttt{SupportsDepthStencil()} returns \texttt{false}
rather than throwing, since it is a capability query, not an action (the same ``ask, don't
catch'' shape Chapter~\ref{ch:sdl-renderer} documents for \texttt{SDL\_RENDERER}'s own
\texttt{SupportsCapability} overrides); and four factory methods
(\texttt{CreateTexture3D} / \texttt{CreateTextureCube} / \texttt{CreateRenderTargetCube}/
\texttt{CreateEffectBackend}) return \texttt{nullptr} rather than throwing at all, because
\texttt{AsciiGraphicsBackend} never overrides them --- they fall through to
\cnaclass{IGraphicsBackend}'s own shared defaults, which already return \texttt{nullptr}, so
adding a local override would only duplicate behavior the base class already provides for
free.

Three further methods --- \texttt{DrawColoredPrimitives}, \texttt{DrawIndexedColoredPrimitives},
\texttt{DrawInstancedPrimitivesEx} --- are deliberately \emph{not} exercised by this test at
all, and the test's own comments explain why honestly rather than silently skipping them:
each needs a real \texttt{IVertexBufferBackend\&} argument, but \texttt{CreateVertexBuffer}
and \texttt{CreateIndexBuffer16} already throw (Checks~J/K above), so no real vertex or index
...
confirmed correct by code review instead, since both forward to the same wrapped inner backend
with the identical one-line pattern as every other method in the file.
...
\texttt{ClearColorAndDepth} and its five siblings, the vertex/index buffer factories,
\texttt{DrawColoredPrimitives} --- throw via \texttt{ThrowNo3D()}, while the remaining
3D-only surface (\texttt{Draw*Ex} / \texttt{DrawInstancedPrimitivesEx},
\texttt{CreateIndexBuffer32}, and the same four \texttt{Create\{Texture3D, TextureCube,
RenderTargetCube, OcclusionQuery, EffectBackend\}} factories) is left entirely on
\cnaclass{IGraphicsBackend}'s own shared defaults, unoverridden. The header comment names its
own precedent directly: this is ``confirmed against \texttt{SDL\_RENDERER}'s own precedent of
