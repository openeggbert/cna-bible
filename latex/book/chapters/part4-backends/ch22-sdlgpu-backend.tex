...
\texttt{sdlgpu\_swapchain\_recovery\_test.cpp} proves that latter path without pretending a real
device loss is easy to reproduce. On frame 10 it calls SDL's own
\texttt{SDL\_ReleaseWindowFromGPUDevice()}, makes \texttt{Present()} fail, reclaims the identical
window with \texttt{SDL\_ClaimWindowForGPUDevice()}, and calls \texttt{Present()} again. The test
