...
6-array-layer \texttt{WGPUTexture} (the same layout \texttt{WebGPUTextureCubeBackend} already
uses for an upload-only \cnaclass{TextureCube}), one shared \texttt{size}$\times$\texttt{size}
...
upload-only \cnaclass{TextureCube} as \texttt{EnvironmentMapEffect.EnvironmentMap} worked, but
...
// WebGPUTextureCubeBackend (upload-only) and WebGPURenderTargetCubeBackend
