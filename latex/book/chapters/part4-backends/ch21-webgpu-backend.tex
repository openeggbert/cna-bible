...
\subsection{A worked example: \texttt{RenderTargetCube}, and the cast that used to silently
fail}
...
Both \texttt{WebGPUTextureCubeBackend} and \texttt{WebGPURenderTargetCubeBackend} now implement
\texttt{IWebGPUCubeSamplable} alongside their own primary interface, and the draw-dispatch code
resolves through it with a single \texttt{dynamic\_cast} rather than the old hardcoded
concrete-type cast:
