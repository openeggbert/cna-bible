...
\section{Sampling a render target: a wrong-handle-type cast, not a crash}
\label{sec:bgfx-wrong-handle-cast}

Chapter~\ref{ch:vulkan-backend} named this backend's own \cnaclass{RenderTargetCube}-sampling
bug as ``a genuine wrong-handle-cast, diagnosed precisely'' in contrast to Vulkan's own
still-unresolved equivalent --- this section is that precise diagnosis. Both
\texttt{BgfxSpriteBatchBackend::Draw} and \cnaclass{BgfxGraphicsBackend}'s own
\texttt{envMapping} branch (the code path \cnaclass{EnvironmentMapEffect}, per
Chapter~\ref{ch:stock-effects}, reaches) unconditionally \texttt{static\_cast} whatever
\cnaclass{ITextureBackend} / \cnaclass{ITextureCubeBackend} they are handed to the plain,
ordinary-texture concrete type. \cnaclass{RenderTarget2D} and \cnaclass{RenderTargetCube} are
each backed by their own, entirely separate sibling backend class --- consistent with
...
The reason this compiles cleanly and never crashes, rather than failing loudly the moment it is
exercised, is a genuinely instructive piece of type-confusion: bgfx represents both handle kinds
identically at the ABI level, as a bare \texttt{struct \{ uint16\_t idx; \}}. The cast is
structurally valid C\texttt{++} --- both types really do have that exact layout --- so the
compiler has no basis to reject it, and the resulting object does not crash on access, because
\texttt{idx} really is a valid, in-range integer. It is simply the \emph{wrong} integer: an
index into bgfx's framebuffer handle pool, read and used as though it indexed the texture handle
pool instead. The practical consequence is a render target that silently samples whatever
texture happens to occupy the coincidentally-matching slot in the wrong pool --- confirmed by
direct memory-layout analysis and by new smoke tests added specifically to prove the cast
\emph{does not crash} on either \cnaclass{RenderTarget2D} or \cnaclass{RenderTargetCube}, which
is exactly why ordinary ``does it crash'' testing had not caught this before. Not yet fixed
(Tasks~873/874) --- a render-to-texture technique (a portal, a security-camera view, a
reflection probe rendered as an ordinary 2D texture rather than through the dedicated cube-face
path) that samples its own render target back through \cnaclass{SpriteBatch} or
\cnaclass{EnvironmentMapEffect} on this backend should be treated as unverified until this is
resolved, even though nothing about running the code signals that anything went wrong.
