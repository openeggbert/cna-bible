...
correctly recompute their scale from the new logical dimensions, so after such a resize their
coordinate conversion and the physical presentation can disagree. Closing that gap needs
...
\section{Letterboxing is fixed, but input coordinates are not guessed}

The permanent output-mode limitation does \emph{not} leave input mapping ambiguous. The
backend implements both \texttt{TransformWindowToLogical()} and its inverse directly from the
real \texttt{SDL\_Window} size, rather than reaching into \texttt{free-direct}'s deliberately
private SDL renderer. For a logical size $(L_w,L_h)$ and physical window size $(P_w,P_h)$, it
recomputes the same centered-letterbox transform that \texttt{free-direct} uses:
...
manager. This is a useful separation of responsibilities: output policy is constrained by
\texttt{free-direct}, but coordinate conversion still has a concrete, tested contract for
input code.
