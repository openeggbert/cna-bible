...
\section{RenderTarget2D: a second surface, the same shadow-backbuffer trick}
...
\texttt{Dx3RenderTargetBackend} owns its own private, always-Lockable
\texttt{DDSCAPS\_OFFSCREENPLAIN} surface, defined (like \texttt{Dx3TextureBackend}) entirely
...
\section{Design decision 4, and the two real free-direct gaps it sidesteps entirely}
...
\begin{description}
  \item[The discarded \texttt{dwBPP} parameter.] \texttt{IDirectDraw::SetDisplayMode(width,
        height, dwBPP)} stores \texttt{width} / \texttt{height} but silently discards
        \texttt{dwBPP} --- \texttt{free-direct}'s own \texttt{CreateSurface} always allocates a
        ...
        would affect.
  \item[The mixed-depth \texttt{BlitFrom} silent-no-op.] \texttt{DirectDrawSurfaceImpl::BlitFrom}
        has real fast/slow paths only for same-depth 8-on-8 and 32-on-32 blits; a mismatched pair
