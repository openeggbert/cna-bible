...
that: its \texttt{Back}-face call to \texttt{set\_stencil\_func\_separate()} is passed the same
\texttt{stencilMask} local variable as the \texttt{Front}-face call, with no separate
...
device.set_stencil_func_separate(::easygl::CullFace::Back,
    ...
    referenceStencil, static_cast<unsigned int>(stencilMask));  // same stencilMask, not a
                                                                  // separate ccw one
\end{lstlisting}
