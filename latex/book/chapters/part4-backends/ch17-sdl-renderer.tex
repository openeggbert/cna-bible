...
construct.  CNA deliberately distinguishes a backend-independent description of work from
the first operation that asks this particular backend to execute that work.  A
...
backend allocation.  All four of its public constructor shapes therefore work on
...
This is not merely an API-design claim.  The backend's dedicated regression test constructs
...
asset code prepare vertex layouts without a false backend failure, while still refusing the
...
run \texttt{Apply()} on this backend.  They are data/configuration objects until a primitive
...
\cnaclass{DepthStencilState} assignments also round-trip as data, but their backend application
...
constructors immediately call their backend factories and fail before their own argument
validation runs.  On this backend even a zero or negative requested count therefore produces
...
\begin{itemize}
  ...
  \item \textbf{Presentation formats (stored, not selected).} Neither the constructor nor a
        ...
        window transition request before the backend's empty format hook.
...
the precise distinction the fourteen-product matrix in
...
pixel-verification test on this backend needs one specific presentation mode}
