...
\texttt{ALooper}, an event queue, rollback, and one dedicated polling thread.  CNA itself uses
no JNI.  Its only Android-specific link adds \texttt{android} and \texttt{log} to the Devices
module; the root CMake file contains no Android branch.  Portable sensor mathematics stays
outside \texttt{\_\_ANDROID\_\_} guards so native tests can exercise it, while the I/O boundary
is platform-gated.

\section{What actually ran}

Two emulator campaigns are recorded.  On 4~July~2026 an NDK-built \texttt{CnaTests} executable
was pushed to a Medium Phone AVD.  It ran 230 Net/Gamer/ENet/Packet cases plus 1,831 other cases;
surviving build-system repairs corroborate the run.  On 5~July the Devices application produced
a 7.3\,MB APK, installed and started it, captured the emulator screen, and injected sensor data
...
Android graphics is a separate and still open boundary.  The project records that it has never
attempted graphics execution on Android and does not even establish that the SDL renderer
compiles cleanly for \texttt{arm64-v8a}.  WebGPU is the only renderer containing an explicit
Android native-window surface path.  A Devices APK proving sensors must not be promoted into an
OpenGL, SDL-renderer, or WebGPU graphics claim.
...
Three tests share the word Metal.  \texttt{Metal\_PortableHelpers} is deliberately registered
on every host, whereas \texttt{Metal\_Smoke} and \texttt{Metal\_Capabilities} require a real
Metal configure.  Thus a Linux \texttt{ctest -R '\^Metal'} run can be entirely green while
executing no Objective-C\texttt{++}, Apple framework, shader compiler, or GPU command.  The
renderer identity of the build must accompany every quoted result.
...
An iOS platform enum exists, but renderer selection rejects iOS and tvOS.  The tree has no
Xcode project, application metadata, SDK-root configuration, signing path, or device/simulator
test.  Listing the enum as support would confuse a vocabulary token with a port.
