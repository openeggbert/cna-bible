...
CNA targets seven platform identities, but support is not a Boolean attached to each one.
Linux x86\_64 is the only self-hosting first-class target at the audited revision. Other paths
are cross-compiled and exercised through Wine, a browser, or an Android emulator, or have not
executed at all. The architectural contract is nevertheless coherent: SDL absorbs common host
services, while platform-only code is selected as CMake targets rather than scattered behind
