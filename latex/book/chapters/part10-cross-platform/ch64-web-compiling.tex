...
Two adjustments apply project-wide before any renderer-specific configuration happens.
...
cited depends on real unwinding working). The \texttt{WEBGL2} identity (using the shared EasyGL
implementation and a GLES~3.0 context) is the default renderer under Emscripten; Linux instead
defaults to \texttt{OPENGLES3} ---
...
renderer's own CMake test registration are excluded here, and a handful of process-spawning
...
renderer. The \texttt{CANVAS} renderer has a real, confirmed success story: a genuine
...
genuinely links and runs under \texttt{node}, with a real, renderer-agnostic GoogleTest suite
passing in full.

\begin{lstlisting}[style=cnashell]
# Worked example: the real invocation shape behind that CANVAS success
# story, following the same -DCNA_GRAPHICS_RENDERER convention this book's
...
The default \texttt{WEBGL2}/EasyGL path tells a different, more honest story: at the time of
the cited measurement, no Emscripten build directory had existed for \texttt{WEBGL2}, and no
automated workflow builds it under Emscripten either. Every specific claim in this project's
own Web/Emscripten graphics-limitations documentation about EasyGL behavior under
...
Crucially, this build machinery is entirely renderer-agnostic: \texttt{ThirdPartySDL.cmake}
branches only on platform (\texttt{EMSCRIPTEN} / \texttt{ANDROID} / \texttt{WIN32} / \texttt{APPLE}/
plain Unix), never on \texttt{CNA\_GRAPHICS\_RENDERER}. That means the SDL3-for-Emscripten cache
...
which is exactly why \texttt{docs/canvas-renderer.md}'s own manual browser verification
...
has never been exercised against a real browser: \texttt{EasyGL}'s own renderer implementation
...
\texttt{EasyGLRenderer.cpp} defines two \texttt{EM\_JS} blocks --- real, inline
...
installed once at renderer construction, is what turns the resulting
...
\texttt{InstallEmscripten\allowbreak{}ContextLoss\allowbreak{}Callbacks()}'' --- the call to
...
The public entry point is less isolated than those renderer methods suggest. Every
\cnaclass{Game} sends each SDL event through the ordinary input bridge first and then reserves
non-repeated F9 for the loss request and F10 for restore. On Web those are genuinely separate,
asynchronous browser operations; on desktop EasyGL either key runs the whole synchronous
cycle. Calling \texttt{SetContextRecoveryEnabled(false)} after device construction but before
content creation also prevents future EasyGL resources from joining the recovery registry
and permits uploaded \cnaclass{Texture2D} objects to discard their CPU shadow. A later
simulated loss cannot reconstruct those resources. This browser branch remains source-audited
rather than executed in a real browser; the complete product/evidence matrix is in
...
Reading \texttt{modules/graphics/examples/CMakeLists.txt} directly turns up a genuine, previously-undocumented gap
...
the sense that the renderer most users would actually reach for (\texttt{EasyGL}/WebGL2) has
