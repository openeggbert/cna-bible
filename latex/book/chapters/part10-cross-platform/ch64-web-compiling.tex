...
C\texttt{++}23 framework through Emscripten, link it with SDL's browser port, and package one
of five renderer identities into a WebAssembly application.  That compact description hides
three independent questions: can the source be translated, can the program survive the
browser main-loop hand-off, and has the selected renderer actually been exercised?  This
chapter answers only the first.  Chapters~\ref{ch:web-main-loop} and
