...
Volume~I's ecosystem chapter introduced \texttt{sharp-runtime} as the foundation layer every
...
Chapter~\ref{ch:design-philosophy} in Volume~I already introduced (\texttt{bytecs},
...
specifically to model a real XNA quirk this book has already touched on in Volume~II ---
...
than spawning an extra OS thread per continuation. \cnaclass{Threading::Thread} is a thin
wrapper over \texttt{std::thread} that deliberately does not start at construction --- a
second call to \texttt{Start()} throws \texttt{ThreadStateException}, matching real .NET's own
one-shot-start contract exactly, rather than silently allowing a thread object to be
restarted the way a naive wrapper might.
