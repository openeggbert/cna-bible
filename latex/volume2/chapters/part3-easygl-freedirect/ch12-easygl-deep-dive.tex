...
Volume~I introduced \texttt{easy-gl} as the OpenGL/OpenGLES wrapper behind CNA's
\texttt{EASYGL} backend. This chapter goes one level deeper --- into its own layering, its
capability-detection mechanism, and a real dependency Volume~I's overview did not have room
to mention.

\section{A layer Volume~I did not cover: meta-gl}
...
Chapter~\ref{ch:easygl-backend} in Volume~I described CNA's own \texttt{EASYGL} backend
...
Two gaps are worth knowing about directly rather than assuming closed. First,
\texttt{Config::enable\_debug\_logging} currently does nothing at all --- no
