...
The Avatar system is the strangest corner of CNA, and deliberately so. Volume~I's opening
chapter already flagged it: unlike every other namespace in this book, Avatar was ported from
...
Chapter~1 of Volume~I asked this book to preserve rather than round off into a vague ``Avatar
support is limited.''
...
specifically because \cnaclass{Model}/\cnaclass{ModelMesh} (Volume~I's models-and-meshes
chapter) encode a rigid, per-mesh parent-bone hierarchy --- the wrong shape entirely for
...
\cnaclass{SkinnedEffect}'s (Volume~I's stock-effects chapter) 72-bone cap, and the
...
already covered in Volume~I's backend chapters, consistent with that backend's documented
