...
\texttt{FrameworkDispatcher::Update()} (Volume~I's game-loop chapter) drives
...
Volume~I's game-loop chapter), and \textbf{NOXNA} (a CNA-only addition with no FNA counterpart
...
Volume~I's per-backend presentation differences (its backend-architecture chapter).
...
\texttt{NOXNA} subsystems already named in passing in Volume~I's overview chapter:
