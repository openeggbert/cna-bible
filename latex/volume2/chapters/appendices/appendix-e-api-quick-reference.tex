...
Volume~I's own Appendix~A did the same job for the core framework and graphics classes; this
appendix extends that lookup companion to the namespaces this volume covers --- Input, Audio,
Media, Networking, GamerServices, and a handful of \repolink{sharp-runtime} core types every
other chapter in this volume leans on. As in Volume~I's appendix, every entry here was read
directly from the corresponding header at the time of writing, and this is a summary of classes
already given full narrative treatment elsewhere in this volume, not a substitute for reading
that treatment.
...
and a \texttt{Begin}/\texttt{End} fake-async pair, per the pattern Chapter~8 of this volume
introduces for \cnaclass{Storage}), four gamer collections (\texttt{AllGamers}/%
...
deliberate, documented exception to this volume's ``restore what FNA stubbed out'' pattern ---
Xbox~360 achievement artwork was streamed from Xbox LIVE at request time with no real local
equivalent to substitute, so it stays throwing by design, the same reasoning Chapter~5's
...
unit, and a full arithmetic/comparison operator set --- the type Chapter~6 of Volume~I's
\cnaclass{GameTime} and this volume's \cnaclass{Storage}, \cnaclass{NetworkSession}, and
...
streams \cnaclass{StorageContainer::OpenFile} in Chapter~8 returns) derives from.

\section{Reading this appendix safely}

The same caution Volume~I's own API appendix states applies here without modification: every
