...
The latest full-book rebuild includes the `RenderTargetCube` upload and target-transition
contract correction. `main.pdf` is 484 pages; the targeted undefined-reference and
duplicate-label checks are empty; makeindex accepted 2,015 entries with no rejection or
warning. The affected ranges were rendered and inspected: Ch.11 physical page 141, Ch.16
pages 185--188, Ch.23 pages 254--257, and Appendix A page 449. The new transition matrix
repeats its header cleanly across two pages without clipping or cell collision; no changed
page has a page-edge spill, malformed heading, running-header collision, or folio defect.
`NEXT.md` records the source findings, commands, and unpushed local commit state.
