...
The latest forced full-book rebuild includes the buffer streaming/readback/vertex-layout
contract correction. `main.pdf` is 478 pages; the targeted undefined-reference and
duplicate-label checks are empty; makeindex accepted 1,983 entries with no rejection or
warning. The final affected ranges were rendered and inspected: Ch.11 physical pages
143--144, Ch.15 pages 175--176, Ch.16 pages 181--183, Ch.18 pages 199--200, and Appendix A
page 446. The new two-page buffer table repeats its header correctly and has no clipping or
cell collision; no changed page has a page-edge spill, malformed table, running-header, or
folio defect. `NEXT.md` records the source findings, commands, and unpushed local commit state.
