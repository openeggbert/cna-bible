...
ContentTypeReader, and XNB header/decompression audits.
`main.pdf` is 541 pages; the targeted undefined-reference and duplicate-label checks are empty;
makeindex accepted 2,200 entries with no
rejection or warning.
The current ContentReader/ContentTypeReader/XNB-container pages 105--108, together with the
...
| 8 | Content and Assets | 1,261 (was 242) | 55 | The current twenty-pass audit covers XNB/CNJ readers, exact resolution and cache semantics, error/type/concurrency boundaries, manifests, service/device requirements, and ResourceContentManager's explicit embedded-resource stub. The latest pass distinguishes header parsing from admission control: permissive under-declared lengths, unsafe short LZX prefix, post-allocation-only LZX output check, unbounded whole-file paths, default-only manager limits, FNA divergence, and focused test gaps; three declared XNB limits remain inactive on `develop`. | **In progress (~27 of ~55 pages, PDF-verified clean)** |
