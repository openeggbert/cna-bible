...
ResourceContentManager/OpenStream, TitleContainer/TitleLocation, and ContentReader audits.
`main.pdf` is 539 pages; the targeted undefined-reference and duplicate-label checks are empty;
makeindex accepted 2,188 entries with no
rejection or warning.
The current ContentReader pages 105--106, together with the previously checked pages 50, 58--59,
60, 63--64, 103--118, 124--125, and 126--127, were rendered and inspected.
...
| 8 | Content and Assets | 1,127 (was 242) | 55 | The current eighteen-pass audit covers XNB/CNJ readers, exact resolution and cache semantics, error/type/concurrency boundaries, manifests, service/device requirements, and ResourceContentManager's explicit embedded-resource stub. The latest pass separates ContentReader's one-shot body session, raw stream/manager lifetime, reader-table initialization, shared fixups, disposal callback gap, external-reference instantiation, partial limits, FNA divergence, and focused test gaps; three declared XNB limits remain inactive on `develop`. | **In progress (~24 of ~55 pages, PDF-verified clean)** |
