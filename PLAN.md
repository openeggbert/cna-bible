...
| 2 | Ecosystem Map | 155 (was 141) | 25 | Fixed three stale "Chapter references in Part~N" placeholders into real \ref{}s (Ch.33/34/35, Ch.36, Ch.44); added easy-gl's own scope-vs-CNA's-EASYGL-backend distinction and free-direct's real split-by-transport test-suite detail (ENet 1/1, loopback 7/8-by-design) | **In progress (~4 of ~25 pages, PDF-verified clean)** |
| 3 | Design Philosophy | 289 (was 256) | 30 | Added a real NOXNA-discipline worked example (the self-verifying `cna_strict_xna_api_check`/`cna_strict_xna_api_leak_check` CTest pair, incl. the deliberate-violation source file); corrected an imprecise "NOXNA build option" claim (also fixed in Ch.1) to the real mechanism (`CNA_STRICT_XNA_API` define, two CMake targets); fixed a pre-existing long-path overfull-hbox defect | **In progress (~6 of ~30 pages, PDF-verified clean)** |
| 4 | Building CNA | 229 (was 173) | 35 | Added a full top-level CMake option reference table (7 options beyond `-DCNA_GRAPHICS_BACKEND`, read from `CMakeLists.txt`), incl. a worked clarification that `CNA_NOXNA` (a real, unrelated CMake option gating an opt-in extended render pipeline) is a coincidental name collision with the `NOXNA` marker macro from Ch.3 | **In progress (~4 of ~35 pages, PDF-verified clean)** |
| 5 | First Game | 213 (was 131) | 40 | Added a second complete worked example (frame-rate-independent keyboard-driven sprite movement, grounded in the real `Keyboard`/`KeyboardState`/`TimeSpan` headers), incl. a real finding that the first program's `Draw(texture, float, float)` overload is `NOXNA`-only and the real XNA overload takes a `Vector2` | **In progress (~4 of ~40 pages, PDF-verified clean)** |
| 6 | Game Loop | 274 (was 216) | 40 | Added two worked examples grounded in real headers/tests: `GameServiceContainer` register/consume (incl. the two real `std::invalid_argument` findings for null/duplicate registration) and `GraphicsDeviceManager`'s `PresentationMode::Letterbox` setup; fixed a pre-existing running-header overflow (long section title colliding with the folio number) via a short `\section[...]` form | **In progress (~6 of ~40 pages, PDF-verified clean)** |
...
| 8 | Content and Assets | 191 (was 125) | 55 | Added a `ContentManager` constructor/method reference plus a worked example using the real `GetContentManifest()`/`ContentManifestEntry` introspection API to show the resolution order concretely; fixed four stale plain-text `Chapter~N` citations (incl. one that pointed at the wrong chapter) into real `\ref{}`s | **In progress (~4 of ~55 pages, PDF-verified clean)** |
...
| 10 | SpriteBatch | 204 (was 131) | 55 | Added a `Draw` overload-family reference table (longtable, wraps correctly) and two worked examples (rotate-around-center via `origin`, tied to the SDL\_RENDERER pivot bug already covered; `MeasureString`-based text centering); fixed a pre-existing overfull-hbox (`RasterizerState::CullCounterClockwise`) | **In progress (~4 of ~55 pages, PDF-verified clean)** |
| 11 | Textures and Render Targets | 244 (was 207) | 60 | Added a worked `Texture2D` example (procedural checkerboard via `SetData`/`GetData`), incl. a real finding that `SetData`/`GetData` have no generic-template overload unlike real XNA's `T[]` form; fixed a pre-existing overfull-hbox in the `VertexBuffer` struct list | **In progress (~6 of ~60 pages, PDF-verified clean)** |
| 12 | Models and Meshes | 190 (was 103) | 60 | Added two worked examples: the classic per-mesh bone-transform draw loop (grounded in real `CopyAbsoluteBoneTransformsTo`/`IEffectMatrices` signatures, tied to the two-loaders `ParentBone` gap), and an `AnimationPlayer` example adapted directly from its own two-bone test fixture, incl. the `GetWorldTransforms` vs `GetSkinTransforms` distinction verified against the real `.cpp` | **In progress (~4 of ~60 pages, PDF-verified clean)** |
| 13 | Stock Effects | 227 (was 191) | 80 | Added a worked `BasicEffect` example (lit, textured cube), incl. a real `EnableDefaultLighting()`-ordering hazard verified against its own `.cpp` implementation (it unconditionally overwrites `DirectionalLight0-2` every call) | **In progress (~6 of ~80 pages, PDF-verified clean)** |
| 14 | State Objects | 191 (was 159) | 55 | Added a worked example making the chapter's own by-value-not-by-reference architectural finding concrete: mutating a `BlendState` after assigning it to the device has no effect, mutating through `getBlendStateProperty()` does, verified against the real `setBlendStateProperty(const BlendState&)` signature | **In progress (~4 of ~55 pages, PDF-verified clean)** |
| 15 | Shader/.fx Gap | 133 (was 78) | 30 | Corrected a real, significant inaccuracy: the chapter previously said `ShaderEffect` exists only on the three Direct3D backends with HLSL source, but `CreateEffectBackend()` is real on EasyGL/Vulkan/BGFX/SdlGpu too (EasyGL alone has 38 real tests, GLSL source) — rewrote both the main section and "honest summary" to match; added a worked GLSL example adapted from a real EasyGL test | **In progress (~4 of ~30 pages, PDF-verified clean)** |
| 16 | Backend Architecture | 174 (was 133) | 40 | Added an `ISpriteBatchBackend` deep-dive tying Ch.10's SDL_RENDERER transform-matrix bug to its real no-op default (and a new finding: no `GraphicsCapability` value exists to ask about it), plus a worked `SupportsCapability` example; fixed three pre-existing overfull-hboxes | **In progress (~4 of ~40 pages, PDF-verified clean)** |
| 17 | SDL_Renderer Backend | 151 (was 115) | 55 | Added a worked example showing this backend's real `SupportsCapability` override (unconditional `false` for all eight values, verified against the real header) as the "ask, don't catch" pattern in concrete use, contrasted with Ch.16's `MultipleRenderTargets` finding | **In progress (~4 of ~55 pages, PDF-verified clean)** |
...
| 24 | DX3/free-direct Backend | 88 (was 65) | 30 | Fixed a stale plain-text cross-reference ("Chapters~6 and~5") to real `\ref{}`s; added a worked example with real `BlendState::AlphaBlend` factors (verified against `BlendState.cpp`) showing why the bug is visible for `AlphaBlend` but was harmless for `Opaque`'s own degenerate factors — a math error caught and corrected in this session's own draft before commit | **In progress (~2 of ~30 pages, PDF-verified clean)** |
| A | API Quick Reference | 272 (was 229) | 40 | Added the previously-missing `GraphicsDevice` and four state-object classes as a new section (grouped by real property/method shape, read directly from `GraphicsDevice.hpp`), the most glaring omission from an appendix otherwise covering every other Part II/III class; fixed three pre-existing overfull-hboxes | **In progress (~6 of ~40 pages, PDF-verified clean)** |
