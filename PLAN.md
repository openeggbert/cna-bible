...
**Status: Phase 1 execution is active. The current book is 505 physical PDF pages
...
The latest full-book rebuild includes the presentation-format/fullscreen/default audit.
`main.pdf` is 505 pages; the targeted undefined-reference and duplicate-label checks are
empty; makeindex accepted 2,069 entries with no rejection or warning. The affected ranges
were rendered and inspected: Ch.6 physical page 64, Ch.9 page 113, Ch.16 pages 190--192,
Ch.17 page 209, Ch.18 page 216, Ch.19 page 227, Ch.20 page 233, Ch.21 page 243, Ch.22 page
252, Ch.23 page 264, Ch.25 page 287, and Appendix A page 470. The new backend matrix repeats
its header cleanly on both continuation pages; no changed page has clipping, cell collision,
page-edge spill, malformed heading, running-header collision, or folio defect. All overfull
warnings introduced by the batch were removed before the final build. `NEXT.md` records the
source findings, commands, and unpushed local commit state.
...
| 6 | Game Loop | 496 (was 216) | 40 | Covers GameServiceContainer, PresentationMode, a full DrawableGameComponent example, fixed-timestep catch-up and `IsRunningSlowly` hysteresis, and the default-fixed-timestep `ResetElapsedTime()` no-op. Recent presentation passes separate timestep scheduling from VSync blocking and explain why the manager's stored backbuffer/depth/fullscreen preferences do not uniformly describe native attachments or an accepted window transition. | **In progress (~10 of ~40 pages, PDF-verified clean)** |
...
| 9 | GraphicsDevice | 1156 (was 966) | 90 | Full method/lifecycle coverage, adapter and format-query limits, draw-matrix extraction, render-target/MRT and capability findings, plus corrected stencil/depth-clear history. Recent passes correct `GetBackBufferData`'s four backend-dependent result classes and raw-pointer validation gaps, then establish that round-tripped interval, backbuffer/depth formats, and fullscreen fields are requested state rather than native attachment/window queries. | **In progress (~31 of ~90 pages, PDF-verified clean)** |
