...
**Status: Phase 1 execution is active. The current book is 502 physical PDF pages
...
### Current verification checkpoint (2026-07-25)

The latest full-book rebuild includes the swap-interval/default audit.
`main.pdf` is 502 pages; the targeted undefined-reference and duplicate-label checks are
empty; makeindex accepted 2,056 entries with no rejection or warning. The affected ranges
were rendered and inspected: Ch.6 physical page 64, Ch.9 pages 112--113, Ch.16 pages
188--190, Ch.17 page 207, Ch.19 page 226, Ch.23 pages 261--262, and Appendix A page 467.
The new backend matrix repeats its header cleanly after the page break; no changed page has
clipping, cell collision, page-edge spill, malformed heading, running-header collision, or
folio defect. All overfull warnings introduced by the batch were removed before the final
build. `NEXT.md` records the source findings, commands, and unpushed local commit state.
...
| 9 | GraphicsDevice | 1141 (was 966) | 90 | Full method/lifecycle coverage, adapter and format-query limits, draw-matrix extraction, render-target/MRT and capability findings, plus corrected stencil/depth-clear history. Recent passes correct `GetBackBufferData`'s four backend-dependent result classes and raw-pointer validation gaps, then establish that a round-tripped `PresentationInterval` is only requested state: the backend hook may ignore or narrow it and no public capability/query reports the active interval. | **In progress (~31 of ~90 pages, PDF-verified clean)** |
...
current fully rebuilt total of 496 physical PDF pages.
