...
| 16 | Backend Architecture | 1905 (was 222) | 40 | Deep dives now cover SpriteBatch, render targets, effects, occlusion queries, index/vertex buffers, texture/readback/update defaults, input-coordinate routing, swap intervals, presentation formats/fullscreen, recovery/debug hooks, and build-time selection. The latest fourteen-product matrix separates shared ClearOptions routing from immediate, deferred, 2D-masked, software, and synthetic execution models, including per-format and cube-target masking defects. A new generic resource-lifetime audit distinguishes public handle ownership from GraphicsDevice's raw address tracker: normal device-first disposal is ordered safely, but copy/move constructors do not transfer tracker entries. `Texture2D` copies share a backend while remaining untracked, and movable resources leave their old address registered; device-first teardown can therefore retain and later destroy a native backend after its context. The source-repository fix is explicitly `needs_human`/out of this book checkout's scope. | **In progress (32 physical pages, PDF-verified clean, ~32 of ~40 target pages)** |
