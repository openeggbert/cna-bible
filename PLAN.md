...
**Status: Phase 1 execution is active. The current book is 496 physical PDF pages
...
The latest full-book rebuild includes the backbuffer-readback/default audit.
`main.pdf` is 496 pages; the targeted undefined-reference and duplicate-label checks are
empty; makeindex accepted 2,039 entries with no rejection or warning. The affected ranges
were rendered and inspected: Ch.9 physical pages 111--112 and 117, Ch.16 pages 186--188,
Ch.22 pages 251--252, Ch.23 page 259, and Appendix A page 461. The new backend matrix
repeats its header cleanly across both page breaks; no changed page has clipping, cell collision, page-edge spill,
...
| 9 | GraphicsDevice | 1126 (was 966) | 90 | Full method/lifecycle coverage, adapter and format-query limits, draw-matrix extraction, render-target/MRT and capability findings, plus corrected stencil/depth-clear history. The latest pass corrects `GetBackBufferData`: its bound-target worked probe is Software-specific, the method has four backend-dependent result classes rather than one portable backbuffer contract, and the raw-pointer wrapper omits offset/capacity/rectangle/overflow validation. | **In progress (~30 of ~90 pages, PDF-verified clean)** |
...
| 16 | Backend Architecture | 1066 (was 222) | 40 | Deep dives now cover SpriteBatch, render targets, effects, occlusion queries, index/vertex buffers, texture/readback/update defaults, input-coordinate routing, and build-time selection. The latest fourteen-product readback matrix separates current-target, actual-backbuffer, synthetic-clear, and inherited-throw contracts; records logical/physical coordinate mismatches, per-backend region rules, and the actual assertion layer behind each claim. | **In progress (19 physical pages, PDF-verified clean, ~19 of ~40 target pages)** |
...
| 23 | Direct3D Backends | 749 (was 268) | 85 | Covers the D3D9 XNA oracle and native limits; D3D11 lifetime, draw-proof, MRT finalization, and presentation behavior; and D3D12 headless testing, synchronization, resource budgets, MSAA, and mip-generation architecture. The latest correction separates D3D11's real public backbuffer staging path from D3D12's direct render-target-resource proofs: D3D12 inherits the public default throw and its headless smoke suite cannot prove a swapchain bridge. | **In progress (15 physical pages, PDF-verified clean, ~15 of ~85 target pages)** |
...
| A | API Quick Reference | 369 (was 229) | 40 | Covers `GraphicsDevice`, state objects, buffers, effects, occlusion queries, `SpriteFont`, and core model types. The latest concise warning adds `GetBackBufferData`'s four result classes and missing raw-pointer bounds validation alongside the existing instancing, Texture2D shadow, and mip caveats. | **In progress (~9 of ~40 pages, PDF-verified clean)** |
