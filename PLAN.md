...
Decimal/DateTime-reader, CurveReader, Texture2DReader, and Texture3DReader audits.
`main.pdf` is 547 pages; the targeted undefined-reference and duplicate-label checks are empty;
makeindex accepted 2,237 entries with no
rejection or warning.
The current ContentReader/ContentTypeReader/XNB-container/type-name/scalar/Decimal/Curve/Texture2D/Texture3D pages 105--116, together with the
...
| 8 | Content and Assets | 1,661 (was 242) | 55 | The current twenty-six-pass audit covers XNB/CNJ readers, exact resolution and cache semantics, error/type/concurrency boundaries, manifests, service/device requirements, and ResourceContentManager's explicit embedded-resource stub. The latest Texture3DReader pass distinguishes the current fixed per-slice DXT decoding and raw-Color byte check from unchecked signed arithmetic, unbounded mip topology, unsupported formats, missing generic 3D capability admission, and the absent real XNB fixture; its checked arithmetic is only on unmerged `feature/audit`. The preceding Texture2DReader pass separates its exact raw-Color byte check and DXT software path from missing mip-topology/device-axis admission, raw signed-multiply overflow, unsupported formats, and absent Xbox byte swaps. Earlier passes confirm CurveReader's unchecked loop/continuity casts, unvalidated float keys, copied-and-appended direct existing-instance behavior, no checked-in XNB Curve fixture, and absent key-count limit; identify Decimal's MSVC absence, reserved-flag/signed-zero acceptance divergence, and DateTime Kind loss/invalid-kind acceptance; map 26 scalar/math readers' literal layouts and enum gap; and establish canonical factory-key/table-admission and container hardening gaps. | **In progress (~31 of ~55 pages, PDF-verified clean)** |
