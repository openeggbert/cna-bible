...
**Status: Phase 1 execution is active. The current book is 468 physical PDF pages
...
The forced full-book rebuild that had been pending since commit `87fb144` now completes.
`main.pdf` is 468 pages; the targeted undefined-reference and duplicate-label checks are empty;
makeindex accepted 1,953 entries with no rejection or warning. Every page in Ch.20/21/22/23/25/
34/35 and Appendices E/F was rendered and inspected from that full PDF, as was the previously
mislabelled pending Ch.5. Those files are now full-book PDF-verified clean, not merely
isolated-verified. `NEXT.md` records the exact physical ranges and commands.
...
| 23 | Direct3D Backends | 547 (was 268) | 85 | Covers the D3D9 XNA oracle and native limits; D3D11 lifetime, draw-proof, MRT finalization, and presentation behavior; and D3D12 headless testing, synchronization, resource budgets, MSAA, and mip-generation architecture. Latest D3D9 Texture3D and render-target claims were corrected against the live smoke tests. | **In progress (~12 of ~85 physical pages, PDF-verified clean)** |
...
current fully rebuilt total of 468 physical PDF pages.
