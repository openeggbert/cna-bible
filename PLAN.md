...
| 8 | Content and Assets | 1,494 (was 242) | 55 | The current twenty-four-pass audit covers XNB/CNJ readers, exact resolution and cache semantics, error/type/concurrency boundaries, manifests, service/device requirements, and ResourceContentManager's explicit embedded-resource stub. The latest CurveReader pass confirms FNA field order but exposes unchecked loop/continuity casts, unvalidated float keys, copied-and-appended direct existing-instance behavior, no checked-in XNB Curve fixture, and the absent key-count limit (negative means empty; large grows to exhaustion); the sole hardening observation remains only on unmerged `feature/audit`. The preceding pass identifies Decimal's MSVC absence, reserved-flag/signed-zero acceptance divergence, and DateTime Kind loss/invalid-kind acceptance; TimeSpan remains a literal tick reader. Earlier passes map 26 scalar/math readers' literal layouts and enum gap, canonical factory-key/table-admission limits, and container hardening gaps. | **In progress (~27 of ~55 pages, PDF-verified clean)** |
