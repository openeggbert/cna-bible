...
**Status: Phase 1 execution is active. The current book is 547 physical PDF pages
...
Decimal/DateTime-reader, CurveReader, and Texture2DReader audits.
`main.pdf` is 547 pages; the targeted undefined-reference and duplicate-label checks are empty;
makeindex accepted 2,231 entries with no
rejection or warning.
The current ContentReader/ContentTypeReader/XNB-container/type-name/scalar/Decimal/Curve/Texture2D pages 105--114, together with the
...
| 8 | Content and Assets | 1,582 (was 242) | 55 | The current twenty-five-pass audit covers XNB/CNJ readers, exact resolution and cache semantics, error/type/concurrency boundaries, manifests, service/device requirements, and ResourceContentManager's explicit embedded-resource stub. The latest Texture2DReader pass separates its exact raw-Color byte check and DXT software path from missing mip-topology/device-axis admission, raw signed-multiply overflow, unsupported formats, and absent Xbox byte swaps; the proposed stricter checks are only on unmerged `feature/audit`. The preceding CurveReader pass confirms FNA field order but exposes unchecked loop/continuity casts, unvalidated float keys, copied-and-appended direct existing-instance behavior, no checked-in XNB Curve fixture, and the absent key-count limit (negative means empty; large grows to exhaustion); the sole hardening observation remains only on unmerged `feature/audit`. Earlier passes identify Decimal's MSVC absence, reserved-flag/signed-zero acceptance divergence, and DateTime Kind loss/invalid-kind acceptance; TimeSpan remains a literal tick reader; map 26 scalar/math readers' literal layouts and enum gap; and establish canonical factory-key/table-admission and container hardening gaps. | **In progress (~29 of ~55 pages, PDF-verified clean)** |
