...
audit. `main.pdf` is 519 pages; the targeted undefined-reference and duplicate-label checks
are empty; makeindex accepted 2,119 entries with no rejection or warning. The affected physical pages
114--115, 149, 205--207, 219, 226, 236, 243, 257, 265--266, 283--284, 293, 299, 481--482, and
489 were rendered and inspected. The fourteen-product matrix repeats its header correctly
across both continuation pages. No changed page has clipping, cell collision, page-edge spill,
malformed heading, running-header collision, or folio defect. A full reference build confirms
that the complete 492-warning pre-existing overfull set is unchanged; all eight warnings
introduced while drafting this batch were removed.
...
| 9 | GraphicsDevice | 1294 (was 966) | 90 | Full method/lifecycle coverage, adapter and format-query limits, draw-matrix extraction, render-target/MRT and capability findings, plus corrected stencil/depth-clear history. Recent passes establish requested-vs-native presentation state and correct the three recovery/debug extensions: recovery changes shared Texture2D shadow lifetime even behind an empty hook, Vulkan alone emits markers, and D3D9 throws for both policy and marker calls after the policy call has already changed shared state. The latest pass distinguishes the four Clear overloads, FNA-style attachment masking, the shared depth-format/cube gaps, fourteen backend execution models, and Reset's old-state `DeviceResetting` observation. | **In progress (24 of ~90 printed pages, PDF-verified clean)** |
