...
**Status: Phase 1 execution is active. The current book is 484 physical PDF pages
...
The latest full-book rebuild includes the `Texture2D` update-default and mip-evidence
correction. `main.pdf` remains 484 pages; the targeted undefined-reference and
duplicate-label checks are empty; makeindex accepted 2,018 entries with no rejection or
warning. The affected ranges were rendered and inspected: Ch.11 physical pages 139--140,
Ch.16 pages 184--187, Ch.19 page 212, and Appendix A page 449. The new backend matrix fits
cleanly without clipping or cell collision; no changed page has a page-edge spill, malformed
heading, running-header collision, or folio defect. The one local Ch.19 overfull warning
found on the first build was removed before the final build. `NEXT.md` records the source
...
| 20 | BGFX Backend | 364 (was 149) | 55 | Covers renderer selection, render-target view allocation, the fixed handle-cast bug, instancing limits, and Task 914's real Texture3D/TextureCube readback and mip tests. Historical standalone warnings were rechecked by rendering every chapter page in the full book. | **In progress (~8 of ~55 physical pages, PDF-verified clean)** |
...
| 22 | SDL GPU Backend | 492 (new chapter) | 45 | Added by explicit author decision in 2026-07-21. Covers shader binding/runtime compilation, Y-flip and uniform-limit fixes, resource lifetimes, present/recovery behavior, validation, custom effects, pipeline/sampler state, 2D/3D ordering, real two-output MRT, and seven precisely scoped remaining limits. | **In progress (~10 of ~45 physical pages, PDF-verified clean)** |
