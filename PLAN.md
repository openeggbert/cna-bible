...
**Status: Phase 1 execution is active. The current book is 551 physical PDF pages
...
Decimal/DateTime-reader, CurveReader, Texture2DReader, Texture3DReader, TextureCubeReader, and
SpriteFontReader audits.
`main.pdf` is 551 pages; the targeted undefined-reference and duplicate-label checks are empty;
makeindex accepted 2,243 entries with no
rejection or warning.
The current ContentReader/ContentTypeReader/XNB-container/type-name/scalar/Decimal/Curve/Texture2D/Texture3D/TextureCube/SpriteFont pages 105--119, together with the
...
| 8 | Content and Assets | 1,806 (was 242) | 55 | The current twenty-eight-pass audit covers XNB/CNJ readers, exact resolution and cache semantics, error/type/concurrency boundaries, manifests, service/device requirements, and ResourceContentManager's explicit embedded-resource stub. The latest SpriteFontReader pass records its real DXT3 and LZX fixtures, but also its unchecked parallel glyph tables and absent-default-character end dereference; validation exists only on unmerged `feature/audit`. The preceding TextureCubeReader pass records the actual six-face DXT1 fixture and face/mip order, but also the current raw-Color byte-count out-of-bounds read, unbounded aggregate cube data and level topology, absent generic cube-backend admission, and moved-from direct existing-instance behavior. Earlier passes distinguish Texture3D's repaired per-slice DXT decoding from unchecked signed arithmetic and absent real XNB coverage; confirm CurveReader's unchecked loop/continuity casts, unvalidated float keys, copied-and-appended direct existing-instance behavior, no checked-in XNB Curve fixture, and absent key-count limit; identify Decimal's MSVC absence, reserved-flag/signed-zero acceptance divergence, and DateTime Kind loss/invalid-kind acceptance; map 26 scalar/math readers' literal layouts and enum gap; and establish canonical factory-key/table-admission and container hardening gaps. | **In progress (~35 of ~55 pages, PDF-verified clean)** |
