...
**Status: Phase 1 execution is active. The current book is 543 physical PDF pages
...
Decimal/DateTime-reader audits.
`main.pdf` is 543 pages; the targeted undefined-reference and duplicate-label checks are empty;
makeindex accepted 2,217 entries with no
rejection or warning.
The current ContentReader/ContentTypeReader/XNB-container/type-name/scalar/Decimal pages 105--112, together with the
...
| 8 | Content and Assets | 1,424 (was 242) | 55 | The current twenty-three-pass audit covers XNB/CNJ readers, exact resolution and cache semantics, error/type/concurrency boundaries, manifests, service/device requirements, and ResourceContentManager's explicit embedded-resource stub. The latest pass identifies Decimal's MSVC absence, reserved-flag/signed-zero acceptance divergence, and DateTime Kind loss/invalid-kind acceptance; TimeSpan remains a literal tick reader. The preceding pass maps 26 scalar/math readers' literal little-endian/field layouts, version-zero policy, weak direct-only test matrix, and absent generic EnumReader fallback; it also records raw string/Char encoding and validation boundaries. The previous pass distinguishes canonical factory keys from validated .NET names: assembly metadata is ignored recursively, while full structural consumption, base/arity/duplicate validation, per-name byte bounds, and generic nesting limits are absent; only limited bracket errors become ContentLoadException. The container pass established permissive under-declared lengths, unsafe short LZX prefix, post-allocation-only LZX output check, unbounded whole-file paths, default-only manager limits, FNA divergence, and focused test gaps; three declared XNB limits remain inactive on `develop`. | **In progress (~27 of ~55 pages, PDF-verified clean)** |
