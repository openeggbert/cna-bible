...
**Status: Phase 1 execution is active. The current book is 513 physical PDF pages
...
The latest full-book rebuild includes the public `GraphicsDevice::Clear` / `ClearOptions`
audit. `main.pdf` is 513 pages; the targeted undefined-reference and
duplicate-label checks are empty; makeindex accepted 2,103 entries with no rejection or
warning. The affected physical pages 112--113, 127--128, 147--148, 171, 203--205, 217, 227,
231--232, 239, 253--254, 264--265, 274--275, 287, 293, 478, and 483 were rendered and
inspected. The fourteen-product matrix repeats its header correctly across both continuation
pages; pages 254 and 265 are intentional chapter-boundary blanks. No changed page has
...
| 9 | GraphicsDevice | 1220 (was 966) | 90 | Full method/lifecycle coverage, adapter and format-query limits, draw-matrix extraction, render-target/MRT and capability findings, plus corrected stencil/depth-clear history. Recent passes establish requested-vs-native presentation state and correct the three recovery/debug extensions: recovery changes shared Texture2D shadow lifetime even behind an empty hook, Vulkan alone emits markers, and D3D9 throws for both policy and marker calls after the policy call has already changed shared state. The latest pass distinguishes the four Clear overloads, FNA-style attachment masking, the shared depth-format/cube gaps, and fourteen backend execution models. | **In progress (~31 of ~90 pages, PDF-verified clean)** |
