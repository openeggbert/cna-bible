...
**Status: Phase 1 execution is active. The current book is 535 physical PDF pages
...
concurrency/reentrancy, content-manifest introspection, format-selection/fallback, and
ResourceContentManager/OpenStream audits.
`main.pdf` is 535 pages; the targeted undefined-reference and duplicate-label checks are empty;
makeindex accepted 2,166 entries with no
rejection or warning.
Current physical pages 50, 58--59, 60, 63--64, 103--116,
...
| 8 | Content and Assets | 972 (was 242) | 55 | Added ContentManager reference + manifest worked example; (depth pass) quoted two real .cnj files directly from cnj.md (a self-contained SpriteFont document and a Texture2D sourceFile/colorKey metadata sidecar); fixed two pre-existing overfull-hboxes; (fifth pass, mined `plan_xnb.md`'s Phase G, complete 2026-07-16, not previously used) **the chapter's own ".xnb reader still growing" framing was itself stale**: every phase through Phase G is now complete, so updated the framing to "largely complete"; added two new subsections --- a real hardening pass finding 3 more heap-buffer-overflows via deterministic whole-container fuzzing (Texture2D/3D/Cube byteCount-vs-dimension validation, a dormant Texture3D compressed-volume bug, DxtUtil's unenforced dataSize parameter), and the custom-reader extension point verified against a real third-party reader (prime31/Nez's BitmapFontReader) plus the ReflectiveReader<T> permanent scope decision; incidentally cross-checked a potential plan_xnb.md/plan_net.md conflict about NetworkSession::Dispose() idempotency and confirmed Ch.30's own claim (already fixed) is accurate, no correction needed; (sixth pass) added a new closing section on the real SoundEffect/Song/Video .xnb content readers (previously only the graphics-side readers were covered in depth), grounded directly in the real reader source; (seventh pass) corrected the worked `Unload()` comment and traced generic, weak-texture, and move-only cache ownership, disposal, manifest persistence, and the real hotswap example; (eighth pass) established that `Game::setContentProperty()` memberwise-copies a manager rather than replacing it like FNA, including copied cache state and raw-device lifetime hazards; (ninth pass) corrected the provider constructor's stale promise of service-based graphics resolution and documents the mandatory explicit device association; (tenth pass) corrected the custom-XNB registration API and maps the independent global and per-manager reader lifetimes, replacement rules, and known boundaries; (eleventh pass) establishes the root/path/caching boundary, including traversal and absolute-name escapes, root-switch cache staleness, and nested-reference differences; (twelfth pass) maps `Load<T>()`'s non-uniform exception boundary and success-only cache write, including FNA's stricter preconditions and the limits of failed-load retry semantics; (thirteenth pass) establishes the unsupported concurrent and reentrant mutation boundary, including the active-reader self-replacement lifetime hazard; (fourteenth pass) corrects the manifest helper from a resolver claim to an explicitly bounded, unsorted diagnostic inventory, including candidate-only semantics, compressed/malformed-XNB ambiguity, stale snapshot and live-registration distinction, and returned-reference lifetime; (fifteenth pass) replaces a simplified three-way resolver story with the exact XNB/literal/CNJ/native sequence, terminal-candidate behavior, explicit-XNB naming trap, type-specific ordered extensions, and targeted coverage gaps; (sixteenth pass) establishes ResourceContentManager as a documented surface-only stub, including its inert virtual stream hook, loose-load inheritance, FNA ResourceManager divergence, and a needs-human embedded-resource implementation boundary | **In progress (~21 of ~55 pages, PDF-verified clean)** |
...
current fully rebuilt total of 519 physical PDF pages.
