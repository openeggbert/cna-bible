...
**Status: Phase 1 execution is active. The current book is 486 physical PDF pages
...
The latest full-book rebuild includes the draw-fallback, instancing, and vertex-binding
audit. `main.pdf` is 486 pages; the targeted undefined-reference and duplicate-label checks
are empty; makeindex accepted 2,026 entries with no rejection or warning. The affected ranges
were rendered and inspected: Ch.9 physical pages 114--115, Ch.16 pages 184--187, Ch.20 page
226, Ch.22 pages 245--246, and Appendix A page 453. The new backend matrix fits cleanly
without clipping or cell collision; no changed page has a page-edge spill, malformed heading,
running-header collision, or folio defect. All overfull warnings introduced by the batch were
removed before the final build. `NEXT.md` records the source findings, commands, and unpushed
local commit state.
...
| 16 | Backend Architecture | 826 (was 222) | 40 | Deep dives now cover SpriteBatch, render targets, effects, occlusion queries, index/vertex buffers, texture/readback/update defaults, and build-time selection. The latest matrix proves the ordinary `Draw*Ex` fallbacks dead, but the shared instancing throw live on SDL GPU and Software; it classifies all fourteen products, missing-instance behavior, fixed-shader limits, and the strongest located test evidence. | **In progress (~13 of ~40 pages, PDF-verified clean)** |
...
| 25 | DX3/free-direct Backend | 264 (was 129) | 30 | Covers alpha blending, Wrap/Mirror sampling, shadow-backed render targets, the 32bpp design boundary, real bidirectional letterbox input mapping, and the still-open free-direct-side resize/presentation mismatch. | **In progress (~6 of ~30 physical pages, PDF-verified clean)** |
...
| F | API Quick Reference: Ecosystem and Platforms | 245 (was 181) | 25 | Covers Input, Audio/Media, Devices/Sensors, Networking/GamerServices, Storage, and core sharp-runtime types; the latest pass adds MemoryStream construction, close, extraction-lifetime, and overflow behavior. | **In progress (~6 of ~25 physical pages, PDF-verified clean)** |
...
current fully rebuilt total of 478 physical PDF pages.
