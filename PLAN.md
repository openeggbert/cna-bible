...
**Status: Phase 1 execution is active. The current book is 507 physical PDF pages
...
The latest full-book rebuild includes the context-recovery/debug-hook/default audit.
`main.pdf` is 507 pages; the targeted undefined-reference and duplicate-label checks are
empty; makeindex accepted 2,080 entries with no rejection or warning. The affected ranges
were rendered and inspected: Ch.6 physical page 58, Ch.9 page 121, Ch.16 pages 205--207,
Ch.18 page 218, Ch.23 page 262, Ch.40 page 410, and Appendix E page 489. The new backend
matrix fits on one page with its complete final row; no changed page has clipping, cell
collision, page-edge spill, malformed heading, running-header collision, or folio defect.
All overfull warnings introduced by the batch were removed before the final build.
...
| 6 | Game Loop | 513 (was 216) | 40 | Covers GameServiceContainer, PresentationMode, a full DrawableGameComponent example, fixed-timestep catch-up and `IsRunningSlowly` hysteresis, and the default-fixed-timestep `ResetElapsedTime()` no-op. Recent contract passes separate timestep scheduling from VSync blocking and document the hidden global event-pump behavior: input sees non-repeated F9/F10 first, then the framework invokes backend loss/restore hooks. | **In progress (~10 of ~40 pages, PDF-verified clean)** |
...
| 9 | GraphicsDevice | 1167 (was 966) | 90 | Full method/lifecycle coverage, adapter and format-query limits, draw-matrix extraction, render-target/MRT and capability findings, plus corrected stencil/depth-clear history. Recent passes establish requested-vs-native presentation state and correct the three recovery/debug extensions: recovery changes shared Texture2D shadow lifetime even behind an empty hook, Vulkan alone emits markers, and D3D9 throws for both policy and marker calls after the policy call has already changed shared state. | **In progress (~31 of ~90 pages, PDF-verified clean)** |
...
| 16 | Backend Architecture | 1433 (was 222) | 40 | Deep dives now cover SpriteBatch, render targets, effects, occlusion queries, index/vertex buffers, texture/readback/update defaults, input-coordinate routing, swap intervals, presentation formats/fullscreen, recovery/debug hooks, and build-time selection. The latest fourteen-product matrix separates the universal Texture2D-shadow consequence from EasyGL registry behavior, EasyGL/D3D9's two real loss channels, Vulkan's deferred native marker, D3D9's throwing marker, and the ten source-only no-op products. | **In progress (27 physical pages, PDF-verified clean, ~27 of ~40 target pages)** |
