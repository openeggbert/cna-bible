...
**Status: Phase 1 execution is active. The current book is 490 physical PDF pages
...
The latest full-book rebuild includes the logical/window input-coordinate audit.
`main.pdf` is 490 pages; the targeted undefined-reference and duplicate-label checks are
empty; makeindex accepted 2,032 entries with no rejection or warning. The affected ranges
were rendered and inspected: Ch.16 physical pages 181--184, Ch.25 pages 273--275, Ch.26
pages 281--283, and Appendix F pages 478--480. The new backend matrix repeats its header
cleanly across the page break; no changed page has clipping, cell collision, page-edge spill,
malformed heading, running-header collision, or folio defect. All overfull warnings
introduced by the batch were removed before the final build. `NEXT.md` records the source
findings, commands, and unpushed local commit state.
...
| 9 | GraphicsDevice | 1097 (was 966) | 90 | Full method/lifecycle coverage, adapter and format-query limits, draw-matrix extraction, render-target/MRT and capability findings, plus corrected stencil/depth-clear history. The latest pass documents the actual `DrawInstancedPrimitives` bridge: only the first positive-frequency binding reaches a backend, while frequency, offsets, and later streams are lost. It also records the independent singular/plural binding-state defect and the missing hardware-instancing capability query. | **In progress (~29 of ~90 pages, PDF-verified clean)** |
...
| 16 | Backend Architecture | 935 (was 222) | 40 | Deep dives now cover SpriteBatch, render targets, effects, occlusion queries, index/vertex buffers, texture/readback/update defaults, input-coordinate routing, and build-time selection. The latest fourteen-product matrix distinguishes the SDL-renderer, registered-backend, and raw-pass-through tiers; records incomplete presentation-mode transforms, WebGPU/SDL GPU's window-vs-physical-pixel defect, DX3's shadowed helper math, Vulkan's resize mismatch, and the actual test evidence. | **In progress (16 physical pages, PDF-verified clean, ~16 of ~40 target pages)** |
...
| 22 | SDL GPU Backend | 505 (new chapter) | 45 | Added by explicit author decision in 2026-07-21. Covers shader binding/runtime compilation, Y-flip and uniform-limit fixes, resource lifetimes, present/recovery behavior, validation, custom effects, pipeline/sampler state, 2D/3D ordering, and real two-output MRT. Its open-limit list now counts nine, adding the live higher-mip `Texture2D` discard and inherited instancing throw. | **In progress (~11 of ~45 physical pages, PDF-verified clean)** |
| 23 | Direct3D Backends | 731 (was 268) | 85 | Covers the D3D9 XNA oracle and native limits; D3D11 lifetime, draw-proof, MRT finalization, and presentation behavior; and D3D12 headless testing, synchronization, resource budgets, MSAA, and mip-generation architecture. Latest D3D9 Texture3D and render-target claims were corrected against the live smoke tests. The 2026-07-25 ShaderEffect pass adds the exact shared D3D11/D3D12 fixed ABI: a 32-byte SpriteBatch vertex, a name-ignored 128-byte uniform layout, inherited no-op array/extra-texture setters, D3D11's separately bound shader/input/buffer state versus D3D12's up-front fixed single-target PSO, and the real red-to-cyan public-API HLSL proof used by both smoke tests | **In progress (14 physical pages, PDF-verified clean, ~14 of ~85 target pages)** |
...
| A | API Quick Reference | 363 (was 229) | 40 | Covers `GraphicsDevice`, state objects, buffers, effects, occlusion queries, `SpriteFont`, and core model types. The latest concise warning adds the instanced-draw binding loss, stale singular/plural binding state, and backend-dependent throw/fallback/silent-return boundary alongside the existing Texture2D shadow/mip warning. | **In progress (~9 of ~40 pages, PDF-verified clean)** |
...
current fully rebuilt total of 490 physical PDF pages.
