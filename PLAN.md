...
**Status: Phase 1 execution is active. The current book is 549 physical PDF pages
...
Decimal/DateTime-reader, CurveReader, Texture2DReader, Texture3DReader, and TextureCubeReader
audits.
`main.pdf` is 549 pages; the targeted undefined-reference and duplicate-label checks are empty;
makeindex accepted 2,240 entries with no
rejection or warning.
The current ContentReader/ContentTypeReader/XNB-container/type-name/scalar/Decimal/Curve/Texture2D/Texture3D/TextureCube pages 105--117, together with the
...
| 8 | Content and Assets | 1,735 (was 242) | 55 | The current twenty-seven-pass audit covers XNB/CNJ readers, exact resolution and cache semantics, error/type/concurrency boundaries, manifests, service/device requirements, and ResourceContentManager's explicit embedded-resource stub. The latest TextureCubeReader pass records the actual six-face DXT1 fixture and face/mip order, but also the current raw-Color byte-count out-of-bounds read, unbounded aggregate cube data and level topology, absent generic cube-backend admission, and moved-from direct existing-instance behavior; its equality guard and checked arithmetic exist only on unmerged `feature/audit`. The preceding Texture3DReader pass distinguishes the current fixed per-slice DXT decoding and raw-Color byte check from unchecked signed arithmetic, unbounded mip topology, unsupported formats, missing generic 3D capability admission, and the absent real XNB fixture. Earlier passes confirm CurveReader's unchecked loop/continuity casts, unvalidated float keys, copied-and-appended direct existing-instance behavior, no checked-in XNB Curve fixture, and absent key-count limit; identify Decimal's MSVC absence, reserved-flag/signed-zero acceptance divergence, and DateTime Kind loss/invalid-kind acceptance; map 26 scalar/math readers' literal layouts and enum gap; and establish canonical factory-key/table-admission and container hardening gaps. | **In progress (~33 of ~55 pages, PDF-verified clean)** |
