...
**Status: Phase 1 execution is active. The current book is 478 physical PDF pages
...
The latest forced full-book rebuild includes the completed end-to-end audit of
`MultiSampleAntiAliasing` and `AnisotropicFiltering`. `main.pdf` is 478 pages; the targeted
undefined-reference and duplicate-label checks are empty; makeindex accepted 1,978 entries
with no rejection or warning. The final affected ranges were rendered and inspected:
Ch.9 physical pages 122--124, Ch.14 pages 166--168, Ch.16 pages 182--186, Ch.22 pages
231--233, Ch.23 pages 239--241, Ch.24 pages 257--259, and Appendix C pages 451--453.
The new two-page capability table repeats its header correctly and has no clipping or cell
collision; no changed page has a page-edge spill, malformed table, running-header, or folio
defect. `NEXT.md` records the source findings, commands, and unpushed local commit state.
...
| 21 | WebGPU Backend | 325 (was 178) | 45 | Covers PbrEffect, compressed-upload architecture, RenderTargetCube sampling/pass isolation, and the current tested state of texture readback, culling, scissor, and viewport; retains the narrower real gaps. | **In progress (~8 of ~45 physical pages, PDF-verified clean)** |
