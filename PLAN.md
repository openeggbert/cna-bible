...
resource-lifecycle, construction/SDL-ownership, multisample-backend-rebuild, and
Game/GraphicsDeviceManager lifecycle audits. `main.pdf` is 523 pages; the targeted
undefined-reference and duplicate-label checks are empty; makeindex accepted 2,124 entries with
no rejection or warning. Current physical pages 50, 60, 63--64, 110--111, 114--115, 124--125,
and 126--127 were rendered and inspected.
...
resource-lifecycle, one construction-drafting, one multisample-drafting, and all
manager-lifecycle drafting boxes without repeating that global comparison
...
| 6 | Game Loop | 518 (was 216) | 40 | Covers GameServiceContainer, PresentationMode, a full DrawableGameComponent example, fixed-timestep catch-up and `IsRunningSlowly` hysteresis, and the default-fixed-timestep `ResetElapsedTime()` no-op. Recent contract passes separate timestep scheduling from VSync blocking and document the hidden global event-pump behavior: input sees non-repeated F9/F10 first, then the framework invokes backend loss/restore hooks. The current audit corrects `DrawableGameComponent`'s nonexistent device-service subscription and the manager's inert selection virtuals. | **In progress (~10 of ~40 pages, PDF-verified clean)** |
