...
| 25 | DX3/free-direct Backend | 264 (was 129) | 30 | Fixed a stale plain-text cross-reference ("Chapters~6 and~5") to real `\ref{}`s; added a worked example with real `BlendState::AlphaBlend` factors (verified against `BlendState.cpp`) showing why the bug is visible for `AlphaBlend` but was harmless for `Opaque`'s own degenerate factors — a math error caught and corrected in this session's own draft before commit; (depth pass) added a direct, parallel worked example to Ch.17's SDL_RENDERER Wrap/Mirror gap, adapted from the real `dx3_sampling_test.cpp` (identical 2-texel red/green setup, this backend gets the correct wrap/mirror/clamp answer where SDL_RENDERER cannot); (third pass) added a `RenderTarget2D` section: the shadow-backbuffer trick reused per-target via `Dx3RenderTargetBackend`'s own private offscreen surface, the `Impl::ActiveSurface()` bind-redirect verified with two distinct clear colors (`dx3_texture_rendertarget_test.cpp`), the zero-code `DiscardContents` reuse of shared `GraphicsDevice.cpp` logic, the honest MRT throw, and the 4096x4096 dimension cap thrown honestly rather than silently truncated; (fourth pass) added a section on design decision 4 (32bpp-only surfaces) and how it structurally sidesteps two real, independently-documented free-direct gaps (the discarded `SetDisplayMode` `dwBPP` parameter; `BlitFrom`'s mixed-depth silent-no-op), grounded in `free-direct`'s own `docs/directdraw-limitations.md` and confirmed against the real `Dx3GraphicsBackend.cpp` source; (fifth pass) documented the separately-tested, bidirectional logical/window transform that reproduces free-direct's fixed centered-letterbox scale and offsets without access to its private renderer, including the live-window-size test's round-trip, centering, uniform-scale, and fit-largest checks; resolved all seven pre-existing isolated-Ch.25 overfull boxes while validating it; (sixth pass) documented the separate post-first-present virtual-resolution bug: CNA recreates its logical surfaces but free-direct retains the old one-time logical-presentation rectangle, so physical output and the otherwise-correct recomputed input transform can diverge | **In progress (~6 of ~30 pages; isolated LaTeX/PDF-verified clean; full-book rebuild pending)** |
